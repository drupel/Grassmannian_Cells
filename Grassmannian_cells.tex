...
It turns out that all $2$-quivers can be built from the $2$-quivers of the lifts $\tilde P_1$, $\tilde P_2$ and $\tilde P_2^{[n-1]}=\tilde P_2^{(1,\ldots,n-1)}$ as defined in Section \ref{Lifting}, i.e. we have:
\[
  \tilde P_2^{[n-1]}=(1,\alpha_1)\xlongleftarrow{\alpha_1}(2,1).%=P_1^{(1)}=j^{(1)}_1\xlongleftarrow{v_1}i^{(1)}_1
\]
We define $P_m^{[k]}:=F_{K(n)}\tilde P_m^{[k]}$.
%In other words, $P_2^{(n-1)}$ is chosen as be the cokernel of a short exact sequence
%\[
%  \ses{ P_1^{n-1}}{ P_2}{P_2^{(n-1)}}.
%\]
Choosing the lifts of $P_1$ supported at $(1,\alpha_2),\ldots,(1,\alpha_n)$, we have 
\begin{align}\label{eq:case2}
  \Ext( P_2^{[n-1]},P_1)\cong \bigoplus_{i=2}^{n}\Ext(\tilde P_2^{[n-1]},\alpha_i\tilde P_{1})\cong\langle (2,1)\xrightarrow{\alpha_i}(1,\alpha_i)\mid i=2,\ldots,n\rangle.
\end{align}

This defines recursively the truncated preprojective representation $\tilde P_{2}^{[k]}$ for $k=n-1,\ldots,0$ with $\tilde P_2^{[0]}=\tilde P_2$ by 
\begin{align}
  \label{coeff}\xymatrix@R10pt@C20pt{&&(1,\alpha_1)\\ (2,1)\ar^{\alpha_1}[rru]\ar^{\alpha_{n-k}}[rrd] &&\vdots\\&&(1,\alpha_{n-k})}.
\end{align}
Note that, for general $m\geq 2$, we have a short exact sequence
...
We define the $2$-quiver of $P_m^{[k]}$ by
\\
