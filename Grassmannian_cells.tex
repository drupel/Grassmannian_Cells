...
\begin{remark}
  \footnote{we should have $P_m=(\Sigma_iP_{m-1})^T$}
  \sayD{We need to change the conventions above to include notation for the equivalence between $\rep K(n)$ and $\rep K(n)^{op}$.}
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For every $\uj\in I_l$, there exists a truncated preprojective representation $\tilde P_{m}^{\uj}$ which is the cokernel of
      ...
      On the universal covering, we have $\Hom(\tilde P_{m-1,k},\tilde P_m)\cong\Hom(\tilde P_{m-1,k},\tilde P_m^{\uj})=\kk$  where $k\neq j_i$ for $i=1,\ldots,l$, $\Hom(\tilde P_m,\tilde P_m^{\uj})=\kk$, $\Ext(\tilde P_{m},\tilde P_{m}^{\uj})=0$ and $\Ext(\tilde P_{m}^{\uj},\tilde P_{m}^{\uj})=0$.
  \end{enumerate}
\end{lemma} 
\begin{proof}
...
\begin{lemma}
  \label{le:subrep}
  \sayD{This should be moved to a statement on $K(n)$ and then deduced from that one.}
  Let $\uj\in I_l$.
...
\begin{lemma}
  ...
  \sayD{Part (1) should follow from Corollary~\ref{cor:truncated preprojective isomorphism}.  Do parts (2), (3), (4) have analogues for $K(n)$?  The second half of (3) and part (5) should follow from Lemma~\ref{le:basic homological properties}.  The first half of (4) is roughly the same as Lemma~\ref{le:truncated extensions}.}
...
\begin{proposition}
  ...
  Let $m\geq 1$.
  Let $\alpha(m,r)=\dim P_{m+1}-r\dim P_m$ with $0\leq r\leq n-1$.
  Let $R_\alpha(K(n))$ be the affine space of representations of dimension $\alpha(m,r)$.
  The following hold:
