...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For any pair $i,j\in\{1,\ldots,n\}$ with $i\ne j$ and any $J\subsetneq\{i\}^c$, there is a commutative diagram
      ...
      \end{equation}
    \item For any admissible word $\ui\in A_1^{(k)}$ with $1\le k\le m$ and any proper subsets $I,J\subsetneq\{1,\ldots,n\}$ with $i_k\in I$ and $i_1\notin J$, there exist (unique up to scale) surjective maps $\tilde\pi_{m+1}^{\ui,J,I}:\tilde P_{m+1}^J\to \tilde P_{m+1-k,\ui}^I$ satisfying:
      \[\tilde\pi_{m+1}^{\ui,J,I}\Big|_{\tilde P_{m+1-\ell,\ui_\ell}}=\tilde\pi_{m+1-\ell,\ui_\ell}^{\ui'_\ell,I}
      \qquad\text{and}\qquad
    \tilde\pi_{m+1}^{\ui,J,I}\Big|_{\tilde P_{m+1-k,\ui}}=\tilde\pi_{m+1-k,\ui}^I\]
      for $\ell<k$, where $\ui_\ell=(i_1,\ldots,i_\ell)$ and $\ui'_\ell=(i_{\ell+1},\ldots,i_k)$.
  \end{enumerate}
\end{lemma}
\begin{remark}
  Note that we may take $J=\varnothing$ in Lemma~\ref{le:truncated quotients}.3 but the conditions impose $I\ne\varnothing$.
\end{remark}
\begin{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    \item[($\dagger$)] For any $E\in\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$ such that $\pi_{m+1}^{J,\bfI}(E)\neq 0$, we have $\pi_{m+1}^{J,\bfI}(E')\neq 0$ for all $E'\in C_E$, where $C_E$ is the affine cell which contains $E$.
  \end{itemize}
  \vs
