...
In Section \ref{sec:combinatorics}, we reveal the combinatorics behind the obtained cell decompositions introducing the notion of $2$-quivers which is a slight generalization of the notion of usual quivers. Theorem \ref{thm:2quivercells} can be formulated as follows.\begin{thm}
With every truncated preprojective representation $P_{m+1}^V$ we can associated a $2$-quiver $\mathcal Q_{m+1}^{[\dim V]}$ such that the cells of the cell decomposition attached to $\Gr_\bfe(P_{m+1}^V)$ are labeled by strong successor closed subsets $\beta\subset (\mathcal Q_{m+1}^{[\dim V]})_0$. In particular, the Euler characteristic $\chi(\Gr_\bfe(P_{m+1}^V))$ is given by the number of these subsets.
\end{thm}
\sayT{Insert something about Dyck path combinatorics and the correspondence here}
\sayT{maybe we can erase some parts which show up twice in the introduction?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{remark}
  ...
  However, there does not seem to be a reasonable analogue of part (2) when considering Calcero-Chapoton maps for $K(n)$.
\end{remark}
\begin{proof}
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item $\cQ_V$ as the $2$-quiver obtained from $\cQ$ when replacing the $2$-arrow $V$ by a usual arrow $\alpha_V:p\to q$. 
      \sayD{It doesn't seem to be part of the definition that there must exist such an arrow in $Q$.}
  \end{enumerate}
...
We aim to construct $\cQ_{m+1}^{[k]}$ so that the strong successor closed subsets are in one-to-one correspondence with the cells of quiver Grassmannians of~$\tilde P_{m+1}^{[k]}$.
...
In short, the $2$-quiver $\mathcal Q_{m+1}^{[k]}$ should be viewed as being attached to any (truncated) preprojective representation of the appropriate dimension.
...
\begin{remark}\mbox{}
  \begin{enumerate}
    \item It will turn out that every vertex of the quiver $\mathcal Q_{m+1}^{[k]}$ uniquely corresponds to a vertex of the universal covering quiver $\widetilde{K(n)}$, but that different vertices of $\cQ_{m+1}^{[k]}$ can correspond to the same vertex of $\widetilde{K(n)}$.
      Also every usual arrow of $\mathcal Q_{m+1}^{[k]}$ does correspond to an arrow of $\widetilde{K(n)}$ and thus to an arrow of $K(n)$.
      In particular, the quiver obtained from $\mathcal Q_{m+1}^{[k]}$ when forgetting the $2$-arrows gives rise to a (possibly non-disjoint) union of subquivers of $\widetilde{K(n)}$.
  ...
  \item Recall that the representation $\tilde P_2^{[n-1]}$ is defined by the support quiver
    ...
    \begin{align}
      \label{eq:case2}
      \Ext\big(P_2^{[n-1]},P_1\big)
      \cong\bigoplus_{i=2}^{n}\Ext\big(\tilde P_2^{[n-1]},\tilde P_{1,i}\big)
      \cong\big\langle (2,1)\xrightarrow{\alpha_i}(1,\alpha_i)\mid i=2,\ldots,n\big\rangle.
    \end{align}

  \item  For $m\geq 2$, recall that we have $\dim\Hom\big(P_m,\tau P_{m+1}^{[k]}\big)=\dim\Ext\big(P_{m+1}^{[k]},P_m\big)$ by the Auslander-Reiten formula.
    This induces isomorphisms
    \begin{align}
      \label{eq:homext} 
      \bigoplus_{i=k+1}^n \Hom\big(\alpha_{i}^{(-1)^{m+1}}.\tilde P_m,\tau\tilde P_{m+1}^{[k]}\big)
      \cong\bigoplus_{i=k+1}^n \Ext\big(\tilde P_{m+1}^{[k]},\alpha_{i}^{(-1)^{m+1}}.\tilde P_m\big)
      \cong\Ext\big(P_{m+1}^{[k]},P_m\big).
    \end{align}
    This means that we can choose a basis of $\Ext\big(P_{m+1}^{[k]},P_m\big)$ which is compatible with the lifts to the universal covering and which gives a one-to-one correspondence between the basis elements of $\Ext\big(P_{m+1}^{[k]},P_m\big)$ and $\Hom\big(P_m,\tau P_{m+1}^{[k]}\big)$. 
    In view of Corollary~\ref{cor:perpendicular}, we have also an isomorphism 
    \[\Ext\big(\tilde P_{m+1}^{[k]},\tilde P_{m,k}\big)\cong\Ext\big(\tilde P_{m+1}^{[k]},\tau\tilde P_{m+1}^{[k]}\big).\]
    These isomorphisms will also be present in the structure of the $2$-quivers $\cQ_{m+1}^{[k]}$.
    Finally note that there exists a short exact sequence
    \[\ses{\Hom(P_m,P_m)}{\Ext\big(P_{m+1}^{[k]},P_m\big)}{\Ext\big(P_{m+1}^{[k+1]},P_m\big)}.\]
    Thus a basis of $\Ext\big(P_{m+1}^{[k]},P_m\big)$ can be obtained by taking the last $k$ basis elements of a basis of $\Ext\big(P_{m+1}^{[n-1]},P_m\big)$.
  \end{enumerate}
\end{remark}

The $2$-quivers $\cQ_{m+1}^{[k]}$ will be built inductively.
To begin, the $2$-quiver $\mathcal Q_1=\cQ_1^{[0]}$ of $\tilde P_1$ is defined simply as the quiver consisting of a single vertex $(1,e)$.
Moreover, we define the translated $2$-quivers $\cQ_{1,i}$ by the vertices $(1,\alpha_i)$.
Using (\ref{eq:case2}), we recursively construct the (truncated) preprojective representations $\tilde P_{2}^{[k]}$ for $k=n-1,\ldots,0$ using their support quivers
\begin{align}
  \label{coeff}
...
In abuse of notation, we also denote the support quiver by $\mathcal Q_{2}^{[k]}$.
In order to keep the illustrations and combinatorics simple, we will work with this quiver instead.
We also define the translated two quivers $\cQ_{2,i}$ as those obtained from $\cQ_2$ by translating all vertices and ($2$-)arrows by $\alpha_i^{-1}$.

The next step is to define the $2$-quivers $\cQ^{[k]}_3$ of the (truncated) preprojective lifts $\tilde P^{[k]}_3$ whence the remaining $2$-quivers attached to the (truncated) preprojectives $\tilde P_m^{[k]}$ can be obtained recursively.
First, we define
\[\cQ_3^{[n-1]}:=\alpha_{n}^{-1}.\cQ_2^{[1]},\]
i.e. we translate all vertices -- understood as vertices of $\widetilde{K(n)}$ -- and (2-)arrows by $\alpha_n^{-1}$.
Then $\cQ_3^{[k]}$ can be found as a sub-2-quiver of $\cQ_3$ which is constructed recursively by connecting $\cQ_3^{[i]}$ to $\cQ_{2,i}$ for $i=n-1,\ldots, 1$ in the following way: \\
\[
...
Observe that the middle vertex of $\mathcal Q_3^{[n-1]}$ corresponds to $(2,\alpha_n^{-1})$ and that the target of the $2$-arrow, again colored red, into $\cQ_{2,n-1}$ is the vertex corresponding to $(1,e)$ of $\widetilde{K(n)}$.
In general, the 2-arrow from $\cQ_3^{[i]}$ into $\cQ_{2,i}$ has target the vertex corresponding to $(1,e)$ in $\widetilde{K(n)}$.
We again define translated versions $\cQ_{3,i}:=\alpha_i.\cQ_3$ of $\cQ_3$ which again means that we translate $\cQ_{3}$ as a union subquiver contained in $\widetilde{K(n)}$.
...
Note that the image of the homomorphism corresponding to the basis element spanning 
\[\Hom(\alpha_{k}^{-1}.\tilde P_{2},\tau\tilde P_{m}^{[k]})\cong\Ext(\tilde P_3^{[n-1]},\alpha_k^{-1}\tilde P_2)\]
corresponds to the subquiver $(2,\alpha_k^{-1})\xrightarrow{\alpha_k} (1,e)$.
This translates to the fact that $\cQ_3^{[k]}$ is glued to the image of the homomorphism $\tilde P_{2,k}\to \tau \tilde P_3^{[k]}$ which is of dimension $(1,1)$. In view of Lemma~\ref{lem:2equivalence}, we forgo inserting the corresponding rectangles around the subquiver and just connect the $2$-arrows to the sinks of the corresponding subquivers of dimension $(1,1)$. 
...
Let us assume that we already constructed the $2$-quivers $\cQ_{l}^{[j]}$ of $P_{l}^{[j]}$ for $l\leq m$, $j=0,\ldots, n-1$ and $l=m+1$, $j=n-1,\ldots,k$. As before, we define the translated $2$-quivers $\cQ_{m,i}=\alpha_i^{(-1)^{m+1}}.\cQ_m$.
Moreover, we define 
\[\cQ_{m+1}^{[n-1]}:=\begin{cases}\alpha_{n}^{-1}.\cQ_{m}^{[1]}\text{ if }m\text{ is even}\\\alpha_{1}.\cQ_{m}^{[1]}\text{ if }m\text{ is odd}\end{cases}.\]
...
\begin{theorem}\label{thm:2quivercells}
\begin{enumerate}
\item The cells of the cell decomposition of $\Gr_\tbfe(\tilde P_m^{[k]})$ (resp. $Gr_\bfe( P_m^{[k]})$) induced by Theorem \ref{cellscover}  can be labeled by strong successor closed subsets $\beta\subset\mathcal Q_m^{[k]}$ of dimension type $\tbfe\in\NN^{\widetilde{K(n)_0}}$ (resp. $\bfe\in\NN^{K(n)_0}$) yielding a one-to-one correspondence between cells and strong successor closed subsets.

\item  For $\tbfe\in\NN^{\widetilde{K(n)_0}}$ (resp. $\bfe\in\NN^{K(n)_0}$), the Euler characteristic $\chi(\Gr_{\tbfe }(\tilde P_m^{[k]}))$ (resp. $\chi(\Gr_{\bfe }(P_m^{[k]}))$) is given by the number of strong successor closed subsets of dimension type $\tbfe$  (resp. $\bfe$) of the $2$-quiver of $\mathcal Q_m^{[k]}$.
...
We proceed by induction on $m$ and $k$. The case of the representation $\tilde P_1$ is trivial. We have $(\tilde P_2^{[k]})_q\in\{0,1\}$ for all $q\in\widetilde{K(n)}$, whence the  subrepresentations are in one-to-one correspondence with the successor closed subsets of the quiver (\ref{coeff}) which is equivalent to  $\mathcal Q_2^{[k]}$ . Equivalently, we have $\Gr_{\tbfe}(\tilde P_2^{[k]})\in\{\varnothing,\{\pt\}\}$ such that $\Gr_{\tbfe}(\tilde P_2^{[k]})=\{\pt\}$ if and only if $\tbfe\subset \mathcal Q_2^{[k]}$ is strong successor closed.
 ...
 % Denote by $d(\beta_1+\beta_2)$ the dimension vector corresponding to this subset.
  ...
  But this is precisely the condition on $\beta_1\coprod\beta_2$ to be strong successor closed as $\mathcal Q_{m+1}^{[k]}$ is connected to $\mathcal Q_{m-1}^{[k]}$ by a $2$-arrow. This shows the first claim.
\end{proof}
