...
\begin{itemize}
  ...
  \item $A_1=\bigoplus_{i=1}^n \CC a_i$ is the $A_0$-bimodule spanned by the arrows of $K(n)$, in particular $e_ka_ie_\ell=\delta_{k1}\delta_{\ell2}a_i$ for $1\le i\le n$ and $k,\ell\in\{1,2\}$.
\end{itemize}
...
\begin{lemma}
  For any morphism $\theta:M\to N$ between representations $M,N\in\rep K(n)$, the same maps $\theta_1:M_1\to N_1$ and $\theta_2:M_2\to N_2$ give a morphism $\theta:g.M\to g.N$ for any $g\in GL_n(\CC)$.
\end{lemma}
\begin{proof}
  ...
  for $1\le i\le n$ so that $\theta$ gives a morphism from $g.M$ to $g.N$. 
\end{proof}

Write $\Sigma_1$ and $\Sigma_2$ for the BGP-reflection functors of $K(n)$ \cite{bgp}. 
Note that (up to relabeling vertices) the quiver $K(n)$ itself does not change when applying the reflection functors and thus we view these as endofunctors acting on the category $\rep K(n)$ of finite-dimensional representations. %, but the vertices $1$ and $2$ are exchanged. 
...
\begin{lemma}
  The reflection functors $\Sigma_i:\rep K(n)\to\rep K(n)$ are $GL_n(\CC)$-equivariant. 
\end{lemma}
...
Incorporate these into other sections...
...
Define quiver representations and quiver Grassmannians...
