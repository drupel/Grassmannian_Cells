...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For any pair $i,j\in\{1,\ldots,n\}$ with $i\ne j$, there is a commutative diagram
      \begin{equation}
        \label{dia:truncated coherence}
        \xymatrix@R40pt@C60pt{\tilde P_{m+1} \ar^(.4){\tilde\pi_{m+1}^{\{i\}^c}}[d] & \ar[l] \tilde P_{m,i} \ar_(.35){\tilde\pi_{m,i}^{\{i\}}}[dl]\ar^(.4){\tilde\pi_{m,i}^{\{j\}^c}}[d] & \ar[l] \tilde P_{m-1,(i,j)} \ar_(.35){\tilde\pi_{m-1,(i,j)}^{\{j\}}}[dl]\\
        \tilde P_{m+1}^{\{i\}^c}\cong\tilde P_{m,i}^{\{i\}}\ar^{\tilde\pi_{m,i}^{\{i\},\{j\}^c}}[r] & \tilde P_{m,i}^{\{j\}^c}\cong \tilde P_{m-1,(i,j)}^{\{j\}}}
      \end{equation}
    \item For any admissible word $\ui\in A_1^{(k)}$ with $1\le k\le m$ and any proper subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$, there exist (unique up to scale) surjective maps $\tilde\pi_{m+1}^{\ui,I}:\tilde P_{m+1}\to \tilde P_{m+1-k,\ui}^I$ satisfying:
      \[\tilde\pi_{m+1}^{\ui,I}\Big|_{\tilde P_{m+1-\ell,\ui_\ell}}=\tilde\pi_{m+1-\ell,\ui_\ell}^{\ui'_\ell,I}
      \qquad\text{and}\qquad
    \tilde\pi_{m+1}^{\ui,I}\Big|_{\tilde P_{m+1-k,\ui}}=\tilde\pi_{m+1-k,\ui}^I\]
      for $\ell<k$, where $\ui_\ell=(i_1,\ldots,i_\ell)$ and $\ui'_\ell=(i_{\ell+1},\ldots,i_k)$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  The commutativity of the upper triangles in (2) follows from the construction of the isomorphisms in the bottom row, while the commutativity of the lower triangle is immediate from the definitions of these maps.

  These diagrams stitch together to give the composition 
  \[\tilde\pi_{m+1}^{\ui,I}:=\tilde\pi_{m+1-k,\ui}^{\{i_k\},I}\circ\tilde\pi_{m+2-k,\ui_{k-1}}^{\{i_{k-1}\},\{i_k\}^c}\circ\cdots\circ\tilde\pi_{m,i_0}^{\{i_1\},\{i_2\}^c}\circ\tilde\pi_{m+1}^{\{i_1\}^c}\]
  which can be seen to satisfy the desired properties from (3) inductively using the commutativity from (2).
\qed\end{proof}
...
Given $\tilde M\in\rep \tilde K(n)$, write
\[\tilde M^\perp := \{\tilde N\in\rep \tilde K(n):\Hom(\tilde M,\tilde N)=0=\Ext(\tilde M,\tilde N)\}.\]
\begin{corollary}
