...
\begin{remark}It will turn out that every vertex of the quiver $\mathcal Q_m^{[k]}$ uniquely corresponds to a vertex of the universal covering quiver $\widetilde{K(n)}$.
  For general $m$, every $2$-arrow $V:\Gamma(1)\to\Gamma(2)$ corresponds to a basis element of $\Ext(\tilde P_{m+1}^{[n-1]},\tilde P_m)$. If the subquivers $\Gamma(1)$ and $\Gamma(2)$ have precisely one source and one sink respectively, the $2$-arrow corresponds to a unique arrow of the original quiver $K(n)$ as every arrow of the universal covering does. In these cases, we label the $2$-arrows by the corresponding arrow.
...
It turns out that all $2$-quivers can be built from the $2$-quivers of $\tilde P_1$, $\tilde P_2$ and $\tilde P_2^{[n-1]}$. Recall that the representation $\tilde P_2^{[n-1]}$ is defined by the support quiver
\[
...
\draw (0,0) node (A) {$(1,\alpha_{n-k})$} +(3,0) node (B) {$(1,\alpha_{n-k+1})$}+(3,-.8) node (P1) {\footnotesize$ \mathcal Q_2^{[k+1]}$} +(6,0) node (B1) {$\dots$} +(9,0) node (C) {$(1,\alpha_{2})$} +(9,-.7) node (P3) {\footnotesize$ \mathcal Q_2^{[n-2]}$}+(11.5,0) node (D) {$(1,\alpha_1)$}  +(14,0) node (E) {$(2,1)$} +(11.5,-.6) node (P4) {\footnotesize$\mathcal Q_2^{[n-1]}$}; 
...
\noindent
where the $2$-arrows are coloured red. This $2$-quiver is clearly equivalent to the quiver (\ref{coeff}). In abuse of notation, we also denote it by $\mathcal Q_{2}^{[k]}$. In order to keep the illustrations and combinatorics simple, we will work with the quiver (\ref{coeff}) instead. For $m\geq 3$, we have
...
Recall that $\tilde P_3^{[n-1]}=\alpha_n^{-1}\tilde P_2^{[1]}$. Moreover, (\ref{eq:homext}) for $m=3$ yields 
...
Note that the image of the homomorphism corresponding to the basis element spanning $\Ext(\tilde P_3^{[n-1]},\alpha_i^{-1}\tilde P_2)$ corresponds to the subquiver $(2,\alpha_i^{-1})\xrightarrow{\alpha_i} (1,1)$ for $i=k+1,\ldots, n$. Thus in this case the $2$-arrow connects the $2$-quiver of $\tilde P_3^{[k]}$ to the image of the corresponding homomorphism $\tilde P_{2,k}\to \tau \tilde P_3^{[k]}$ which is of dimension $(1,1)$. In view of Lemma \ref{lem:2equivalence}, we forgo to insert the corresponding rectangles around the subquiver and just connect the $2$-arrows to the sinks of the corresponding subquivers of dimension $(1,1)$. In summary, the chosen basis of $\Ext(P_3^{[n-1]},P_2)$ motivates the following definition of the $2$-quiver $\mathcal Q_3=\mathcal Q_3^{[0]}$ of $\tilde P_3$ and the $2$-quivers $\mathcal Q_3^{[k]}$ for $k=n-1,\ldots,1$ which can be found as subquivers. 
\\
...
\noindent In abuse of notation, we dropped the labels of the vertices in the illustration. Note that the middle vertex of $\mathcal Q_3^{[n-1]}$ corresponds to $(2,\alpha_n^{-1})$ and that all $2$-arrows, which are again coloured red, end in the vertex corresponding to $(1,1)$ of $\widetilde{K(n)}$. 
...
%\end{align}
Now we can recursively build the $2$-quivers of $ \tilde P_m^{[k]}$ for $m\geq 4$ and $k=n-1,\ldots,0$ when repeating the procedure and considering the natural basis of $\Ext(P_m^{[n-1]},P_{m-1})$ induced by the decomposition (\ref{eq:homext}). 
...
%Recall that we have $\udim P_m^{[k]}=\udim P_{m-1}^{[1]}+(n-k-1)\udim P_{m-1}$. 
 \sayT{improve this paragraph, introduce translated versions of $\mathcal Q_m^{[k]}$?} Let us assume that we already constructed the $2$-quiver of $ P_{m+1}^{[k]}$. For $m\geq 4$, the truncated preprojective $\tau\tilde P_{m+1}^{[k]}\cong\tilde P_{m-1}^{[k]}$ can uniquely be found as a quotient of $\tilde P_{m,k}$. Now the $2$-quiver $\mathcal Q_{m+1}^{[k-1]}$ of $\tilde P^{[k-1]}_{m+1}$ is given by glueing the $2$-quiver $\mathcal Q_{m+1}^{[k]}$ to $\mathcal Q_m$ by a $2$-arrow. Thereby,  $2$-arrows connects the $2$-quiver of $\tilde P_{m+1}^{[k]}$ to the one of Auslander-Reiten translate $\tau \tilde P_{m+1}^{[k]}= \tilde P_{m-1}^{[k]}$ in $\tilde P_{m,k}$. This subquiver can be found at the very right of the $2$-quiver $\mathcal Q_m$. This gives the following $2$-quiver $\mathcal Q_{m+1}^{[k-1]}$:
\\

\begin{tikzpicture}
\draw (0,4) node (A1) {\footnotesize$\mathcal Q_{m-1,1}$}+(0,-1.2) node (PM) {\footnotesize$\mathcal Q_{m,k}$} +(1.5,0) node (B1) {\footnotesize$\mathcal Q_{m-1,2}$}+(1.5,-1.1) node (P11) {\footnotesize$\mathcal Q_{m}^{[1]}$} +(3,0) node (B11) {\footnotesize$\dots$} +(4.5,0) node (D1) {\footnotesize$\mathcal Q_{m-1,n-1}$}  +(6,0) node (E1) {\footnotesize$\mathcal Q_{m-2,2}$} +(4.5,-1) node (P41) {\footnotesize$\mathcal Q_{m}^{[n-2]}$}+(6,-.9) node (P51) {\footnotesize$\mathcal Q_{m-1}^{[1]}$}+(7.5,0) node (A1b) {\footnotesize$\dots$} +(9,0) node (B1b) {\footnotesize$\mathcal Q_{m-2,k+1}$}+(9,-.8) node (P11b) {\footnotesize$\mathcal Q_{m-1}^{[k]}$} +(10.5,0) node (B11b) {\footnotesize$\dots$} +(12,0) node (D1b) {\footnotesize$\mathcal Q_{m-2,n-1}$} +(12,-.6) node (P41b) {\footnotesize$\mathcal Q_{m-1}^{[n-2]}$} +(13.5,0) node (E1b) {\footnotesize$\mathcal Q^{[n-1]}_{m-2}$} ; 
...
%In order to keep the diagrams as simple as possible we did not insert the labels of the vertices. As every vertex  $q\in(\mathcal Q_m^{[k]})_0$ corresponds a vertex $\tilde q\in \widetilde{K(n)_0}$, in any case it is straightforward to associate an arrow $\alpha_i\in K(n)_1$ with every ($2$-)arrow of $\mathcal Q_m^{[k]}$. 
 Note that two different vertices of $(\mathcal Q_m^{[k]})_0$ can correspond to the same vertex of $\widetilde{K(n)}$. Writing dimension vectors $\tbfe\in\NN^{\widetilde{K(n)_0}}$ as $\tbfe=\sum_{q\in \widetilde{K(n)_0}}\tbfe_q\cdot q$, the dimension types $\tbfe (\beta)$ and  $\bfe (\beta)$ of a subset $\beta\subset (\mathcal Q_m^{[k]})_0$ are defined by
...
We proceed by induction on $m$ and $k$. The case of the representation $\tilde P_1$ is trivial. We have $(\tilde P_2^{[k]})_q\in\{0,1\}$ for all $q\in\widetilde{K(n)}$, whence the  subrepresentations are in one-to-one correspondence with the successor closed subsets the quiver (\ref{coeff}) which is equivalent to  $\mathcal Q_2^{[k]}$ . Equivalently, we have $\Gr_{\tbfe}(\tilde P_2^{[k]})\in\{\varnothing,\{\pt\}\}$ such that $\Gr_{\tbfe}(\tilde P_2^{[k]})=\{\pt\}$ if and only if $e\subset \mathcal Q_2^{[k]}$ is successor closed.
  ...
  \]
  We have $(\mathcal Q_{m+1}^{[k-1]})_0=(\mathcal Q_{m+1}^{[k]})_0\coprod{}(\mathcal Q_m)_0$.
	Let $\beta_1\coprod\beta_2\subset (\mathcal Q_{m+1}^{[k-1]})_0\coprod{}(\mathcal Q_m)_0$ be a pair of strong successor closed subsets which gives rise to a pair of non-empty cells by induction hypothesis.
 % Denote by $d(\beta_1+\beta_2)$ the dimension vector corresponding to this subset.
  By Proposition \ref{quotient}, the fibre over the pair of cells corresponding to $\beta_1\coprod\beta_2$ is empty if and only if $\beta_2=(\mathcal Q_{m+1}^{[k-1]})_0$ and $\beta_1\cap(cap(\mathcal Q_{m-1}^{[k]})_0=\varnothing$ where $(\mathcal Q_{m-1}^{[k]}$ is the $2$-quiver of $\tilde\tau P_{m+1}^{[k]}=\tilde P_{m-1}^{[k]}$ sitting inside $\mathcal Q_m$.
