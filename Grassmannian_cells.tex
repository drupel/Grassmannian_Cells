...
\begin{example}
  ...
  \end{equation}
  Then $\tilde P_{1,i}\subset\tilde P_2$ corresponds to the one-dimensional space at vertex $(1,\alpha_i)$.
  ...
  \end{equation}
  where the top vertex of dimension $n-1$ is $(1,e)$ and the arrow from $\alpha_i^{-1}.\cA_i$ emanates from its unique source~$(2,\alpha_i^{-1})$.
  Then $\tilde P_{2,i}\subset\tilde P_3$ has one-dimensional spaces at each vertex of the quiver
  \[\xymatrix@R40pt@C10pt{&&&(1,e)&&&\\ &&&(2,\alpha_i^{-1})\ar_{\alpha_i}[u]\ar_{\alpha_1}[llld]\ar_(.65){\alpha_{i-1}}[ld]\ar^(.65){\alpha_{i+1}}[rd]\ar^{\alpha_n}[rrrd]&&&\\ (1,\alpha_i^{-1}\alpha_1)&\cdots&(1,\alpha_i^{-1}\alpha_{i-1})&&(1,\alpha_i^{-1}\alpha_{i+1})&\cdots&(1,\alpha_i^{-1}\alpha_n)}\]
  ...
  Now $\tilde P_{3,i}$ can be found as the subrepresentation corresponding to the subquiver
  \[\xymatrix@R40pt@C10pt{
    &&&\cA_i\ar^{\alpha_i}[d]&&&\\
    &&&n-1&&&\\
    \alpha_i\alpha_1^{-1}.\cA_1\ar^{\alpha_1}[rrru]&\cdots&\alpha_i\alpha_{i-1}^{-1}.\cA_{i-1}\ar^(.35){\alpha_{i-1}}[ru]&&\alpha_i\alpha_{i+1}^{-1}.\cA_{i+1}\ar_(.35){\alpha_{i+1}}[lu]&\cdots&\alpha_i\alpha_n^{-1}.\cA_n\ar_{\alpha_n}[lllu]}\]
  where the central vertex is $(1,\alpha_i)$.
  Note that taking the subquiver $\cA_i$ together with the image of the map $\alpha_i$ gives a subrepresentation of $\tilde P_{3,i}$ isomorphic to $\tilde P_2$ while taking the subquiver $\alpha_i\alpha_j^{-1}.\cA_j$ together with the image of the map $\alpha_j$ gives a subrepresentation of $\tilde P_{3,i}$ isomorphic to $\tilde P_{2,\alpha_i\alpha_j^{-1}}$.
\end{example}
...
\begin{lemma}
  ...
  For $m\ge2$, we have $\Hom(\tilde P_{m,j},\tau\tilde P_{m+1}^I)\cong\CC$.
...
\end{lemma}
\begin{proof}
  The first claim is immediate by applying the Auslander-Reiten formulas \cite[Theorem IV.2.13]{ass} to Lemma~\ref{le:properties}.(3).
  Indeed, this gives
...
\begin{remark}
  We should point out that the case $m=3$ of Lemma~\ref{le:special subrepresentations} is rather special because it is the only one for which the unique morphism $\tilde P_{m,j}\to\tau\tilde P_{m+1}^I$ is not surjective.
  Indeed, the image of the unique homomorphism $\tilde P_{2,j}\to\tau\tilde P_3^I$ is the representation with support quiver
