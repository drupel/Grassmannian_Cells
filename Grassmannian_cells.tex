...
\end{lemma}
\begin{proof}
  The first claim is immediate since $\Hom(X,P_\ell)\ne0$ (resp. $\Ext(P_\ell,X)\ne0$) for some $\ell\ge1$ implies all indecomposable summands of $X$ are preprojectives of the form $P_r$ with $1\le r\le\ell$ (resp. $1\le r\le\ell-2$), whereas $P_{m+1}^V$ is not preprojective.\sayT{The proof as written seems to be false as $\Hom(X,P_\ell)\ne0$ does not imply $\Hom(Y,P_\ell)\neq 0$ for all direct summands $Y$ of $X$. The easiest way would be to reorganize 3.8-3.10 in such way that we can assume that $X=P_m^V$ is indecomposable.} Note that $\udim P_{m+1}^V <\udim P_{m+1}$, but $\udim P_r\geq \udim P_{m+1}$ if $\Hom(P_{m+1},P_r)\neq 0.$  For the second claim we apply the functor $\Hom(P_{m+1},-)$ to the sequence \eqref{eq:truncated preprojectives} and obtain an isomorphism $\Hom(P_{m+1},P_{m+1})\cong\Hom(P_{m+1},P_{m+1}^V)$ by which the identity map on $P_{m+1}$ is taken to the projection $\pi_V:P_{m+1}\to P_{m+1}^V$.
\end{proof}
...
\begin{proof}
  ...
  But then Lemma~\ref{le:injective evaluation maps} implies $\iota_{m-1}$ is the zero map, a contradiction.\sayT{why does this imply that $\iota_{m-1}=0$? This should contradict the fact that \eqref{eq:AR sequence} is an AR-sequence.}
...
Using Lemma \ref{le:properties}.(2), we recursively obtain	$\Hom(\tilde P_m,\tilde P^{\mathbf j}_{m-r+1})\cong\Hom(\tilde P_m,\tilde P_{m-k,(j_1^c,\ldots,j_{k}^c)}^{(j_{k}^c)})=\kk$.
