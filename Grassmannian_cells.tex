...
\draw (0,4) node (A1) {\footnotesize$Q_{m-1,1}$}
  ...
  +(11.2,0) node (B11b) {\footnotesize$\dots$}
  +(12.8,0) node (D1b) {\footnotesize$Q_{m-2,n-1}$}
  +(12.8,-.6) node (P41b) {\footnotesize$\mathcal Q_{m-1}^{[n-2]}$}
...
\draw (4.45,0) node (B) {$\mathcal Q_{m,r+1}$}+(0,-.8) node (P1) {\footnotesize$\mathcal Q_{m+1}^{[r]}$} +(2.5,0) node (B1) {$\dots$} +(5,0) node (C) {$\mathcal Q_{m,n-2}$} +(5,-.7) node (P3) {\footnotesize$\mathcal Q_{m+1}^{[n-3]}$}+(7.5,0) node (D) {$\mathcal Q_{m,n-1}$}  +(10,0) node (E) {$\mathcal Q_{m+1}^{[n-1]}$} +(7.5,-.6) node (P4) {\footnotesize$\mathcal Q_{m+1}^{[n-2]}$}; 
...
\end{flushleft}
\noindent 
As already mentioned two different vertices of $\mathcal Q_m^{[r]}$ can correspond to the same vertex of $\widetilde{K(n)}$.
Writing dimension vectors $\tbfe\in\NN^{\widetilde{K(n)_0}}$ as $\tbfe=\sum_{q\in \widetilde{K(n)_0}}\tbfe_q\cdot q$, the dimension types $\tbfe(\beta)$ and  $\bfe(\beta)$ of a subset $\beta\subset(\mathcal Q_m^{[r]})_0$ are defined by
\[\tbfe (\beta)=\sum_{q\in\beta} \tilde q\in\NN^{\widetilde{K(n)_0}},\quad \bfe (\beta)=G\big(\tbfe(\beta)\big)\in\NN^{K(n)_0},\]
where $\tilde q\in\widetilde{K(n)}_0$ is the vertex which corresponds to $q\in\beta\subset(\mathcal Q_m^{[r]})_0$.
