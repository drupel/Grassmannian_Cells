...
This is obtained in Section~\ref{sec:fibrations} by combining a torus action on quiver Grassmannians which is introduced in Section~\ref{torusaction} and the so-called Caldero-Chapoton map for short exact sequences of representations.
...
\begin{thm}
  ...
  Then there exists a map $d:\supp(\hat X)\to\ZZ$ -- with a corresponding torus action on every $X_i=\bigoplus_{\chi\in A_{Q}} X_{(i,\chi)}$ defined by $t.x_{(i,\chi)}=t^{d(i,\chi)}x_{(i,\chi)}$ for $x_{(i,\chi)}\in X_{(i,\chi)}$ -- which induces a torus action on $\Gr_\bfe^Q(X)$ such that
...
\end{thm}
This torus action can be iterated in such a way that the remaining torus fixed points are precisely the subrepresentations which can be lifted to the universal covering quiver.
...
In Section \ref{sec:combinatorics}, we reveal the combinatorics behind the obtained cell decompositions introducing the notion of $2$-quivers which is a slight generalization of the notion of usual quivers.
...
The results of \cite{rupel} giving a combinatorial construction of counting polynomials for quiver Grassmannians of preprojective/preinjective representations of generalized Kronecker quivers $K(n)$ suggest the following.
\begin{conjecture}
  There exists a torus action on $\Gr_\bfe(P_{m+1}^V)$ such that the affine cell corresponding to a compatible pair $(S_H,S_V)$ has dimension...
\end{conjecture}
...
\subsection*{Acknowledgements}We would like to thank Giovanni Cerulli Irelli, Hans Franzen, Oliver Lorscheid and Markus Reineke for very fruitful discussions related to this project.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{theorem}
  \label{thm:bb}
  \cite[Section 4]{bb}
  Each attracting set $\Att(C)$ is a locally closed $\CC^*$-invariant subvariety of $X$ and the natural map $\Att(C)\to C$ is an affine vector bundle.
...
Indeed, we can trivialize each affine vector bundle $\Att(C_i)\to C_i$ over each affine piece of $C_i$ to get a decomposition of $X$ into affine spaces.
...
We consider the decomposition $X_i=\bigoplus_{\chi\in A_Q} X_{(i,\chi)}$ and define a torus action on each $X_{(i,\chi)}$ via $t.x_{(i,\chi)}=t^{d(i,\chi)}x_{(i,\chi)}$ which is then extended linearly to each $X_i$.
...
\begin{theorem}\label{thm:torusfixedpoints}
  ...
  Then the torus action on $\bigoplus_{i\in Q_0} X_i$ determined by $t.x_{(i,\chi)}=t^{d(i,\chi)}x_{(i,\chi)}$ for $x_{(i,\chi)}\in X_{(i,\chi)}$ induces a torus action on $\Gr_\bfe^Q(X)$ such that
  \[\Gr^Q_\bfe(X)^{\CC^*}\cong\bigsqcup_{\hat\bfe} \Gr^{\hat Q}_{\hat\bfe}(\hat X),\]
  where $\hat\bfe$ runs through all dimension vectors compatible with $\bfe$.
\end{theorem}
\begin{proof}
...
\end{proof}
The next step is to iterate the torus action keeping in mind the following idea: every representation, which lifts to the universal covering quiver, also lifts to the universal abelian covering quiver and also to the iterated universal abelian covering quivers, i.e.\ to each $\hat Q^{(k)}:=\widehat{\hat Q^{(k-1)}}$ with $\hat Q^{(1)}:=\hat Q$.
...
\begin{corollary}
  ...
  \begin{enumerate}
    \item If $\Gr_\bfe(X)$ is smooth and $\Gr^{\tilde Q}_\tbfe(\tilde X)$ has a cell decomposition into affine spaces, then $\Gr_\bfe(X)$ has a cell decomposition into affine spaces.
...
%  The action of $GL_n(\CC)$ on $\cH_m$ induces a transitive action of $GL_n(\CC)$ on each component $\Gr_d(\cH_m)$.
%\end{lemma} 
%\begin{proof}
%  The map $\theta\mapsto g.\theta$ for $\theta\in \cH_m$ induces a group homomorphism $GL_n(\CC)\to GL(\cH_m)$ which we denote by $g\mapsto\tilde g_m$.
%  We claim that this homomorphism is surjective which immediately implies the result.
%
%  We work by induction on $m$.
%  Suppose $GL_n(\CC)\to GL(\cH_m)$ is surjective.
%
%\end{proof}

%Similarly, for $V^g\in \Gr(\cH_m^g)$, we identify $g.P_m\otimes V^g$ with its image in $g.P_{m+1}$ under the natural evaluation map. 
%\begin{lemma}
...
\begin{proof}
 ...
 % \footnote{The proof using moduli spaces needs to be written}
  ...
  %\[X_{\bfe,m}=\{(\tilde E,V):V\in \Gr_d(\cH_m),\ P_m\otimes V\subset\tilde E\subset P_{m+1}i,\ \udim\tilde E=\bfe+d\udim P_m\}.\]
  %Under the natural projection $\pi:X_{\bfe,m}\to \Gr_d(\cH_m)$, the fiber $\pi^{-1}(V)$ identifies with $\Gr_\bfe(P_{m+1}^V)$.
  %In particular, this implies $\Gr_\bfe(P_{m+1}^V)$ is smooth for each $V\in \Gr_d(\cH_m)$.
  ...
  %Observe that the projection map $\pi$ is $GL_n(\CC)$-equivariant and thus by Lemma~\ref{le:transitivity} the quiver Grassmannians $\Gr_\bfe(P_{m+1}^V)$ and $\Gr_\bfe(P_{m+1}^W)$ are isomorphic for any $V,W\in \Gr_d(\cH_m)$.
\end{proof}
...
\begin{proof}
  ...
  \[\xymatrix{
    ...
    0\ar[r] &\Ext\big(\tilde P_{m+1}^I,\tilde P_m(I,j)\big)\ar[r]\ar[d] &  \Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\ar[r]\ar[d] & \Ext(\tilde P_{m+1}^I,\tilde K_m)\ar[r]\ar[d] & 0\\
    0\ar[r] &\Ext\big(\tilde P_{m+1}^I,\tilde P_m(I,j)/V\big)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/U)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde K_m/W)\ar[r]\ar[d] & 0 \\
    &0&0&0&}\]
...
\end{itemize}
In the first case, we have $\Gr_\tbfg(\tilde P_{m}^{\uj})=\{\pt\}$ and thus there exists a single cell $C_0=\mathbb{A}^0$. Fix a cell $B_i\subset\Gr^{\tilde Q}_\tbff(P_{m-1,j_l})$. As the cell decomposition of $\Gr^{\tilde Q}_\tbff(P_{m-1,j_l})$ is compatible with $\pi_{m}^{\bf i}$, we either have that the fiber over $B_i\times C_0$ is empty or that the fiber is of constant dimension, thus building a new cell $CB_i$. For all representations $U\in CB_i$ we have $\pi_{m,\uj'}^{\bf j'}(U)\neq 0$ because $\pi_{m,\uj}^{\bf j}$ is surjective and $\pi_{m,\uj'}^{j_l}(U)=\tilde P_{m}^{\uj}$.

In the second case, the fiber over $B_i\times C_k$ is non empty and of constant dimension, thus building a new cell $CB_{i,k}$. As we have $\pi^{\bf j}_{m,\uj}(V)=0$ for all $V\in C_k$, it follows that $\pi^{\bf j'}_{m,\uj'}(U)=0$ for all representations $U\in (CB)_{i,k}$.
...
\end{theorem}
\begin{proof}
  The first part follows by combining the results of Section~\ref{torusaction} and Theorems~\ref{thm:bb} and~\ref{cellscover}, taking into account that every preprojective representation of $\widetilde{ K(n)}$ is a lift of a preprojective representation (c.f.\ Lemma~\ref{le:lifts of transjectives}).
...
\begin{remark}
  ...
  That is, given a 2-arrow $V:\Gamma(1)\implies\Gamma(2)$ for which $\Gamma(1)$ has a unique source $p$ and $\Gamma(2)$ has a unique sink $q$, we get an equivalent 2-quiver by replacing this 2-arrow with a usual arrow $a_V:p\to q$.
\end{remark}
...
\begin{corollary}
  ...
  Inside $D_m$, $m\ge2$, there are precisely $u_{m-2}$ vertical edges which are immediately preceded by exactly $n-1$ horizontal edges, all other vertical egdes are immediately preceded by exactly $n$ horizontal edges.
\end{corollary}
\begin{proof}
...
Similarly, for a vertical edge $v\in V_{m+1}$ and a subset $S_V\subset V_{m+1}$, the \emph{local shadow path} of $v$ with repect to $S_V$ is $D(v;S_V)=ev$ for the shortest subpath of $D_{m+1}$ for which $|ev\cap H_{m+1}|=n|ev\cap S_V|$ and we take $D(v;S_V)=h_1v$ if there does not exist such an edge $e$.
\begin{definition}
...
\begin{proof}
  ...
  It follows that $H_{m,r+1,n}^{[r+1]}\sqcup H_{m+1}^{[r+1]}$ contains $n(n-r)u_{m-1}-nu_{m-2}$ horizontal edges and $V_{m-1}^{[r+1]}\sqcup V_{m+1}^{[r+1]}$ contains $n(n-r)u_{m-2}-nu_{m-3}$ vertical edges (note that $H_{m,r+1,n}^{[r+1]}$ naturally identifies with the Dyck path $H_{m-1}^{[r+1]}$).
  ...
  Thus by induction, we must have $H_{m-2}^{[r+1]}\subset(\varphi^*)^{-1}S_V$ and $V_m^{[r+1]}\subset\Omega^{-1}S_H$.
  It follows from piecewise compatibility that $H_m^{[r+1]}\cap(\varphi^*)^{-1}S_V=\varnothing$.
  But then by the definition of $\varphi^*$ we have $V_{m-1}^{[r+1]}\cap S_V=\varnothing$ and $S_V^{[r+1]}=V_{m+1}^{[r+1]}$ so that $D(v_{u_m};S_V)=hv_{u_m}$ with $h$ as in the first case above.
...
We conjecture that the torus action on $\Gr_\bfe(P_{m+1})$ can be chosen to provide a geometric explaination of this result.
\begin{conjecture}
...
\bibitem{brenner-butler} 
  S.~Benner, M.~C.~R.~Butler: The equivalence of certain functors occuring in the representation theory of artin algebras and species. Journal of the London Mathematical Society \textbf{2}(1), 183-187 (1976).
\bibitem{cc} 
...
%\bibitem{hr} 
 % D.~Happel, C.~M.~Ringel: Tilted Algebras. Transactions of the American Mathematical Society {\bf 274}(2), 399-443 (1982).
\bibitem{llz} 
...
%\bibitem{rin} 
 % C.~M.~Ringel: Reflection functors for hereditary algebras. Journal of the London Mathematical Society (2) {\bf 21}(3), 465-479 (1980).
