...
Depending on context (more specifically the parity of $m+1$ below), we will sometimes identify the word $(i_1,\ldots,i_l)$ with the element $\alpha_{i_1}\alpha_{i_2}^{-1}\alpha_{i_3}\cdots\alpha_{i_l}^{(-1)^{l+1}}\in W_n$ and sometimes with the element $\alpha_{i_1}^{-1}\alpha_{i_2}\alpha_{i_3}^{-1}\cdots\alpha_{i_l}^{(-1)^l}\in W_n$.
In this way, we associate to each $\ui=(i_1,\ldots,i_k)\in A_1^{(k)}$, $1\leq k\leq m$, a preprojective subrepresentation $\tilde P_{m+1-k,\ui}\subset\tilde P_{m+1}$ which lifts $P_{m+1-k}$.
More precisely, we obtain a sequence of preprojective representations 
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item We have $\Hom_{K(n)}(P_m,P_{m+1})\cong\bigoplus_{i=1}^n \Hom_{\widetilde{K(n)}}(\tilde P_{m,i},\tilde P_{m+1})$, where each $\Hom_{\widetilde{K(n)}}(\tilde P_{m,i},\tilde P_{m+1})$ is one-dimensional.
    ...
    \item For every $\ui\in A_1^{(l)}$, there exists a truncated preprojective representation $\tilde P_{m+1}^\ui$ which is the cokernel of
      \begin{align}
        \label{ses1}
        \ses{\bigoplus_{j=1}^l \tilde P_{m,i_j}}{\tilde P_{m+1}}{\tilde P_{m+1}^{\ui}}.
      \end{align}
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  $$\CC=\Hom_{K(n)}(P_m,P_m)\cong\bigoplus_{i=1}^n \Hom_{\widetilde{K(n)}}(\tilde P_{m,i},\tilde P_{m,j}).$$
  But $\Hom_{\widetilde{K(n)}}(\tilde P_{m,j},\tilde P_{m,j})\cong\CC$ and so we must have $\Hom_{\widetilde{K(n)}}(\tilde P_{m,i},\tilde P_{m,j})=0$ for $i\neq j$.
  The vanishing of $\Ext(\tilde P_{m,i},\tilde P_{m,j})$ follows in the same manner using that $P_m$ is exceptional.

  For the final claim, observe that under the isomorphism from part (1) the sequence $\ui\in A_1^{(l)}$ corresponds to an $l$-dimensional subspace $V\in H_m$.
  The map $\bigoplus_{j=1}^l \tilde P_{m,i_j}\to \tilde P_{m+1}$ is then a lift of the evaluation morphism $ev_V:P_m\otimes V\to P_{m+1}$ and hence is injective by Lemma~\ref{le:injective evaluation maps}.
  Taking the cokernel defines the truncated preprojective $\tilde P_{m+1}^\ui$.
\end{proof}
