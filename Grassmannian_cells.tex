...
Although it has been shown by various authors that every projective variety can be realized as a quiver Grassmannian -- which is maybe not surprising as the classification problem of indecomposable quiver representations is wild -- it turns out that very interesting phenomena arise when restricting to certain quivers or representations with certain properties.
...
We write $\rep Q$ for the hereditary abelian category of finite-dimensional $\C$-representations of $Q$ and we assume in the following that all representations are finite-dimensional.\sayT{Inserted finite-dimensional as we do not need infite-dimensional one (I am also not sure if we need to be careful at some points otherwise)}
...
By Lemma \ref{le:unique morphisms} the roots $d(m,r)$ are Schur roots as there exist representations with trivial endomorphism ring.
...
\begin{definition}
  ...
  Given a 2-quiver $\tilde Q$, we call a subset $\beta\subset Q_0$ \emph{strong successor closed in $\tilde Q$} if it is successor closed in $Q$ and for each 2-arrow $V:\Gamma(1)\to\Gamma(2)$ in $Q_2$ with $\Gamma(1)_0\subset\beta$ we have $\Gamma(2)_0\cap\beta\ne\varnothing$.  
\end{definition}

We introduce a notion of equivalence of $2$-quivers.
\begin{definition}
  \label{def:2equivalence}
  Let $\tilde Q=(Q,Q_2)$ be a $2$-quiver with a $2$-arrow $V:\Gamma(1)\to\Gamma(2)$ in $Q_2$ such that one of the following conditions is satisfied:
  \begin{enumerate}
  ...
  \begin{enumerate}
    \item $\tilde Q_p$ as the $2$-quiver obtained from $\tilde Q$ when replacing the $2$-arrow $V$ by a $2$-arrow $V_p:\{p\}\to\Gamma(2)$;
    \item $\tilde Q_q$ as the $2$-quiver obtained from $\tilde Q$ when replacing the $2$-arrow $V$ by a $2$-arrow $V_q:\Gamma(1)\to \{q\}$;
    \item $\tilde Q_V$ as the $2$-quiver obtained from $\tilde Q$ when replacing the $2$-arrow $V$ by a usual arrow $\alpha_V:p\to q$. 
  \end{enumerate}
  This defines a relation on the set of $2$-quivers denoted by $\tilde Q\to \tilde Q_p$, $\tilde Q\to \tilde Q_q$ and $\tilde Q\to \tilde Q_V$ respectively.
...
\begin{lemma}
  ...
  Let $\tilde Q=(Q,Q_2)$ and $\tilde Q'=(Q',Q_2')$ be equivalent $2$-quivers.
  If $\beta\subset Q_0$ is strong successor closed in $\tilde Q$, it also defines a strong successor closed set of vertices in $\tilde Q$.
  In particular, the number of subsets of $Q_0$ which are strong successor closed in $\tilde Q$ coincides with the number of subsets of $Q_0'$ which are strong successor closed in $\tilde Q'$.
\end{lemma}
...
\begin{proof}
  By induction, we only need to consider the cases $\tilde Q'\in\{\tilde Q_p,\tilde Q_q,\tilde Q_V\}$ where one of the conditions of Definition~\ref{def:2equivalence} is satisfied.

  Assume first that $\tilde Q'\in\{\tilde Q_p,\tilde Q_q\}$.
  ...
  We only consider the case $\tilde Q'=\tilde Q_p$ below, the argument for $\tilde Q'=\tilde Q_q$ is dual.

  Let $\beta\subset Q_0$ be strong successor closed in $\tilde Q$.
  To see that $\beta$ is strong successor closed in $\tilde Q_p$ it suffices to consider the 2-arrow $V_p:\{p\}\to\Gamma(2)$.
  Suppose $\{p\}\subset\beta$.
  As $\beta$ is successor closed in $Q$ and $p$ is a source in the connected quiver $\Gamma(1)$, we have $\Gamma(1)_0\subset\beta$ and thus $\Gamma(2)_0\cap\beta\ne\varnothing$, i.e.\ $\beta$ is strong successor closed in $\tilde Q_p$.
  The reverse implication is immediate since $\{p\}\subset\Gamma(1)_0$.

  Now assume $\tilde Q'=\tilde Q_V=\big(Q_V,(Q_V)_2\big)$. 
  Let $\beta\subset Q_0$ be strong successor closed in $\tilde Q$.
  Since $(Q_V)_2\subset Q_2$, to see that $\beta$ is strong successor closed in $\tilde Q_V$ we only need to show that $\beta$ is successor closed in $Q_V$.  
  ...
  Finally, let $\beta\subset(Q_V)_0$ be strong sucessor closed in $\tilde Q_V$.
  Since $Q_1\subset(Q_V)_1$, we immediately see that $\beta$ is successor closed in $Q$.
  To see that $\beta$ is strong successor closed in $\tilde Q$, it suffices to consider the 2-arrow $V:\{p\}\to\{q\}$ for which the claim is obvious as above.
\end{proof}
...
The next step is to associate a $2$-quiver $\mathcal Q_{m+1}^{[k]}$ with the lifted truncated preprojective representations 
\[\tilde P_{m+1}^{[k]}:=
  \begin{cases}
