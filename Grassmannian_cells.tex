...
\begin{theorem}
  ...
  These cell decompositions are compatible with the projection maps $\tilde\pi_{m+1}^{\ui,I,J}:\tilde P_{m+1}^J\to\tilde P_{m+1-k,\ui}^I$ from Lemma~\ref{le:truncated quotients} in the following sense:
  \begin{itemize}
    \item[($\dagger$)] for any $\tilde E\in\Gr_\tbfe(\tilde P_{m+1}^J)$ such that $\tilde\pi_{m+1}^{\ui,I,J}(\tilde E)\neq 0$ for some admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, with $i_1\notin J$ and subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$, we have $\tilde\pi_{m+1}^{\ui,I,J}(\tilde E')\neq 0$ for all $\tilde E'\in C_{\tilde E}$, where $C_{\tilde E}$ is the affine cell which contains $\tilde E$.
  \end{itemize}
% Moreover, the cell decompositions are independent of the choices made during their inductive construction.
\end{theorem}
...
\end{corollary}
\begin{proof}
  For the standard lifts $\tilde P_m$, this is just the special case $J=\varnothing$ in Theorem~\ref{cellscover}.
  The claim for any translate $\tilde P_{m,w}$ for $w\in W_n$ is an immediate consequence of this.
\qed\end{proof}
\subsubsection*{Proof of Theorem~\ref{cellscover}}
  ...
  By the induction on $m$, the quiver Grassmannian $\Gr_\tbfe(\tilde P_m^{\{j\}})$ admits a cell decomposition so that the compatibility condition~($\dagger$) holds for each of the maps $\tilde\pi_m^{\ui'_1,I,\{j\}}:\tilde P_m^{\{j\}}\to\tilde P_{m+1-k,\ui'_1}^I$.
  But~$\Gr_\tbfe(\tilde P_{m,j}^{\{j\}})$ is naturally isomorphic to $\Gr_\tbfe(\tilde P_m^{\{j\}})$ using the action of $W_n$, under this correspondence the cell decomposition becomes compatible with the projection maps $\tilde\pi_{m,j}^{\ui'_1,I,\{j\}}:\tilde P_{m,j}^{\{j\}}\to\tilde P_{m+1-k,\ui}^I$.
  Then using the equivalence $\Gr_\tbfe(\tilde P_{m+1}^J)\cong\Gr_\tbfe(\tilde P_{m,j}^{\{j\}})$ coming from Lemma~\ref{le:truncated quotients}.1, we obtain the desired cell decomposition of $\Gr_\tbfe(\tilde P_{m+1}^J)$ which is compatible with the projection maps $\tilde\pi_{m+1}^{\ui,I,J}$.

  Now consider $J\subsetneq I\subsetneq \{1,\ldots,n\}$ with $I\setminus J=\{j\}$.
  ...
  By way of induction, we assume all quiver Grassmannians $\Gr_\tbff(\tilde P_{m,j})$ and $\Gr_\tbfg(\tilde P_{m+1}^I)$ admit cell decompositions satisfying ($\dagger$), say
  \begin{equation}
    \label{eq:cellB}
  ...
  \begin{itemize}
    ...
    \item If $i_1=j$, the restricted map $\tilde\pi_{m+1}^{\ui,I',J}\Big|_{\tilde P_{m,j}}$ coincides with $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}$ by Lemma~\ref{le:truncated quotients}.4.
      By construction, we have $\tilde E'\cap \tilde P_{m,j}\in B_{\tbff,k}$ for all ~$\tilde E'\in \AA^d\times B_{\tbff,k} \times C_{\tbfg,\ell}$.
      But $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(\tilde E\cap\tilde P_{m,j})=\tilde\pi_{m+1}^{\ui,I',J}(\tilde E)\ne0$ implies $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(\tilde F)\ne0$ for all~$\tilde F\in B_{\tbff,k}$, in particular $\tilde\pi_{m+1}^{\ui,I',J}(\tilde E')=\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(\tilde E'\cap \tilde P_{m,j})\neq 0$ for all $\tilde E'\in \AA^d \times B_{\tbff,k} \times C_{\tbfg,\ell}$.
  \end{itemize}
  Thus the compatibility condition ($\dagger$) holds for the cell decomposition of $\Gr_\tbfe(\tilde P_{m+1}^J)$.

% To see the final claim, there are three cases to consider.
% The case $|J|=n-1$ is immediate from the construction above.
% When $|J|<n-2$, we may find subsets $I,I'\subsetneq\{1,\ldots,n\}$ containing $J$ so that $I\setminus J=\{j\}$, $I'\setminus J=\{j'\}$, and $j\ne j'$.
% This gives rise to the following commutative diagram:
% \[\xymatrix{& & 0\ar[d]& 0\ar[d]&\\
%   & & \tilde P_{m,j'}\ar[d]\ar@{=}[r]& \tilde P_{m,j'}\ar[d]&\\
%   0\ar[r]&\tilde P_{m,j} \ar[r]\ar@{=}[d] & \tilde P_{m+1}^J\ar[d]\ar[r]\ar@{}[dr]|(.7){\lrcorner} & \tilde P_{m+1}^I\ar[r]\ar[d]& 0\\
%   0\ar[r] &\tilde P_{m,j}\ar[r] &  \tilde P_{m+1}^{I'}\ar[r]\ar[d] & \tilde P_{m+1}^{I\cup I'}\ar[r]\ar[d] & 0\\
% & & 0& 0&}\]
% Let $A:=\AA^d\times B\times C$ be an affine cell for $\Gr_\tbfe(\tilde P_{m+1}^J)$ constructed as above using this middle horizontal sequence, say with $B$ an affine cell of $\Gr_\tbff(\tilde P_{m,j})$ and $C$ an affine cell of $\Gr_\tbfg(\tilde P_{m+1}^I)$.
% Thus, for $\tilde E\in A$, we have $\tilde E\cap P_{m,j}\in B$ and $\tilde\pi_{m+1}^{I,J}(\tilde E)\in C$.
% Similarly, the affine cell $C$ can be written as $\AA^{d'}\times B'\times D$ with $B'$ an affine cell of $\Gr_{\tbff'}(\tilde P_{m,j'})$ and $D$ an affine cell of $\Gr_\tbfh(\tilde P_{m+1}^{I\cup I'})$ so that elements $\tilde G\in C$ satisfy $\tilde G\cap \tilde P_{m,j'}\in B'$ and $\tilde\pi_{m+1}^{I\cup I',I}(\tilde G)\in D$.
% In particular, this implies $\tilde E\cap \tilde P_{m,j'}\in B'$ and $\tilde\pi_{m+1}^{I\cup I',J}(\tilde E)\in D$ for any $\tilde E\in A$.
% Then for $\tilde E\in A$, writing $\tilde G'=\tilde\pi_{m+1}^{I',J}(\tilde E)$, we see $\tilde G'\cap \tilde P_{m,j}\in B$ and $\tilde\pi_{m+1}^{I\cup I',I'}(\tilde G')\in D$.
% Thus $\tilde\pi_{m+1}^{I',J}(\tilde E)$ lies in an affine cell $C':=\AA^{d''}\times B\times D$ for $\Gr_{\tbfg'}(\tilde P_{m+1}^{I'})$.
% The induced map $A\to B'\times C'$, $\tilde E\mapsto \big(\tilde E\cap \tilde P_{m,j'},\tilde\pi_{m+1}^{I',J}(\tilde E)\big)$, is surjective by a routine chase through the diagram above.
% For any $\tilde E\in Gr_\tbfe(\tilde P_{m+1}^J)$ with $\tilde E\cap \tilde P_{m,j'}\in B'$ and $\tilde\pi_{m+1}^{I',J}(\tilde E)\in C'$, the following hold:
% \begin{itemize}
%   \item $\tilde E\cap \tilde P_{m,j}=\tilde\pi_{m+1}^{I',J}(\tilde E)\cap \tilde P_{m,j}\in B$;
%   \item writing $\tilde G=\tilde\pi_{m+1}^{I,J}(\tilde E)$, we have $\tilde G\cap \tilde P_{m,j'}=\tilde E\cap \tilde P_{m,j'}\in B'$ and $\tilde\pi_{m+1}^{I\cup I',I}(\tilde G)\in D$ so that $\tilde\pi_{m+1}^{I,J}(\tilde E)\in C$.
% \end{itemize}
% It follows that $\tilde E\in A$ and thus $A$ identifies with the affine cell for $\Gr_\tbfe(\tilde P_{m+1}^J)$ constructed from the affine cells $B'$ and $C'$ using the middle vertical sequence in the diagram above.

% Now suppose $|J|=n-2$, say $\{1,\ldots,n\}\setminus J=\{j,j'\}$.
% This gives rise to the following commutative diagram:
% \[\xymatrix{& 0 \ar[d] & 0 \ar[d] & & \\
%     0 \ar[r] & \tilde P_{m-1} \ar[r] \ar[d] \ar@{}[dr]|(.3){\ulcorner} & \tilde P_{m,j'} \ar[d]\ar[r] & \tilde P_{m,j'}^{\{j'\}} \ar[d]^{\sim}\ar[r] & 0 \\
%   0 \ar[r] & \tilde P_{m,j} \ar[r]\ar[d] & \tilde P_{m+1}^J\ar[d]\ar[r] & \tilde P_{m+1}^{\{j'\}^c} \ar[r]& 0\\
%   &\tilde P_{m,j}^{\{j\}}\ar[r]^{\sim}\ar[d] & \tilde P_{m+1}^{\{j\}^c}\ar[d] & & \\
% & 0 & 0& &}\]
% where $\tilde P_{m-1}$ is labeled as $\tilde P_{m-1,(j,j)}$ in the left vertical sequence and as $\tilde P_{m-1,(j',j')}$ in the upper horizontal sequence.
% By a chase through this diagram dual to the previous case, we see that the cell decomposition for $\Gr_\tbfe(\tilde P_{m+1}^J)$ is the same regardless of whether we use the middle horizontal or the middle vertical sequence for its construction.
\qed
...
\begin{proof}
  ...
  The results of \cite[Section 3]{cc} show that $\Psi_{\bfe}^{-1}(\tilde F,\tilde G)\cong\AA^{\dim\Hom(\tilde G,P_{j_{r'+1}}/\tilde F)}$ for all $(\tilde F,\tilde G)\in \Gr_\bff(P_{j_{r'+1}})\times \Gr_\bfg\big(P(r')\big)$, in particular the fiber is never empty.
  ...
  Together these yield $\Ext(\tilde G,P_{j_{r'+1}}/\tilde F)=0$ and thus
  \[\dim\Hom(\tilde G,P_{j_{r'+1}}/\tilde F)=\langle \tilde G, P_{j_{r'+1}}/\tilde F\rangle\]
  for all $(\tilde F,\tilde G)\in \Gr_\bff(P_{j_{r'+1}})\times \Gr_\bfg(P(r'))$.
...
While the cell decompositions above appear to depend on the data built into their recursive construction, the compatibility condition ($\dagger$) implies these cell decompositions exhibit a remarkable amount of recursive symmetry.
\begin{corollary}
  Fix an admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, and subsets $I,J\subsetneq\{1,\ldots,n\}$ with $i_1\notin J$ and $i_k\in I$.
  Let $\tilde K_m=\tilde K_m(\ui,I,J)$ denote the kernel of the projection $\tilde\pi_{m+1}^{\ui,I,J}:\tilde P_{m+1}^J\to \tilde P_{m+1-k,\ui}^I$.
  For any dimension vectors $\tbfe,\tbff,\tbfg$ with $\tbfe=\tbff+\tbfg$, and any affine cell $A_\tbfe$ of $Gr_\tbfe(\tilde P_{m+1}^J)$ which intersects the preimage under the Caldero-Chapoton map of a product of affine cells $B_\tbff\times C_\tbfg$ from $Gr_\tbff(\tilde K_m)\times Gr_\tbfg(\tilde P_{m+1-k,\ui}^I)$, we have $\Psi(\tilde E)\in B_\tbff\times C_\tbfg$ for all $\tilde E\in A_\tbfe$.
  Moreover, the induced map $A_\tbfe\to B_\tbff\times C_\tbfg$ is surjective with affine fibers.
\end{corollary}
\begin{proof}
  \begin{itemize}
    \item Implicit in the construction of the cell decomposition for $\Gr_\tbfe(\tilde P_{m+1}^J)$ is a choice of complete filtration for $\tilde P_{m+1}^J$ by subrepresentations of the form $\tilde K_m(\ui',I',J)$.
      In Section~\ref{sec:2quivers}, we fix a standard convention.

    \item By the compatibility of the cell $A_\tbfe$ with the map $\tilde\pi_{m+1}^{\ui,I,J}$, we have $\tilde\pi_{m+1}^{\ui,I,J}(\tilde E)\ne 0$ for all $\tilde E\in A_\tbfe$.
  \end{itemize}

  Suppose the cell $A_\tbfe$ was constructed inductively using $J\subsetneq I'\subsetneq \{1,\ldots,n\}$ with $I'\setminus J=\{j\}$.
\end{proof}

As the $F$-polynomials of truncated preprojective representations only depend on the dimension vector, we may denote them by $F_{d(m,r)}$.
