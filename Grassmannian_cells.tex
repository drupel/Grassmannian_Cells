...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For any $I'\subsetneq\{1,\ldots,n\}$, we have $\Hom(\tilde P_{m+1}^{I'},\tilde P_{m+1}^I)\ne0$ if and only if $I'\subset I$, in which case
      \[\Hom(\tilde P_{m+1}^{I'},\tilde P_{m+1}^I)\cong\CC\quad\text{and}\quad\Ext(\tilde P_{m+1}^{I'},\tilde P_{m+1}^I)=0.\]
...
\begin{proof}
  ...
  The first claim of part (2) is an immediate consequence of Lemma~\ref{le:unique morphisms}.(1) together with Theorem~\ref{covering}.
  For the second claim of part (2), we apply $\Hom(-,\tilde P_{m+1}^I)$ to the sequence \eqref{ses1} for $I'$.



  Lemma~\ref{le:unique preprojective morphism} together with Theorem~\ref{covering}
  

  The first part of (4) follows from Theorem~\ref{covering} together with Lemma~\ref{le:basic homological properties}.
  For the second part of (4), we apply $\Hom(-,\tilde P_{m,j})$ to the sequence \eqref{ses1}.
  Then taking into account Lemma~\ref{le:homdecomposition} part (4), we get the isomorphism
  \[\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\cong\Hom(\textstyle{\bigoplus_{i\in I}}\tilde P_{m,i},\tilde P_{m,j}).\] 
  Then Lemma~\ref{le:homdecomposition}.(2) gives the final claim of part (4).

  Part (5) follows by applying $\Hom(-,\tilde P^I_{m+1})$ to the sequence \eqref{ses1} and then using parts (1) and (2).
\end{proof}
...
\begin{definition}
  Let ${\bf j}:=(\underline{j_1},\underline{j_2},\ldots,\underline{j_r})$ be a sequence with $\underline{j_1}\in I_{n-1}$, $\underline{j_2},\ldots,\underline{j_{r-1}}\in I_{n-2}$ and $\underline{j_r}\in I_s$ with $s\leq n-2$. Define $j_1^c$ as before.
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item the sets $I'_l$ defined recursively by $I'_0=I_0$ and $I'_l:=(I'_{l-1})^c\cup I_l$ for $1\le l\le k$ satisfy $(I'_{l-1})^c\cap I_l=\varnothing$ for $1\le l\le k$.
      In particular, we have $|I'_l|=n-1$ for $0\le l\le k-1$.
  \end{enumerate}
  Any sequence consisting of a single nonempty subset $\bfI=(I_1)$ will also be considered admissible.
  Given an admissible sequence $\bfI$, define a new admissible sequence $\delta\bfI=(I'_1,I_2,\ldots,I_k)$, where we set $\delta(I_1)=(I_1^c)$.
\end{definition}
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item Consider an admissible sequence $\bfI=(I_0,\ldots,I_k)$ with $k<m$ and write $(I'_l)^c=\{i_l\}$ for $0\le l\le k-1$.
      For $\ui=(i_0,\ldots,i_{k-1})$, there exists a sequence of surjections
      \[\tilde P_{m+1}\onto\tilde P_{m+1}^{I'_0}\onto\tilde P_{m,i_0}^{I'_1}\onto\tilde P_{m-1,(i_0,i_1)}^{I'_2}\onto\cdots\onto P_{m+1-k,\ui}^{I'_k}.\]
      the truncated preprojective representation
      \[\tilde P_{m+1}^\bfI:=\tilde P_{m+1-k,\ui}^{I'_{k+1}}\]
      is a quotient of $\tilde P_{m+1}$ with $\Hom(\tilde P_{m+1},\tilde P_{m+1-k}^\bfI)\cong\CC$.
      These quotients  
  \item Let $m\geq 3$ and $1\leq r\leq m-1$ and let $\mathbf j:=(\underline{j_1},\ldots,\underline{j_r})$ be an admissible sequence. The truncated preprojective representation
    \[\tilde P^{\mathbf j}_{m-r+1}:=\tilde P_{m-r+1,(j_1^c,\ldots,j_{r-1}^c)}^{(j_{r-1}^c,j_{r,1},\ldots,j_{r,s})}\]
    is a quotient of $\tilde P_m$ such that $\Hom(\tilde P_m,\tilde P^{\mathbf j}_{m-r+1})=\CC$.
    Defining ${\bf j}(k):=(\underline{j_1},\ldots,\underline{j_k})$ for $k=2,\ldots,r-1$, these quotients satisfy the relation
    \[\tilde P_{m-k,(j_1^c,\ldots,j_k^c)}^{\delta^k{\bf j(k)}}=\tilde P_{m-k+1,(j_1^c,\ldots,j_{k-1}^c)}^{\delta^{k-1}\bf j(k)}\]
    for $k=1,\ldots,r$.
  \end{enumerate}
\end{lemma}
...
\[\xymatrix{0\ar[r] &\bigoplus_{i\in I}\tilde P_{m,i}\ar[r] &  \tilde P_{m+1}\ar[r] & \tilde P_{m+1}^I\ar[r] & 0 \\
  0\ar[r]&\tau\tilde P_{m+1}=\tilde P_{m-1} \ar[r]\ar[u] & \tilde P_{m,j}\ar[u]\ar[r]& \tilde P_{m}^{\uj}\ar[r]\ar@{=}[u]& 0}\]
