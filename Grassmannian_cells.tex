...
\begin{align}\label{eq:homext}d_{X,Y}:\bigoplus_{i\in Q_0}\Hom_\CC(X_i,Y_i)\to\bigoplus_{\alpha\in Q_1}\Hom_\CC(X_{s(\alpha)},Y_{t(\alpha)}),\qquad (f_i)_{i\in Q_0}\mapsto(f_{t(\alpha)}\circ X_\alpha-Y_\alpha\circ f_{s(\alpha)})_{\alpha\in Q_1},\end{align}
...
\begin{lemma}
  ...
  \sayD{This should be upgraded to ``Any lift of a preprojective (or preinjective) for $Q$ is preprojective (or preinjective).''} 
\end{lemma}
\begin{proof}
...
\sayD{The next result needs some serious clean-up, can they be proven for $K(n)$ without passing to the universal cover?}
\begin{lemma}
...
It will be rather important that our chosen lifts of $\tilde P_{2l}$ and $\tilde P_{2l-1}$, $l\ge1$, live on two different components of the universal covering quiver.
...
The next result establishes some basic homological properties of these translated preprojective representations.
\begin{lemma}
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For $j\in\{1,\ldots,n\}$, we have $\Hom(\tilde P_{m+1}^I,\tilde P_{m,j})=0$.
      ...
      \[\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\cong\Hom(\tilde P_{m,j},\tilde P_{m,j})=\CC.\] 
    \item We have $\Hom(P_{m+1}^I,P_{m+1}^I)=\CC$ and $\Ext(\tilde P_{m+1}^I,\tilde P_{m+1}^I)=0$.
      In particular, $P_{m+1}^I$ is an exceptional representation of $\widetilde{K(n)}$.
    %\item We have $\Hom(\tilde P_{m+1}^I,\tau\tilde P_{m+1}^I)=0$ and $\Ext(\tilde P_{m+1}^I,\tau\tilde P_{m+1}^I)=\CC$.
  \end{enumerate}
\end{lemma} 
\begin{proof}
  ...
  %The Auslender-Reiten formulas \cite[Theorem IV.2.13]{ass} then give part (4).

  Part (4) follows by applying $\Hom(-,\tilde P^I_{m+1})$ to the sequence \eqref{ses1} and then using parts (1) and (2).
  %Then the Auslander-Reiten formulas \cite[Theorem IV.2.13]{ass} imply 
...
For $l\ge1$, set
\[A_1^{(l)}:=\{(i_1,\ldots,i_l)\mid i_j\in\{1,\ldots,n\}, i_j\ne i_{j+1}\text{ for $1\le j<l$}\}.\]
...
For $m\ge3$, define the representation 
\begin{lemma}
  Consider a nonempty subset $I\subsetneq\{1,\ldots,n\}$ and fix an element $j\in I$. 
  For $m\ge2$, we have $\Hom(\tilde P_{m,j},\tau\tilde P_{m+1}^I)=\CC$.
  Moreover, the kernel of the unique morphism $\tilde P_{m,j}\to\tau\tilde P_{m+1}^I$ is the following representation 
  \[\tilde P_m(I,j):=
    \begin{cases}
      \bigoplus_{\substack{1\leq i\leq n\\i\neq j}}\tilde P_{m-1,(j,i)}\oplus \bigoplus_{i\in I, i\ne j}\tilde P_{m-2,i} & \text{ if $m\geq 3$;}\\
      \bigoplus_{\substack{1\leq i\leq n\\i\neq j}}\tilde P_{1,(j,i)} & \text{ if $m=2$.}
    \end{cases}\]
\end{lemma}
\begin{proof}
  The first claim follows from Lemma~\ref{le:properties}.(3) by applying the Auslander-Reiten formulas \cite[Theorem IV.2.13]{ass}.
  Indeed, this gives the isomorphism
  \[\dim\Hom(\tilde P_{m,j},\tau\tilde P_{m+1}^I)=\dim\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})=\CC.\]

  We establish the final claim directly for $m=2,3$ and then deduce the general case by applying the reflection recursions \eqref{eq:recursive covers}.
\end{proof}
...
%That is, taking $\varepsilon=(-1)^{m+1}$ we quotient out the translations $\tilde P_{m,w\alpha_i^\varepsilon}$ for $i\in I$.
...
\begin{lemma}
  ...
  For $m\ge1$ and $I\subsetneq\{1,\ldots,n\}$, all non-trivial proper subrepresentations $U$ of $\tilde P_{m+1}^I$ are preprojective.	
\end{lemma}
\begin{proof}
...
The following indicates how the preprojective lifts $\tilde P_m$ are built for $m=2,3,4$.
\begin{example}\label{smallm}[The cases $m=2,3,4$]
  By construction, the lifts of all (truncated) preprojectives are exceptional as representations of $\widetilde{K(n)}$ and thus they are uniquely determined by their dimension vectors.
  ...
  We call it the \emph{support quiver} of the representations.
  ...
  For $i\in\{1,\ldots,n\}$, the support of $\tilde P_2^{\{i\}}$ can be obtained from the quiver of $\tilde P_m$ by erasing the arrow $\alpha_i$ and the corresponding sink. 
  We denote this support quiver of $\tilde P_2^{\{i\}}$ by $\mathcal A_i$.
  In turn, for $I\subset\{1,\ldots,n\}$, the quiver of $\tilde P_2^I$ is obtained by erasing the arrows $\alpha_i$ and the corresponding sinks $(1,\alpha_i)$ for $i\in I$.
  ...
  where the vertex of dimension $n-1$ is the middle vertex $(1,1)$.
  Moreover, for $I\subset\{1,\ldots,n\}$, the representation $\tilde P_3^I$ is obtained by erasing the subquivers $\mathcal A_i$ for $i\in I$ and decreasing the dimension of the space at vertex $(1,1)$ by $|I|$.

  To continue, we assume for convenience that $I=\{1,\ldots,l\}$.
  This is not actually a restriction of generality as the general case can be obtained by relabelling the arrows. 
  The Auslander-Reiten translate $\tau\tilde P_3^I$ is given by
  \[\xymatrix@R10pt@C20pt{&&(2,\alpha_1^{-1})\ar_{\alpha_1}[lld]\\(1,1) &&\vdots\\&&(2,\alpha_l^{-1})\ar_{\alpha_l}[llu]}\]
  In particular, for $i\in I$ the unique homomorphism $\tilde P_{2,i}=\alpha^{-1}_i.\tilde P_2\to\tau\tilde P_3^I$ has image $(2,\alpha_i^{-1})\xrightarrow{\alpha_i}(1,1)$.
 

The representation $\tilde P_4$ is defined by
  \[\xymatrix@R10pt@C20pt{&&\mathcal B_1\ar_{\alpha_1}[lld]\\ n-1 &&\vdots\\&&\mathcal B_n\ar_{\alpha_n}[llu]}\]
  where the vertex of dimension $n-1$ is the middle vertex $(2,1)$ and $\cB_i$ is the quiver
\[\xymatrix@R40pt@C10pt{&&&2n-3&&&\\
\cA_1\ar^{\alpha_1}[rrru]&\hdots&\cA_{i-1}\ar_{\alpha_{i-1}}[ru]&\widehat{\cA_i}&\cA_{i+1}\ar_{\alpha_{i+1}}[lu]&\hdots&\cA_n\ar_{\alpha_n}[lllu]}\]
...
\begin{remark}
  Actually, these examples already show how the (truncated) preprojectives are built, also using the identities \eqref{eq:recursive covers}.
  We should point out that the case $m=3$ is rather special because it is the only case for which the unique morphism $\tilde P_{2,j}\to\tau\tilde P_3^I$ is not surjective.
  Note that if we factor out the image of this morphism, the remaining representation is a direct sum of simple injective representations which disappear after reflecting at the source.
\end{remark}
...
\begin{lemma}
  \label{AR}
  \sayD{Part (1) should follow from Corollary~\ref{cor:truncated preprojective isomorphism}.  Does part (2) have an analogue for $K(n)$?}
  \sayT{The sequence \eqref{ses3} should have an analogue on $K(n)$.  I would be happy with a direct proof of (1) and (2).  I did not check the details so far.  We should also have $\Ext(\tilde P_m^{\uj},P(m,\uj))\neq 0$ on $K(n)$.}
  For $m\geq 3$ and $I\subsetneq\{1,\ldots,n\}$, the following hold.
  %Consider the short exact sequence $$\ses{\tilde P_{m-1,j_l}}{\tilde P_{m}^{\uj'}}{\tilde P_{m}^{\uj}}.$$ The following holds:
  \begin{enumerate}
    \item We have $\tau\tilde P_{m+1}^I=\tilde P_{m-1}^I$.
    ...
    \item For each $j\in I$, there is a short exact sequence
      \begin{align}
        \label{ses3}
...
\end{lemma}
\begin{proof}
  Using the identities \eqref{eq:recursive covers}, it suffices to prove all statements for $m=4$ and arbitrary $1\leq l\leq n-1$.
  Without loss of generality, we choose $I=\{1,\ldots,l\}$.
  We should point out that it is very important for this proof that we consider fixed lifts of all representations in the universal covering as defined in the beginning of this section.
  Note that, starting with these cases, we only reflect at sources and no representation appearing is isomorphic to the simple injective representation $S_2$.  

  In Example~\ref{smallm} it was shown that $\tau\tilde P_4^I=\tilde P_2^I$.
  For $m=5$, we consider the short exact sequence
  \[\ses{\bigoplus_{i\in I}\tilde P_{2,i}}{\tilde P_3}{\tilde P_3^I}\]
  and apply the reflection functor $\tilde\Sigma_1\tilde\Sigma_2$ which is the same as applying $\tau^{-1}$.
  As we have $\tau^{-1}(\tilde P_{2,i})=\tilde P_{4,i}$ and $\tau^{-1}(\tilde P_3)=\tilde P_5$, it follows that $\tau^{-1}\tilde P_3^I=\tilde P_5^I$ which is statement for $m=5$.
  The general statement follows from the identities \eqref{eq:recursive covers}.
  Note that this also shows for $m\ge2$ that
  \[\tilde\Sigma_2(\tilde P_{m+1}^I)^\sigma=\tau^{-1}(\tilde P_m^I).\]

  Considering the support quiver $\tilde P_{3,j}$ from Example~\ref{smallm}, it is clear that we obtain an exact sequence
  \[\ses{\tilde P_3(I,j)}{P_{3,j}}{\tilde P_2^I}.\]
  Actually, the first summand of $\tilde P_3(I,j)$ corresponds to the quivers $\tilde A_i$ at the bottom and the second summand to the $|I|-1$ sources in the second row lying on the left.
  Together with the first statement and the identities \eqref{eq:recursive covers}, this shows the second statement.
\end{proof}

If $m=3$, we replace the short exact sequence \eqref{ses3} by
\begin{align}
  \label{ses4}
  \xymatrix{0\ar[r] &\tilde P_3(I,j)\ar[r] & \tilde P_{2,j}\ar[r] & \left((2,1)\xrightarrow{\alpha_j}(1,\alpha_j)\right)\ar[r] & 0}.
\end{align}
Recall that the image of the unique homomorphism $\tilde P_{2,j}\to\tau\tilde P_3^I$ is just $\left((2,1)\xrightarrow{\alpha_j}(1,\alpha_j)\right)$, see also Example \ref{smallm}. 
...
\end{corollary}
\begin{proof}
  If $m\geq 3$, we consider the long exact sequence obtained when applying $\Hom(\tilde P_{m+1}^I,-)$ to the sequence \eqref{ses3}.
  The result then follows immediately from the identities of Lemma~\ref{AR}.

  If $m=2$, we have $\Hom(\tilde P_3^I,\tilde P_2(I,j))=0$ because $\tilde P_2(I,j)$ is a direct sum of simple projective representations.
  Moreover, it is straightforward that $\Ext(\tilde P_3^I,\tilde P_{1,(j,i)})=0$ for $i\neq j$.
  This is because $\tilde P_{1,(j,i)}$ is supported at a vertex which is not a neighbour of the support of $\tilde P_3^I$, see also Example~\ref{smallm}.
\end{proof}


\newpage
\sayD{Notation changes still needed after this point.}
...
\subsection{Fibrations of Quiver Grassmannians}\label{sec:fibrations}
...
%For simplicity and in order to be in shape(?) with Sections \ref{Lifting} and \ref{sec:fibrations}, we work on the universal cover.
...
\[
  \tilde P_2^{[n-1]}=(1,\alpha_1)\xlongleftarrow{\alpha_1}(2,1).%=P_1^{(1)}=j^{(1)}_1\xlongleftarrow{v_1}i^{(1)}_1
\]
...
%\]
Choosing the lifts $\tilde P_{1,i}$ of $P_1$ supported at $(1,\alpha_i)$ for $2\leq i\leq n$ and using the description of the $\Ext$-groups induced by (\ref{eq:homext}), we have 
...
This yields recursively the truncated preprojective representation $\tilde P_{2}^{[k]}$ for $k=n-1,\ldots,0$ with $\tilde P_2^{[0]}=\tilde P_2$ by 
\begin{align}
  \label{coeff}\xymatrix@R10pt@C20pt{&&(1,\alpha_1)\\ (2,1)\ar^{\alpha_1}[rru]\ar^{\alpha_{n-k}}[rrd] &&\vdots\\&&(1,\alpha_{n-k})}.
\end{align}
For general $m\geq 2$, there exists a short exact sequence
...
We define the $2$-quiver of $\tilde P_2^{[k]}$ by
\\

\begin{tikzpicture}
\tikzstyle{box} = [rectangle, minimum width=2cm, minimum height=1cm, text centered, draw=black]
\draw (0,0) node (A) {$(1,\alpha_{n-k})$} +(3,0) node (B) {$(1,\alpha_{n-k+1})$}+(3,-.8) node (P1) {\footnotesize$ \tilde P_2^{[k+1]}$} +(6,0) node (B1) {$\dots$} +(9,0) node (C) {$(1,\alpha_{2})$} +(9,-.7) node (P3) {\footnotesize$ \tilde P_2^{[n-2]}$}+(11.5,0) node (D) {$(1,\alpha_1)$}  +(14,0) node (E) {$(2,1)$} +(11.5,-.6) node (P4) {\footnotesize$\tilde P_2^{[n-1]}$}; 
...
Note that the image of the homomorphism of the corresponding exact sequence induced by the respective basis elements is the subquiver $(2,\alpha_i^{-1})\xrightarrow{\alpha_i} (1,1)$ for $i=k+1,\ldots, n$. Thus in this case the $2$-arrow connects the $2$-quiver of $\tilde P_3^{[k]}$ to the image of the corresponding homomorphism $\tilde P_2\to \tau \tilde P_3^{[k]}$ which is of dimension $(1,1)$. In view of Lemma \ref{lem:2equivalence}, we forgo to insert the corresponding rectangles around the subquiver and just connected the $2$-arrows to the sinks of the corresponding subquivers. 
 ...
 The chosen basis of $\Ext(P_3^{[n-1]},P_2)$ recursively gives rise to the following $2$-quiver of $P_3^{[k]}$ for $k=n-1,\ldots,0$ where we drop the indices of the vertices in abuse of notation. Note that the middle vertex of $\tilde P_3^{[n-1]}$ is $(2,\alpha_n^{-1})$ and that all $2$-arrows end in the vertex corresponding to $(1,1)$ of $\widetilde{K(n)}$.
\\
...
\draw (2,0) node (I2) {$\bullet$} +(1,1) node (J21) {$\bullet$} +(1,.5) node (J22) {$\bullet$}+(1,0.1) node (D2) { $\vdots$} +(1,-.5) node (J2N1) {$\bullet$} +(1,-1) node (J2N) {$\bullet$} +(0.1,-1.3) node (P1) {\footnotesize$\tilde P_3^{[1]}$} ; 
...
\draw (4,0) node (DOTS) {$\dots$}+(0,-1.2) node (P2b) {\footnotesize$\tilde P_3^{[2]}$};

\draw (6,0) node (I3) {$\bullet$} +(1,1) node (J31) {$\bullet$}   +(1,.5) node (D31) { $\vdots$} +(1,-.1) node (J3N2) {$\bullet$} +(1,-.55) node (J3N1) { $\bullet$} +(1,-1) node (J3N) {$\bullet$} +(0.2,-1.1) node (P3) {\footnotesize$\tilde P_3^{[n-3]}$} ; 
...
\draw (8,0) node (I4) {$\bullet$} +(1,1) node (J41) {$\bullet$} +(1,.5) node (J42) {$\bullet$} +(1,0) node (D4) { $\vdots$} +(1,-.5) node (J4N1) {$\bullet$} +(1,-1) node (J4N) {$\bullet$} +(0.2,-1) node (P4) {\footnotesize$\tilde P_3^{[n-2]}$}; 
...
\draw (10,0) node (I5) {$\bullet$} +(1,1) node (J51) {$\bullet$} +(1,.5) node (J52) {$\bullet$} +(1,0) node (D5) { $\vdots$} +(1,-.5) node (J5N1) {$\bullet$} +(0.2,-.6) node (P5) {\footnotesize$\tilde P_3^{[n-1]}$}   ; 
...
\draw[->,red] (Q2)--(J4N1) node[pos=.5,above,sloped]{\scriptsize$\alpha_{n}$};
\node (Q3) [draw=black,fit={(P4) (Q2) (I4) (J41) (J42) (J4N1) (J4N)}]{};
\draw[->,red] (Q3)--(J3N2)  node[pos=.5,above,sloped] {\scriptsize$\alpha_{n}$};
\node (Q4) [draw=black,fit={(P3) (Q3) (I3) (J31) (J3N1) (J3N2) (J3N)}]{};
\draw[->,red] (Q4)--(DOTS)  node[pos=.5,above] {\scriptsize$\alpha_{n}$};
\node (Q4b) [draw=black,fit={(P2b) (Q4) (DOTS)}]{};

\node (Q5) [draw=black,fit={(P1) (Q4b) (I2) (J21) (J22) (J2N1) (J2N)}]{};
\draw[->,red] (Q5)--(J11)  node[pos=.5,above,sloped,sloped] {\scriptsize$\alpha_{n}$};
\draw[->,red] (Q4b)--(J22)  node[pos=.5,above,sloped] {\scriptsize$\alpha_{n}$};
\draw[->,red] (Q5)--(J11)  node[pos=.5,above,sloped] {\scriptsize$\alpha_{n}$};
...
Now we can recursively the $2$-quivers of $ \tilde P_m^{[k]}$ for $m\geq 4$ and $k=n-1,\ldots,0$ when repeating the procedure and considering the natural basis of $\Ext(P_m^{[n-1]},P_{m-1})$ induced by the decomposition (\ref{eq:homext}). As we have a one-to-one correspondence between basis elements of $\Hom(P_{m-1},\tau P_m^{[k]})$ and $\Ext(P_m^{[k]},P_{m-1})$ when passing to the universal covering quiver, we can proceed as before.

For $m\geq 4$, the truncated preprojective $\tau\tilde P_m^{[k]}$ can uniquely be found as a quotient of $\tilde P_m$.
%Recall that we have $\udim P_m^{[k]}=\udim P_{m-1}^{[1]}+(n-k-1)\udim P_{m-1}$. 
 Let us assume that we already constructed the $2$-quiver of $ P_{m+1}^{[k]}$. The $2$-quiver of $\tilde P^{[k-1]}_{m+1}$ is given by glueing the $2$-quiver of $\tilde P_{m+1}^{[k]}$ to the one of $\tilde P_{m}$ where the $2$-arrows connects the $2$-quiver of $\tilde P_{m+1}^{[k]}$ to its Auslander-Reiten translate $\tau \tilde P_{m+1}^{[k]}= \tilde P_{m-1}^{[k]}$ in $\tilde P_m$. This subquiver can be found at the very right of the $2$-quiver of $\tilde P_m$.
\\

\begin{tikzpicture}
\draw (0,4) node (A1) {$\tilde P_{m-1}$}+(0,-1.2) node (PM) {\footnotesize$\tilde P_{m}$} +(1.5,0) node (B1) {$\tilde P_{m-1}$}+(1.5,-1.1) node (P11) {\footnotesize$\tilde P_{m}^{[1]}$} +(3,0) node (B11) {$\dots$} +(4.5,0) node (D1) {$\tilde P_{m-1}$}  +(6,0) node (E1) {$\tilde P_{m-2}$} +(4.5,-1) node (P41) {\footnotesize$\tilde P_{m}^{[n-2]}$}+(6,-.9) node (P51) {\footnotesize$\tilde P_{m-1}^{[1]}$}+(7.5,0) node (A1b) {$\dots$} +(9,0) node (B1b) {$\tilde P_{m-2}$}+(9,-.8) node (P11b) {\footnotesize$\tilde P_{m-1}^{[k]}$} +(10.5,0) node (B11b) {$\dots$} +(12,0) node (D1b) {$\tilde P_{m-2}$} +(12,-.6) node (P41b) {\footnotesize$\tilde P_{m-1}^{[n-2]}$} +(13.5,0) node (E1b) {$\tilde P^{[n-1]}_{m-2}$} ; 
...
\draw (0,0) node (B) {$\tilde P_{m}$}+(0,-.8) node (P1) {\footnotesize$\tilde P_{m+1}^{[k]}$} +(2.5,0) node (B1) {$\dots$} +(5,0) node (C) {$\tilde P_{m}$} +(5,-.7) node (P3) {\footnotesize$\tilde P_{m+1}^{[n-2]}$}+(7.5,0) node (D) {$\tilde P_{m}$}  +(10,0) node (E) {$\tilde P_{m+1}^{[n-1]}=\tilde P_{m}^{[1]}$} +(7.5,-.6) node (P4) {\footnotesize$\tilde P_{m+1}^{[n-1]}$}; 
\node (Q1) [draw=black,fit={(D) (E) (P4)}]{};
\node (Q2) [draw=black,fit={(Q1) (C) (P3)}]{};
\node (Q2b)[draw=black,fit={(Q2) (B1) }]{};
...
\noindent In order to keep the diagrams as simple as possible we did not insert the labels of the vertices and arrows. As  every vertex  $q\in\mathcal Q(m,k)_0$ corresponds a vertex $\tilde q\in \widetilde{K(n)_0}$, in any case it is straightforward to associate an arrow $\alpha_i\in K(n)_1$ with every ($2$-)arrow of $\mathcal Q(m,k)$. As an example consider the case $n=3$ and $m=3$. The $2$-quiver of $\tilde P_3$ is given by:\\
...
\end{tikzpicture}\\
 ...
 Note that two different vertices of $\mathcal Q(m,k)_0$ can correspond to the same vertex of $\widetilde{K(n)}$. Writing dimension vectors $\tbfe\in\NN^{\widetilde{K(n)_0}}$ as $\tbfe=\sum_{q\in \widetilde{K(n)_0}}\tbfe_q\cdot q$, the dimension types $\tbfe (\beta)$ and  $\bfe (\beta)$ of a subset $\beta\subset\mathcal Q(m,k)_0$ are defined by
$$\tbfe (\beta)=\sum_{q\in\beta}\tilde q\in\NN^{\widetilde{K(n)_0}},\quad \bfe (\beta)=F_{K(n)}\tbfe (\beta).$$


\begin{theorem}\begin{enumerate}
\item  For $\tbfe\in\NN^{\widetilde{K(n)_0}}$, the Euler characteristic $\chi(\Gr_{\tbfe }(\tilde P_m^{[k]}))$ is given by the number of strong successor closed subsets of dimension type $\tbfe $ of the $2$-quiver of $\mathcal Q(m,k)$.
	\item For $\bfe\in\NN^{K(n)_0}$, the Euler characteristic $\chi(\Gr_{\bfe }(P_m^{[k]}))$ is given by the number of strong successor closed subsets of dimension type $\bfe $ of the $2$-quiver of $\mathcal Q(m,k)$.
\end{enumerate}
\end{theorem}
\begin{proof}
The results of Section \ref{QG} imply that the second statement follows from the first one.
We proceed by induction on $m$ and $k$. The case of the representation $\tilde P_1$ is trivial. The strong successor closed subsets of the $2$-quivers  $\mathcal Q(2,k)$ of $\tilde P_2^{[k]}$ coincide with the successor closed subsets of the quivers (\ref{coeff}). As $(\tilde P_2^{[k]})_q\in\{0,1\}$ for all $q\in\widetilde{K(n)}$, the  subrepresentations are in one-to-one correspondence with the successor closed subsets of (\ref{coeff}). Equivalently, we have $\Gr_{\tbfe}(\tilde P_2^{[k]})\in\{\varnothing,\{\pt\}\}$ such that $\Gr_{\tbfe}(\tilde P_2^{[k]})=\{\pt\}$ if and only if $e\subset \mathcal Q(2,k)$ is successor closed.
  ...
  \]
  We have $\mathcal Q(m+1,k-1)_0=\mathcal Q(m+1,k)_0\coprod{}\mathcal Q(m,0)_0$.
	Let $\beta_1\coprod\beta_2\subset \mathcal Q(m+1,k)_0\coprod{}\mathcal Q(m,0)_0$ be a pair of strong successor closed subsets which gives rise to a pair of non-empty cells by induction hypothesis.
 % Denote by $d(\beta_1+\beta_2)$ the dimension vector corresponding to this subset.
  By Proposition \ref{quotient}, the fibre over the pair of cells corresponding to $\beta_1\coprod\beta_2$ is empty if and only if $\beta_2=\mathcal Q(m+1,k)_0$ and $\beta_1\cap\mathcal T(m-1,k)_0=\varnothing$ where $\mathcal T(m-1,k)$ is the $2$-quiver of $\tilde\tau P_{m+1}^{[k]}=\tilde P_{m-1}^{[k]}$ sitting inside $\mathcal Q(m,0)$.
  But this is precisely the condition on $\beta_1\coprod\beta_2$ to be strong successor closed as $\mathcal Q(m+1,k)$ is connected to $\mathcal T(m-1,k)$ by a $2$-arrow.\sayT{compatibility? Classify the cells which satisfy the condition that the quotient on the AR-translate is zero...}
\end{proof}
