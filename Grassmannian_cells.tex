...
The next step is to associate a $2$-quiver with certain lifts of (truncated) preprojective representation $\tilde P_m^{\uj}$ for $\uj\in I_{n-1}$ whose strong successor closed subsets are in one-to-one correspondence with cells of quiver Grassmannians. Recall that the cells of the quiver Grassmannians attached to $P_m^{\uj}:= F_{K(n)}\tilde P_m^{\uj}$ are in one-to-one correspondence with those attached to the lift $\tilde P_m^{\uj}$. Moreover, the quiver Grassmannians $\Gr_{\tbfe}(P_m^V)$ attached to truncated preprojective representations only depend on $\tbfe$ and $\udim P_m^V$. In particular, in view of Theorem \ref{thm:truncpp}, these $2$-quivers can be seen as being attached to any (truncated) preprojective of this dimension.	

Thus it suffices to consider the lifted truncated preprojectives $\tilde P_m^{[k]}:=\tilde P_m^{(1,\ldots,k)}$ for $m\geq 2$ and $0\leq k\leq n-1$. We define $P_m^{[k]}:=F_{K(n)}\tilde P_m^{[k]}$.
...
$$ \Hom(P_{m-1},\tau P_m^{[k]})\cong\bigoplus_{i=k+1}^n\Hom(\alpha_{i}^{(-1)^{m}}\tilde P_{m-1},\tau\tilde P_{m}^{[k]})\cong\bigoplus_{i=k+1}^n\Ext(\tilde P_m^{[k]},\alpha_{i}^{(-1)^{m}}\tilde P_{m-1})\cong\Ext(P_m^{[k]},P_{m-1})$$.  
...
for $i=1,\ldots,n-1$. 
 ...
 This basis recursively gives rise to the following $2$-quiver of $P_3^{[k]}$ for $k=n-1,\ldots,0$ where we drop the indices of the vertices in abuse of notation. Note that the middle vertex of $\tilde P_3^{[n-1]}$ is $(2,\alpha_n^{-1})$ and that all $2$-arrows end in the vertex corresponding to $(1,1)$ of $\widetilde{K(n)}$.
...
Now we can recursively the $2$-quivers of $ \tilde P_m^{[k]}$ for $m\geq 4$ and $k=n-1,\ldots,0$ when repeating the procedure. For $m\geq 4$, the truncated preprojective $\tau\tilde P_m^{[k]}$ can uniquely be found as a quotient of $\tilde P_m$.
...
\noindent where the $2$-arrows connects the $2$-quiver of $\tilde P_{m+1}^{[k]}$ to its Auslander-Reiten translate $\tau \tilde P_{m+1}^{[k]}= \tilde P_{m-1}^{[k]}$ in $\tilde P_m$. This subquiver can be found at the very right of the $2$-quiver of $\tilde P_m$. Note that forgo the colouring of the arrows as it does not play any role from the combinatorial point of view. As an example consider the case $n=3$ and $m=3$. The $2$-quiver of $\tilde P_3$ is given by:\\
...
Write $\mathcal Q(m,k)$ for the $2$-quiver of $\tilde P_m^{[k]}$. Clearly, every vertex  $q\in\mathcal Q(m,k)_0$ corresponds a vertex $\tilde q\in \widetilde{K(n)_0}$. Note that two different vertices of $\mathcal Q(m,k)_0$ can correspond to the same vertex of $\widetilde{K(n)}$. Writing dimension vectors $\tbfe\in\NN^{\widetilde{K(n)_0}}$ as $\tbfe=\sum_{q\in \widetilde{K(n)_0}}\tbfe_q\cdot q$, the dimension types $\tbfe (\beta)$ and  $\bfe (\beta)$ of a subset $\beta\subset\mathcal Q(m,k)_0$ are defined by
...
We proceed by induction on $m$ and $k$. The strong successor closed subsets of the $2$-quivers of $\tilde P_1$, $\tilde P_2$ and $\tilde P_2^{[n-1]}$ coincide with the successor closed subsets of the quivers (\ref{coeff}) which we denote by $\Gamma_M$ for $M\in\{P_1, P_2,P_2^{[n-1]}\}$.
