...
\subsection{Understanding Truncated Preprojectives}
...
\begin{align}\label{recursive}\tilde P_m=\tilde\Sigma_2 \tilde P_{m-1},\quad\tilde P_{m+1}=\tilde\Sigma_{1}\tilde\Sigma_2\tilde P_{m-1}=\tau^{-1} \tilde P_{m-1}.\end{align}
...
\begin{lemma}\label{le:properties}The following holds:
\begin{enumerate}
 \item For $m\geq 2$, we have $\Hom_Q(P_{m-1},P_m)\cong\bigoplus_{i=1}^n \Hom_{\tilde Q}(\tilde P_{m-1,i},\tilde P_m)$, where each $\Hom_{\tilde Q}(\tilde P_{m-1,i},\tilde P_m)$ is one-dimensional;
   ...
   %   \[0\longrightarrow \tilde P_{m-1,i_1}\oplus\cdots\oplus\tilde P_{m-1,i_k}\longrightarrow\tilde P_m\longrightarrow \tilde P_m^{i_1,\ldots,i_k}\longrightarrow 0;\]
    \item For $m\geq 1$, the representations $\tilde P_{m,i}$ are pairwise orthogonal, i.e. for $i\neq k$ we have $$\Hom(\tilde P_{m,i},\tilde P_{m,k})=\Ext(\tilde P_{m,i},\tilde P_{m,k})=0;$$
		...
		\end{enumerate}
\end{lemma} 
\begin{proof}The first statement follows from Theorem \ref{covering}. For $m=1$, the second part is obvious. For $m\geq 2$, it then follows by applying the identities (\ref{recursive}).
...
\begin{align*}\xymatrix@R10pt@C20pt{&&(1,a_1)\\(2,1)\ar^{a_1}[rru]\ar^{a_{n}}[rrd] &&\vdots\\&&(1,a_n)}\end{align*}
...
The representation $\tilde P_3$ is given by the quiver
	...
	\begin{align*}\xymatrix@R10pt@C20pt{&&(2,a_{j_1}^{-1})\ar_{a_{j_1}}[lld]\\(1,1) &&\vdots\\&&(2,a_{j_l}^{-1})\ar_{a_{j_l}}[llu]}\end{align*}
...
\begin{lemma}\label{le:subrep}
  Let $\uj\in I_l$. All non-trivial proper subrepresentations $U$ of $\tilde P_m^{\uj}$ are preprojective.	
...
In this section we provide a direct quiver theoretic description of the non-empty cells in the quiver Grassmannians of (truncated) preprojective representations of $K(n)$.
