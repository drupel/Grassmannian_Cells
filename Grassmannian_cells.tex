...
\sayD{These edits make the statements false.  By viewing the reflection functors as endofunctors, we have to interchange the vertex labels so that the inverse of $\Sigma_1$ is $\Sigma_2$.  Maybe we want to explicitly introduce the functor identifying $\rep K(n)$ with $\rep K(n)^{op}$, is there a standard notation for this?}
...
\begin{remark}
  ...
  \begin{enumerate}
    \item These representations satisfy the following recursions using the reflection functors:
...
\begin{proposition}
  ...
  Let $m\geq 1$. Let $R_\alpha(K(n))$ be the affine space of representations of dimension $\alpha(m,r)$.
  The following hold:
  ...
  \begin{enumerate}
    ...
    \item The set indecomposable representations of dimension $\alpha(m,r)$ is given by a non-empty open subset of $R_\alpha(K(n))$. 
...
\item For every $\uj\in I_l$, there exists a truncated preprojective representation $\tilde P_{m}^{\uj}$ which is the cokernel of
      \begin{align}\label{ses1}\ses{\bigoplus_{i=1}^l \tilde P_{m-1,j_i}}{\tilde P_m}{\tilde P_{m}^{\uj}}.\end{align}
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item There exists a short exact sequences 
      \begin{align}\label{ses3}\sesm{P(m,\uj)}{\tilde P_{m-1,j_l}}{\tau\tilde P_{m}^{\uj}}{\pi^{\bf j_l}_{m-1}}\end{align}
...
If $m=3$, we replace the short exact sequence (\ref{ses3}) by
 \begin{align}\label{ses4}\xymatrix{0\ar[r] &P(3,\uj)\ar[r] &  \tilde P_{2,j_l}\ar[r] & \left((2,1)\xrightarrow{a_{j_l}}(1,a_{j_l})\right)\ar[r] & 0}.\end{align}
...
\begin{proposition} \label{indecomposables}
  \sayD{Move the first two of these into section~\ref{sec:truncated preprojectives}?}
  \sayT{moved}  Let $m\geq 1$ and $\alpha(m,r)=\dim P_{m+1}-r\dim P_m$ with $0\leq r\leq n-1$. The action of $\Gl_n(\CC)$ is transitive on $\mathrm{Ind}(K(n),\alpha(m,r))$.
