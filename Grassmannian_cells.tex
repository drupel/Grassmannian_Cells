...
Observe by an easy induction that $u_{k+1}-u_k>0$ for $k\ge0$ since $n\ge3$.
...
\begin{theorem}
  ...
  \[\udim P_m=(u_m,u_{m-1}),\qquad\qquad \udim I_m=(u_{m-1},u_m).\]
  These satisfy 
  \[\Hom(P_m,P_r)=0, \qquad \Hom(I_r,I_m)=0, \qquad \Ext(P_r,P_m)=0, \qquad \Ext(I_m,I_r)=0\]
  for $1\leq r\leq m$.
...
To simplify the notation, we abbreviate $\tilde P_{2\ell-1,j}:=\tilde P_{2\ell-1,\alpha_j}$ and $\tilde P_{2\ell,j}:=\tilde P_{2\ell,\alpha_j^{-1}}$.
\begin{lemma}
...
Depending on context (in particular, the parity of $m$), we will either identify the word~$(i_1,\ldots,i_k)$ with the element $\alpha_{i_1}\alpha_{i_2}^{-1}\alpha_{i_3}\cdots\alpha_{i_k}^{(-1)^{k+1}}\in W_n$ when $m$ is even or with the element $\alpha_{i_1}^{-1}\alpha_{i_2}\alpha_{i_3}^{-1}\cdots\alpha_{i_k}^{(-1)^k}\in W_n$ when $m$ is odd.
...
Note that, although there is a translate of $\tilde P_{m+1-k}$ corresponding to each word $\ui\in A_1^{(\ell)}$ for $1\le \ell<k$, these will not be naturally equipped with a canonical inclusion into $\tilde P_{m+1}$.
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item For $m\ge2$, we have $\Hom(\tilde P_{m,j},\tau\tilde P_{m+1}^I)\cong\CC$.
      ...
      \[\tilde P_m(I,j):=
        \begin{cases}
          \bigoplus_{\substack{1\leq k\leq n\\k\neq j}}\tilde P_{m-1,(j,k)}\oplus \bigoplus_{i\in I\setminus\{j\}j}\tilde P_{m-2,(j,j,i)} & \text{ if $m\geq 3$;}\\
...
\begin{proof}
  ...
  For the $m\ge3$ case, we apply Lemma~\ref{le:properties} to get the lower sequence in the following commutative diagram
  \[
    \xymatrix{
  ...
  \]
  which exists because the upper sequence is almost split.
  ...
  It then follows from Lemma~\ref{le:homdecomposition} that $\tilde P_{m-1,(j,k)}$ is in the kernel of the vertical morphism $\eta$ for $k\in I\setminus\{j\}$.
  ...
  This also shows that the vertical map $\eta$ is surjective, and thus the map $\tilde P_{m,j}\to \tilde P_{m-1}^I\cong \tilde P_{m+1}^I$ is surjective.
...
\begin{corollary}
  ...
  Consider a non-empty subset $I\subsetneq\{1,\ldots,n\}$ and fix an element $j\in I$.
  For $m\geq 2$, we have $\Hom(\tilde P_{m+1}^I,\tilde P_m(I,j))=\Ext(\tilde P_{m+1}^I,\tilde P_m(I,j))=0$.
\end{corollary}
\begin{proof}
...
\begin{theorem}
  \label{cellscover}
  For $m\geq 1$ and $J\subsetneq\{1,\ldots,n\}$, every quiver Grassmannian $\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$ admits a cell decomposition. In particular, every quiver Grassmannian of a lifted preprojective representation admits a cell decomposition.
\end{theorem}
\begin{proof}
...
\begin{remark}
Note that having proved the claim, for $|J|\leq n-2$ the statement itself is obtained when setting $\bfI=(J\cup\{i\})$ for any $i\in\{1,\ldots,n\}\backslash\{J\}$. For $|J|=n-1$, we can apply Lemma~\ref{le:truncated quotients}, i.e. $P_{m+1}^J\cong\tilde P_{m,j}^{\{j\}}$ with $J^c=\{j\}$, together with the fact that $\tilde P_{m,j}^{\{j\}}$ is obtained when translating $\tilde P_{m}^{\{j\}}$ in the universal covering quiver. Clearly, translation does not affect quiver Grassmannians when also translating dimension vectors $\tbfe$, i.e. quiver Grassmannians of $\tilde P_{m,j}^{\{j\}}$ are just the same as those of $\tilde P_{m}^{\{j\}}$.
...
Finally, note that the claim together with Proposition \ref{label} ensures that the fiber dimension along the points of the affine cell $C_V$ is constant. Thus Theorem \ref{vb} gives the statement of the Theorem.
\end{remark}
