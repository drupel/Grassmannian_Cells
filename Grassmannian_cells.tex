  ...
  \begin{enumerate}
    ...
    \item For any pair $i,j\in\{1,\ldots,n\}$ with $i\ne j$, there is a commutative diagram
      \begin{equation}
        ...
        \xymatrix@R40pt@C60pt{\tilde P_{m+1} \ar^(.4){\tilde\pi_{m+1}^{\{i\}^c}}[d] & \ar[l] \tilde P_{m,i} \ar_(.45){\tilde\pi_{m,i}^{\{i\}}}[dl]\ar^(.4){\tilde\pi_{m,i}^{\{j\}^c}}[d] & \ar[l] \tilde P_{m-1,(i,j)} \ar_(.45){\tilde\pi_{m-1,(i,j)}^{\{j\}}}[dl]\\
...
\begin{proof}
  ...
  The commutativity of the upper triangles in (2) follows from the construction of the isomorphisms in the bottom row, while the commutativity of the lower triangle is immediate from Lemma~\ref{le:properties}.5.

  These diagrams stitch together to give the following composition, where we suppress the lower isomorphisms in the diagram \eqref{dia:truncated coherence}:
...
Finally, note that the claim together with Proposition \ref{fibers} ensures that the fiber dimension along the points of the affine cell $C_V$ is constant. Thus Theorem \ref{vb} gives the statement of the Theorem.
\end{remark}
