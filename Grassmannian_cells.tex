...
\begin{align}\label{eq:homext} \bigoplus_{i=k+1}^n\Hom(\alpha_{i}^{(-1)^{m}}.\tilde P_{m-1},\tau\tilde P_{m}^{[k]})\cong\bigoplus_{i=k+1}^n\Ext(\tilde P_m^{[k]},\alpha_{i}^{(-1)^{m}}. \tilde P_{m-1})\cong \Ext(P_m^{[k]},P_{m-1}).\end{align}
