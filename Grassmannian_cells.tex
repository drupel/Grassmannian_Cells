...
\begin{abstract}
  ...
  The provided bijection between these two labelings gives a geometric explanation for the appearance of Dyck path combinatorics in the theory of quiver Grassmannians. 
	\subclass{16G20,14M15,13F60}
        \keywords{Quiver Grassmannians, torus action, generalized Kronecker quiver, cell decompositions, combinatorial labeling of cells, Dyck path combinatorics}
...
Although it follows from the results of Hille, Huisgen-Zimmermann, Reineke, and Ringel that every projective variety can be realized as a quiver Grassmannian, 
...
A consequence of our main result is a geometric explanation for these computations: we provide a one-to-one correspondence between the strong successor closed subsets of the associated 2-quiver and compatible pairs for an appropriate Dyck path which leads to a geometric explanation for the appearance of Dyck path combinatorics in the theory of quiver Grassmannians. 

The paper is organized as follows.
In Section~\ref{sec:covering}, we collect several results concerning quiver covering theory.
In Section~\ref{sec:RepK(n)}, we recall basic facts concerning the representation theory of generalized Kronecker quivers $K(n)$ which are needed later to investigate the quiver Grassmannians attached to preprojective representations (the preinjective case follows by duality and we do not address it directly).
...
When $X$ lifts all the way to the universal covering quiver, this $\CC^*$-action can be iterated in such a way that the ultimate $\CC^*$-fixed points are precisely the subrepresentations which can be lifted to the universal covering quiver.
...
\label{sec:covering}
\noindent
We refer to \cite{gab} for an introduction to quiver covering theory.
...
\begin{lemma}
  ...
  Any lift of a preprojective (or preinjective) representation is preprojective (preinjective). 
\end{lemma}
\begin{proof}
...
\begin{theorem}
  \label{th:rigids}
  For each $m\ge1$, there exist unique (up to isomorphism) exceptional representations $P_m$ and $Q_m$ of $K(n)$ with dimension vectors 
...
\begin{remark}
  ...
  \begin{enumerate}
    \item The preprojective and preinjective representations satisfy the following recursions using the reflection functors:
      \[P_1=S_1,\quad P_m^\sigma=\Sigma_2 P_{m-1},\quad Q_1=S_2,\quad Q_m^\sigma=\Sigma_1 Q_{m-1}\]
      for $m\ge2$.
...
\begin{definition}
  ...
  \begin{equation}
    ...
    0\longrightarrow P_m\otimes V\stackrel{ev_V}{\longrightarrow} P_{m+1}\stackrel{\pi_{m+1}^V}{\longrightarrow} P_{m+1}^V\longrightarrow 0.
  \end{equation}
\end{definition}
\begin{remark}
...
\begin{lemma}
  ...
  For $V\in \Gr(\cH_m)$, $m\ge1$, the space $\Hom(P_{m+1},P_{m+1}^V)$ is one-dimensional spanned by the natural projection $\pi_{m+1}^V:P_{m+1}\to P_{m+1}^V$.
...
\begin{proof}
 As $\Hom(P_{m+1},P_m)=\Ext(P_{m+1},P_m)=0$, applying the functor $\Hom(P_{m+1},-)$ to the sequence \eqref{eq:truncated preprojectives} gives isomorphisms 
  ...
  Under the first isomorphism, the identity map on $P_{m+1}$ is taken to the projection $\pi_{m+1}^V:P_{m+1}\to P_{m+1}^V$.
...
\begin{proof}
  ...
  \[\xymatrix{
      ...
      0 \ar[r] & P_m\otimes V \ar[r]^{ev_V} & P_{m+1} \ar[r]^{\pi_{m+1}^V} & P_{m+1}^V \ar[r] & 0}\]
...
The truncated preprojective representation $P_{m+1}^V$ has dimension vector
\[\udim P_{m+1}^V=\bfd(m,\dim V):=\udim P_{m+1}-\dim V\cdot\udim P_m.\]
...
\end{lemma}
\begin{proof}We use some of the ideas of the proof of \cite[Lemma IV.1.7]{ass}. For every proper subrepresentation $X$ of $P_{m+1}^V$ together with the natural embedding $\iota$, we get a pullback diagram
\[\xymatrix{0 \ar[r] & P_m\otimes V \ar@{=}[d]\ar^f[r] & E\ar^g[r]\ar^h[d] & X \ar^{\iota}[d]\ar[r] & 0 \\
    0 \ar[r] & P_m\otimes V \ar^{ev_V}[r] & P_{m+1} \ar^{\pi_{m+1}^V}[r] & P_{m+1}^V \ar[r] & 0}\]

  Since the map $ev_V$ is irreducible, $f$ is either a section or $h$ a retraction.  As $h$ is injective by the Snake Lemma, it cannot be a retraction. It follows that $f$ is a section and thus $g$ a retraction. Thus we find a morphism $g':X\to E$ such that $g\circ g'=\mathrm{id}_X$. We obtain
	$$\iota=\iota\circ\mathrm{id}_X=\iota\circ g\circ g'=\pi_{m+1}^V \circ h\circ g',$$i.e. $\iota$ factors through $P_{m+1}$,	and the claim follows from Lemma~\ref{le:preprojective subs}.
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item For each proper subset $I\subsetneq\{1,\ldots,n\}$, there exists a truncated preprojective representation~$\tilde P_{m+1}^I$ fitting into an exact sequence 
      \begin{equation}
        \label{ses1}
        \xymatrix{ 0 \ar[r] & \bigoplus_{i\in I} \tilde P_{m,i} \ar[r]^{ev_I} & \tilde P_{m+1} \ar[r]^{\tilde\pi_{m+1}^I} & \tilde P_{m+1}^I \ar[r] & 0}.
      \end{equation}
      Moreover, $G(\tilde P_{m+1}^I)=:P_{m+1}^{\langle I\rangle}$ is a truncated preprojective of $K(n)$ for each $I\subsetneq\{1,\ldots,n\}$.
  \end{enumerate}
\end{lemma}
\begin{remark}
...
\begin{proof}
  ...
  For part (4), observe that under the isomorphism from part (1) the subset $I\subsetneq\{1,\ldots,n\}$ corresponds to the subspace $\langle I\rangle\subset \cH_m$ spanned by the direct summands $\Hom(\tilde P_{m,i},\tilde P_{m+1})$ for $i\in I$.
  The map $\bigoplus_{i\in I} \tilde P_{m,i}\to\tilde P_{m+1}$ is then a lift of the evaluation morphism $ev_{\langle I\rangle}:P_m\otimes \langle I\rangle\to P_{m+1}$ and hence is injective by Lemma~\ref{le:injective evaluation maps}.
  Taking the cokernel defines the truncated preprojective $\tilde P_{m+1}^I$ and the preceding discussion shows that $G(\tilde P_{m+1}^I)\cong P_{m+1}^{\langle I\rangle}$ is truncated preprojective as well.
\qed\end{proof}
  ...
  \begin{enumerate}
    ...
    \item For each proper subset $J\subsetneq I$, there is an exact sequence 
      \begin{equation}
        ...
        \xymatrix{ 0 \ar[r] & \bigoplus_{i\in I\setminus J} \tilde P_{m,i} \ar[r] & \tilde P^J_{m+1} \ar[r]^{\tilde\pi_{m+1}^{I,J}} & \tilde P_{m+1}^I \ar[r] & 0}.
      \end{equation}
      For any sequence of proper subsets $K\subsetneq J\subsetneq I$, the quotient maps satisfy $\pi_{m+1}^{I,K}=\pi_{m+1}^{I,J}\circ\pi_{m+1}^{J,K}$ and $\pi_{m+1}^I=\pi_{m+1}^{I,J}\circ\pi_{m+1}^J$.
  \end{enumerate}
\end{lemma} 
\begin{proof}
...
\begin{corollary}
  \label{cor:truncated lift}
  For $m\ge1$ and $I\subsetneq\{1,\ldots,n\}$, the lift $\widetilde P_{m+1}^I$ of the truncated preprojective $P_{m+1}^{\langle I\rangle}$ is preprojective.
\end{corollary}
\begin{proof}
  We see explicitly that $\widetilde P_2^I=\Big(\prod\limits_{j\in I^c}\tilde\Sigma_{(1,\alpha_j)}\Big)(\tilde S_{(2,e)})$ is preprojective.
  The case $m\ge2$ then follows immediately from Lemma~\ref{le:truncated tau}.
\qed\end{proof}
...
Here we allow $k=0$ and take $A_1^{(0)}$ to consist of only the empty word $\ui=()$.
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item For $i\in\{1,\ldots,n\}$, there is a canonical isomorphism 
      \[\tilde P_{m+1}^{\{i\}^c}\cong\tilde P_{m,i}^{\{i\}}=\tilde P_{m,i}/\tilde P_{m-1,(i,i)}.\]
    \item For any pair $i,j\in\{1,\ldots,n\}$ with $i\ne j$ and any $J\subsetneq\{i\}^c$, there is a commutative diagram
      \begin{equation}
        ...
        \xymatrix@R40pt@C60pt{
           \ar[r] \tilde P_{m-1,(i,j)} \ar_(.45){\tilde\pi_{m-1,(i,j)}^{\{j\}}}[d]
          & \ar[r] \tilde P_{m,i} \ar_(.45){\tilde\pi_{m,i}^{\{i\}}}[d] \ar_{\tilde\pi_{m,i}^{\{j\}^c}}[dl] 
          & \tilde P_{m+1} \ar_(.45){\tilde\pi_{m+1}^J}[d] \ar_{\tilde\pi_{m+1}^{\{i\}^c}}[dl]\\ 
          \tilde P_{m-1,(i,j)}^{\{j\}}\cong \tilde P_{m,i}^{\{j\}^c}
          & \tilde P_{m,i}^{\{i\}}\cong\tilde P_{m+1}^{\{i\}^c} \ar_(.4){\tilde\pi_{m,i}^{\{j\}^c,\{i\}}}[l] 
          & \tilde P_{m+1}^J \ar_(.4){\tilde\pi_{m+1}^{\{i\}^c,J}}[l]}
      \end{equation}
    \item For an admissible sequence $\ui\in A_1^{(k)}$, $1\le k< m$, and $J\subsetneq\{1,\ldots,n\}$, the restriction of $\tilde\pi_{m+1}^J:\tilde P_{m+1}\to\tilde P_{m+1}^J$ to $\tilde P_{m+1-k,\ui}$ is injective if $J\subsetneq\{i_1\}^c$ and is zero if $i_1\in J$.
    \item For any admissible word $\ui\in A_1^{(k)}$ with $0\le k< m$ and any proper subsets $I,J\subsetneq\{1,\ldots,n\}$ with~$i_k\in I$ and $i_1\notin J$ if $k\ge1$ or with $J\subsetneq I$ if $k=0$, there exist (unique up to scale) surjective maps~$\tilde\pi_{m+1}^{\ui,I,J}:\tilde P_{m+1}^J\to \tilde P_{m+1-k,\ui}^I$.
      These satisfy the following relations:
      \begin{itemize}
        \item $\tilde\pi_{m+1}^{\ui,I,J}\circ\tilde\pi_{m+1}^{J,K}=\tilde\pi_{m+1}^{\ui,I,K}$ for $K\subsetneq J$;
        \item $\tilde\pi_{m+1-k,\ui}^{H,I}\circ\tilde\pi_{m+1}^{\ui,I,J}=\tilde\pi_{m+1}^{\ui,H,J}$ for $I\subsetneq H\subsetneq\{1,\ldots,n\}$;
        %\item $\tilde\pi_{m+1}^{\ui,\varnothing,I}\Big|_{\tilde P_{m+1-k,\ui}} =\tilde\pi_{m+1-k,\ui}^I$;
        %\item $\tilde\pi_{m+1}^{\ui,I,J}\Big|_{\tilde P_{m+1-\ell,\ui_\ell}}=\tilde\pi_{m+1-\ell,\ui_\ell}^{\ui'_\ell,I,\varnothing}$ for $1\le \ell\le k$, where $\ui_\ell:=(i_1,\ldots,i_\ell)$ and $\ui'_\ell:=(i_{\ell+1},\ldots,i_k)$.
      \end{itemize}
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  \[\tilde\pi_{m+1}^{\ui,I,J}:=\tilde\pi_{m+1-k,\ui}^{I,\{i_k\}}\circ\tilde\pi_{m+2-k,\ui_{k-1}}^{\{i_k\}^c,\{i_{k-1}\}}\circ\cdots\circ\tilde\pi_{m,i_1}^{\{i_2\}^c,\{i_1\}}\circ\tilde\pi_{m+1}^{\{i_1\}^c,J}.\]
...
\begin{proof}
  ...
  Therefore $\ker\eta$ is the direct sum of these with the kernel of the induced map $\tilde P_{m-1}\to\tilde P_{m-1}^{I\setminus\{j\}}$, that is with $\bigoplus_{i\in I\setminus\{j\}} \tilde P_{m-2,i}=\bigoplus_{i\in I\setminus\{j\}} \tilde P_{m-2,(j,j,i)}$, and so $\ker\eta=\tilde P_m(j,I)$.
...
\begin{corollary}
  ...
  Consider a non-empty subset $I\subsetneq\{1,\ldots,n\}$ and fix an element $j\in I$.
  For $m\geq 2$, we have $\tilde P_m(j,I)\in (\tilde P_{m+1}^I)^\perp$.
\end{corollary}
\begin{proof}
  ...
  \[\ses{\tilde P_m(j,I)}{\tilde P_{m,j}}{\tau\tilde P_{m+1}^I}.\]
  Following Lemma~\ref{le:properties}.3, we have $\Hom(\tilde P_{m+1}^I,\tilde P_{m,j})=0$ and using the long exact sequence this immediately implies $\Hom\big(\tilde P_{m+1}^I,\tilde P_m(j,I)\big)=0$.
  ...
  Combining with the preceding discussion, this gives $\Ext\!\big(\tilde P_{m+1}^I,\tilde P_m(j,I)\big)=0$.
\qed\end{proof}
...
The following result of Caldero and Reineke lays a foundation for our approach.
\begin{theorem}
  \cite{cr}
  \label{thm:smooth rigid}
  Every quiver Grassmannian of a rigid representations of an acyclic quiver is smooth.
\end{theorem}
This allows the application of the torus method of Bia\l{}ynicki-Birula described in the following section.
...
\begin{theorem}
  ...
  In particular, the existence of a cell decomposition of $C$ implies a cell decomposition of $\Att(C)$.
  ...
  In particular, we have an equality of Euler characteristics $\chi(X)=\chi(X^{\C^\ast})$ and, when each $C_i$ admits a cell decomposition, we obtain a cell decomposition of $X$.
\end{theorem}
...
\begin{corollary}
  \label{cor:iterated fixed points}
...
Define the $\cF$-polynomial of a representation $X$ by 
\[\cF_X=\sum_{\bfe\in\NN^{Q_0}}\chi(\Gr_\bfe(X))y^\bfe\in\ZZ[y_i\mid i\in Q_0].\]
\begin{corollary}
  \label{fpoly}
  ...
  \begin{enumerate}
    ...
    \item We have $\cF_X=S\cF_{\tilde X}$ where $S\cF_{\tilde X}$ is obtained from $\cF_{\tilde X}$ by applying $S:\ZZ[y_{(i,w)}\mid i\in Q_0,w\in W_Q]\to\ZZ[y_i\mid i\in Q_0]$ given by $S(y_{(i,w)})=y_i$ for all $i\in Q_0$ and $w\in W_Q$.
  \end{enumerate} 
\end{corollary}
...
\begin{theorem}
  ...
  Moreover, for $V,W\in \Gr_r(\cH_m)$, $1\le r\le n-1$, we have $\Gr_\bfe(P_{m+1}^V)\cong \Gr_\bfe(P_{m+1}^W)$.
\end{theorem}
\begin{proof}
  Let $g\in\GL_n(\CC)$.
...
Let $\eta:\ses{\tilde M}{\tilde B}{\tilde N}$ be a short exact sequence of $\widetilde{K(n)}$-representations.
For a fixed dimension vector $\tbfe$, there is an induced ``Caldero-Chapoton map'' between quiver Grassmannians
\begin{align*}
  \Psi=\Psi^\eta:\Gr_\tbfe(\tilde B)&\to\bigsqcup_{\tbff+\tbfg=\tbfe} \Gr_\tbff(\tilde M)\times \Gr_\tbfg(\tilde N)\\
  E&\mapsto \big(\tilde E\cap \tilde M,(\tilde E+\tilde M)/\tilde M\big).
\end{align*}
Following \cite[Section 3]{cc}, any non-empty fiber of $\Psi$ satisfies $\Psi^{-1}(\tilde F,\tilde G)\cong\mathbb{A}^{\dim\Hom(\tilde G,\tilde M/\tilde F)}$.
For $\cG_{\tbff,\tbfg}:=\Psi^{-1}\big(\Gr_\tbff(\tilde M)\times \Gr_\tbfg(\tilde N)\big)$, we have 
\begin{equation}
  \label{eq:grassmannian decomposition}
  \Gr_\tbfe(\tilde B)=\bigsqcup_{\tbff+\tbfg=\tbfe} \cG_{\tbff,\tbfg}.
\end{equation}
Then $\Psi$ restricts to a map
\[\Psi_{\tbff,\tbfg}:\cG_{\tbff,\tbfg}\to \Gr_\tbff(\tilde M)\times \Gr_\tbfg(\tilde N).\]
...
\begin{lemma}
  ...
  is closed in $\Gr_\tbfe(\tilde B)$.
\end{lemma}
\begin{proof}
  Recall that $\Gr_\tbfe(\tilde B)$ is a closed subvariety of $\prod_{i\in Q_0} \Gr_{e_i}(\tilde B_i)$.
  For each $i$, the inclusion $\tilde M_i\subset \tilde B_i$ induces an upper-semicontinuous function $\rho_i:\Gr_{e_i}(B_i)\to\ZZ$ given by $\rho_i(\tilde E_i)=\dim(\tilde E_i\cap \tilde M_i)$.
  In particular, for any fixed $f_i$ the set $\rho_i^{-1}\big(\{f_i,f_i+1,\ldots,e_i\}\big)$ is closed in $\Gr_{e_i}(\tilde B_i)$.
  ...
  \[\bigsqcup_{ \tbff'\succ\tbff} \cG_{\tbff',\tbfe-\tbff'}\subset \Gr_\tbfe(\tilde B)\]
  for any fixed $\tbff$.
...
\begin{remark}
  ...
  To apply Theorem~\ref{vb} and establish cell decompositions for the $\cG_{\tbff,\tbfg}$, we will find a cell decomposition of $\Gr_\tbff(\tilde M)\times\Gr_\tbfg(\tilde N)$ such that $\Im\big(\Psi_{\tbff,\tbfg}\big)$ is a union of affine cells, giving an induced cell decomposition of $\Im\big(\Psi_{\tbff,\tbfg}\big)$.

  By Lemma~\ref{le:good partition}, the existence of cell decompositions for each $\cG_{\tbff,\tbfg}$ implies the existence of a cell decomposition for $\Gr_\tbfe(\tilde B)$.
  Indeed, we may take the affine cells of all the $\cG_{\tbff,\tbfg}$ as affine cells of $\Gr_\tbfe(\tilde B)$ with the natural lexicographic total order induced by taking cells from $\cG_{\tbff',\tbfg'}$ after those of $\cG_{\tbff,\tbfg}$ whenever $\tbff'\succ\tbff$.
\end{remark}
...
\begin{equation}
  ...
  \Psi=\Psi^{\eta_m}:\Gr_\tbfe(\tilde P_{m+1}^J)\to\bigsqcup_{\tbff+\tbfg=\tbfe} \Gr_\tbff(\tilde P_{m,j})\times\Gr_\tbfg(\tilde P_{m+1}^I).
\end{equation}
...
\begin{equation}
  ...
  \xi_m:\ses{\tilde P_m(j,I)}{\tilde P_{m,j}}{\tilde C_m},
\end{equation}
where $\tilde C_m=\tilde C_m(I)=\tau\tilde P_{m+1}^I$ for $m\ge3$ and $\tilde C_2=\tilde C_2(j,I)$ is the representation in \eqref{eq:special case} from Remark~\ref{rem:special case}.
Then, in the same way as above, we obtain the following map for any fixed $\tbff$:
\begin{equation}
  \label{eq:truncated CC2}
  \Phi=\Phi^{\xi_m}:\Gr_\tbff(\tilde P_{m,j})\to\bigsqcup_{\tbfs+\tbft=\tbff} \Gr_\tbfs\big(\tilde P_m(j,I)\big)\times\Gr_\tbft(\tilde C_m).
\end{equation}
\begin{proposition}
  \label{quotient}
  ...
  \begin{enumerate}
    \item The fiber $\Psi^{-1}(\tilde F,\tilde P_{m+1}^I)$ is not empty if and only if $\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)=0$.
    \item The fiber $\Psi^{-1}(\tilde F,\tilde P_{m+1}^I)$ is empty if and only if $\Phi(\tilde F)=(\tilde F,0)$, i.e.\ $\tilde F$ is already a subrepresentation of $\tilde P_m(j,I)$.
  \end{enumerate} 
\end{proposition}
\begin{proof}
  Any subrepresentation $\tilde F\subset\tilde P_{m,j}$ produces an exact sequence
  \[\xymatrix{\Ext(\tilde P_{m+1}^I,\tilde F)\ar[r] &\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\ar[r] &\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)\ar[r] & 0}.\]
  But note that the fiber $\Psi^{-1}(\tilde F,\tilde P_{m+1}^I)$ being non-empty gives rise to a pushout diagram
  \[\xymatrix{0\ar[r]& \tilde F\ar[r]\ar[d]\ar@{}[dr]|(.3){\ulcorner} & \tilde F'\ar[d]\ar[r]& \tilde P_{m+1}^I\ar[r]\ar@{=}[d]& 0\\
  ...
  \[\Ext(\tilde P_{m+1}^I,\tilde F)\to\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\cong\CC\]
  is surjective and thus $\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)=0$.
  This argument can be reversed and thus (1) holds.
  
  Now consider $\tilde F\subset\tilde P_{m,j}$ with 
  \[\Phi(\tilde F)=(\tilde S,\tilde T)\in\Gr_\tbfs\big(\tilde P_m(j,I)\big)\times\Gr_\tbft(\tilde C_m).\]
  This gives rise to the following commutative diagram:  
  \[\xymatrix{
    \Ext(\tilde P_{m+1}^I,\tilde S)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde F)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde T)\ar[r]\ar[d] & 0\\
    \Ext\big(\tilde P_{m+1}^I,\tilde P_m(j,I)\big)\ar[r]\ar[d] &  \Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\ar[r]\ar[d] & \Ext(\tilde P_{m+1}^I,\tilde C_m)\ar[r]\ar[d] & 0\\
    \Ext\big(\tilde P_{m+1}^I,\tilde P_m(j,I)/\tilde S\big)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)\ar[r]\ar[d] &\Ext(\tilde P_{m+1}^I,\tilde C_m/\tilde T)\ar[r]\ar[d] & 0 \\
    0&0&0&}\]
    By Corollary~\ref{cor:perpendicular}, we have $\Ext\big(\tilde P_{m+1}^I,\tilde P_m(j,I)\big)=0$ and so $\Ext\big(\tilde P_{m+1}^I,\tilde P_m(j,I)/\tilde S\big)=0$ as well.
  This yields the isomorphisms
  \[\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\cong\Ext(\tilde P_{m+1}^I,\tilde C_m)\quad\text{and}\quad\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)\cong\Ext(\tilde P_{m+1}^I,\tilde C_m/\tilde T).\]

  If $\tilde T=0$, we get an isomorphism $\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)\cong\Ext(\tilde P_{m+1}^I,\tilde P_{m,j})\cong\CC$ and by part (1) we must have an empty fiber $\Psi^{-1}(\tilde F,\tilde P_{m+1}^I)=\varnothing$.

  If $\tilde T\neq 0$, $\tilde C_m/\tilde T$ is a proper factor of $\tilde C_m$ and we must have $\Ext(\tilde P_{m+1}^I,\tilde C_m/\tilde T)=0$.
  Indeed, by Auslander-Reiten theory every non-split morphism $g:\tilde C_m\to\tilde C_m/\tilde T$ factors through the middle term $\tilde Z$ of the AR-sequence 
  \begin{equation}
    \label{eq:ar2}
    \ses{\tilde C_m}{\tilde Z}{\tilde P_{m+1}^I}.
  \end{equation}
  This in particular says the first map of the induced sequence
  \[\Hom(\tilde Z,\tilde C_m/\tilde T)\to\Hom(\tilde C_m,\tilde C_m/\tilde T)\to\Ext(\tilde P_{m+1}^I,\tilde C_m/\tilde T)\to\Ext(\tilde Z,\tilde C_m/\tilde T)\]
  is surjective.
  By Lemma~\ref{le:properties}.4 and the Auslander-Reiten formulas, we have 
  \[\Ext(\tilde C_m,\tilde C_m)=0\qquad\text{and}\qquad\Ext(\tilde P_{m+1}^I,\tilde C_m)=\CC.\]
  For $m=2$, the identities above follow immediately from the corresponding statements for $\tau\tilde P_3^I$.
  Thus the injective map $\CC=\Hom(\tilde C_m,\tilde C_m)\to\Ext(\tilde P_{m+1}^I,\tilde C_m)$ induced by the Auslander-Reiten sequence \eqref{eq:ar2} is actually bijective and so applying $\Hom(-,\tilde C_m)$ to this sequence yields $\Ext(\tilde Z,\tilde C_m)=0$.
  But then $\Ext(\tilde Z,\tilde C_m/\tilde T)=0$ and so $\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)\cong\Ext(\tilde P_{m+1}^I,\tilde C_m/\tilde T)$ must be zero as well.
  From part (1), we see that the fiber $\Psi^{-1}(\tilde F,\tilde P_{m+1}^I)$ is non-empty in this case.
\qed\end{proof}
...
\begin{proposition}
  ...
  The following hold:
  \begin{enumerate}
    \item For $\tilde G\subsetneq\tilde P_{m+1}^I$ and $\tilde F\subseteq\tilde P_{m,j}$, we have $\Psi^{-1}(\tilde F,\tilde G)\cong\AA^{\langle \tilde G,\tilde P_{m,j}/\tilde F\rangle}$.
    \item If $\tilde G=\tilde P_{m+1}^I$, the fiber $\Psi^{-1}(\tilde F,\tilde G)$ is not empty if and only if $\Phi(\tilde F)\neq(\tilde F,0)$.
      In this case, we have $\Psi^{-1}(\tilde F,\tilde G)\cong\AA^{\langle \tilde G,\tilde P_{m,j}/\tilde F\rangle}$.
  \end{enumerate}
\end{proposition}
\begin{remark}
...
\end{remark}
\begin{proof}
  By Lemma~\ref{le:subrep}, any subrepresentation $\tilde G\subsetneq\tilde P_{m+1}^I$ is preprojective.
  But the representation $\tilde P_{m,j}/\tilde F$ is not preprojective as it is a proper quotient of a preprojective representation unless $\tilde F=0$.
  Thus we have $\Ext(\tilde G,\tilde P_{m,j}/\tilde F)=0$. 
  If $\tilde F=0$, we have $\Ext(\tilde G,\tilde P_{m,j})=0$ because for dimension reasons every indecomposable direct summand of $\tilde G$ is isomorphic to a lift of some $P_l$ with $l\leq m$ and, moreover, $\Ext(P_l,P_m)=0$ for $l\leq m+1$.
...
\begin{proof}
  ...
  By Proposition~\ref{quotient}, the image of $\Psi_{\tbff,\tbfg}$ consist precisely of those pairs $(\tilde F,\tilde P_{m+1}^I)$ for which $\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)=0$.
  But the map $\tilde F\mapsto\dim\Ext(\tilde P_{m+1}^I,\tilde P_{m,j}/\tilde F)$ is upper semicontinuous so that its minimal value on $\Gr_\tbff(\tilde P_{m,j})$ is its generic value, i.e.\ the image of $\Psi_{\tbff,\tbfg}$ is open.
\qed\end{proof}

\begin{theorem}
  \label{cellscover}
  For each $m\geq 1$ and $J\subsetneq\{1,\ldots,n\}$, every quiver Grassmannian $\Gr_\tbfe(\tilde P_{m+1}^J)$ admits a cell decomposition.
  These cell decompositions can be chosen compatible with the projection maps $\tilde\pi_{m+1}^{\ui,I,J}:\tilde P_{m+1}^J\to\tilde P_{m+1-k,\ui}^I$ from Lemma~\ref{le:truncated quotients} in the following sense:
  \begin{itemize}
    \item[($\dagger$)] for any $\tilde E\in\Gr_\tbfe(\tilde P_{m+1}^J)$ such that $\tilde\pi_{m+1}^{\ui,I,J}(\tilde E)\neq 0$ for some admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, with $i_1\notin J$ and subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$, there exists a cell decomposition of $\Gr_\tbfe(\tilde P_{m+1}^J)$ so that $\tilde\pi_{m+1}^{\ui,I,J}(\tilde E')\neq 0$ for all $\tilde E'\in C_{\tilde E}$, where $C_{\tilde E}$ is the affine cell which contains $\tilde E$.
  \end{itemize}
\end{theorem}
We record the following immediate consequence before giving the proof.
\begin{corollary}
  Every quiver Grassmannian of any lift of a (truncated) preprojective representation admits a cell decomposition.
\end{corollary}
\begin{proof}
  For the standard lifts $\tilde P_{m+1}^J$, this is Theorem~\ref{cellscover}.
  The claim for any translate $\tilde P_{m+1,w}^J$ for $w\in W_n$ is an immediate consequence of this.
\qed\end{proof}
\subsubsection*{Proof of Theorem~\ref{cellscover}}
  ...
  For $m\ge2$, we work by reverse induction on $|J|$ with the base case $|J|=n-1$, say $J^c=\{j\}$.
  This gives $\tilde P_{m+1}^J\cong\tilde P_{m,j}^{\{j\}}$ by Lemma~\ref{le:truncated quotients}.1 and thus the projection maps $\tilde\pi_{m+1}^{\ui,I,J}$ and $\tilde\pi_{m,j}^{\ui'_1,I,\{j\}}$, where $\ui'_1=(i_2,\ldots,i_k)$, coincide for any admissible sequence $\ui\in A_1^{(k)}$, $k<m$, with $i_1=j$ and any subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$.
  By the induction on $m$, the quiver Grassmannian $\Gr_\tbfe(\tilde P_m^{\{j\}})$ admits a cell decomposition so that the compatibility condition~($\dagger$) holds for the map $\tilde\pi_m^{\ui'_1,I,\{j\}}:\tilde P_m^{\{j\}}\to\tilde P_{m+1-k,\ui'_1}^I$.
  But~$\Gr_\tbfe(\tilde P_{m,j}^{\{j\}})$ is naturally isomorphic to $\Gr_\tbfe(\tilde P_m^{\{j\}})$ using the action of $W_n$, under this correspondence the cell decomposition becomes compatible with the projection map $\tilde\pi_{m,j}^{\ui'_1,I,\{j\}}:\tilde P_{m,j}^{\{j\}}\to\tilde P_{m+1-k,\ui}^I$.
  Then using the equivalence $\Gr_\tbfe(\tilde P_{m+1}^J)\cong\Gr_\tbfe(\tilde P_{m,j}^{\{j\}})$ coming from Lemma~\ref{le:truncated quotients}.1, we obtain the desired cell decomposition of $\Gr_\tbfe(\tilde P_{m+1}^J)$ which is compatible with the projection map $\tilde\pi_{m+1}^{\ui,I,J}$.

  Now suppose $|J|\le n-2$ and consider an admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, with $i_1\notin J$ and a subset $I'\subsetneq\{1,\ldots,n\}$ with $i_k\in I'$.
  Taking $j\notin J$ with $j\ne i_1$, we write $I=J\cup\{j\}$.
  ...
  By way of induction, we assume all quiver Grassmannians $\Gr_\tbff(\tilde P_{m,j})$ and $\Gr_\tbfg(\tilde P_{m+1}^I)$ admit cell decompositions satisfying ($\dagger$) for the maps $\tilde\pi_{m,j}^{j,I',\varnothing}$ and $\tilde\pi_{m+1}^{\ui,I',I}$ respectively, say
  \begin{equation}
    \label{eq:cellB}
    \Gr_\tbff(\tilde P_{m,j})=\coprod_{k=1}^r B_{\tbff,k}
  \end{equation}
  and
  \[\Gr_\tbfg(\tilde P_{m+1}^I)=\coprod_{\ell=1}^s C_{\tbfg,\ell}.\]
  Proposition~\ref{fibers} shows that the fiber of $\Psi_{\tbff,\tbfg}$ over $(\tilde F,\tilde G)\in B_{\tbff,k}\times C_{\tbfg,\ell}$ is empty exactly when $\tbfg=\udim\tilde P_{m+1}^I$ and one of the following conditions is satisfied:
  \begin{itemize}
    \item $m=2$ with $\tbff_{(2,\alpha_j^{-1})}\ne0$ or $\tbff_{(1,e)}\ne0$;
    \item $m\ge3$ with $\tilde\pi_{m,j}^{j,I,\varnothing}(\tilde F)=0$.
  \end{itemize}
  By the compatibility with $\tilde\pi_{m,j}^{j,I,\varnothing}$ for the cell decomposition \eqref{eq:cellB}, we see that either all or none of the fibers over $B_{\tbff,k}\times C_{\tbfg,\ell}$ are empty.
  It follows that there is an induced cell decomposition on the image of $\Psi_{\tbff,\tbfg}$ given by those products of cells $B_{\tbff,k}\times C_{\tbfg,\ell}$ over which the fiber is not empty.
  Theorem \ref{vb} then provides a cell decomposition for $\cG_{\tbff,\tbfg}$, where cells take the form $\AA^d \times B_{\tbff,k} \times C_{\tbfg,\ell}$ with~$d=\langle\tbfg,\udim\tilde P_{m,j}-\tbff\rangle$.
  By Lemma~\ref{le:good partition}, we may order the affine cells in the $\cG_{\tbff,\tbfg}$ appropriately to obtain a cell decomposition of $\Gr_{\tbfe}(\tilde P_{m+1}^J)$ (c.f. Remark~\ref{rem:cell decompositions}).

  Now consider $\tilde E\in\Gr_\tbfe(\tilde P_{m+1}^J)$ which is contained in such a cell, say $\tilde E\in \AA^d \times B_{\tbff,k} \times C_{\tbfg,\ell}$, and assume $\tilde\pi_{m+1}^{\ui,I',J}(\tilde E)\neq 0$.
  Since $i_1\ne j$, the map $\tilde\pi_{m+1}^{\ui,I',J}$ factors as $\tilde\pi_{m+1}^{\ui,I',I}\circ\tilde\pi_{m+1}^{I,J}$ by Lemma~\ref{le:truncated quotients}.4.
  By construction, we have $\tilde\pi_{m+1}^{I,J}(\tilde E')\in C_{\tbfg,\ell}$ for all~$\tilde E'\in \AA^d \times B_{\tbff,k} \times C_{\tbfg,\ell}$.
  But $\tilde\pi_{m+1}^{\ui,I',I}\big(\tilde\pi_{m+1}^{I,J}(\tilde E)\big)=\tilde\pi_{m+1}^{\ui,I',J}(\tilde E)\ne0$ implies $\tilde\pi_{m+1}^{\ui,I',I}(\tilde G)\ne0$ for all~$\tilde G\in C_{\tbfg,\ell}$, in particular $\tilde\pi_{m+1}^{\ui,I',J}(\tilde E')=\tilde\pi_{m+1}^{\ui,I',I}\big(\tilde\pi_{m+1}^{I,J}(\tilde E')\big)\neq 0$ for all $\tilde E'\in \AA^d \times B_{\tbff,k} \times C_{\tbfg,\ell}$.
  Thus the compatibility condition ($\dagger$) with $\tilde\pi_{m+1}^{\ui,I',J}$ holds for this cell decomposition of $\Gr_\tbfe(\tilde P_{m+1}^J)$.
\qed
\bigskip
...
\begin{theorem}
  ...
  The following hold:
  \begin{enumerate}
    \item Every quiver Grassmannian of an indecomposable preprojective or preinjective representation of~$K(n)$ has a cell decomposition.
    \item Let $X\in\rep K(n)$ be an indecomposable representation with dimension vector $(d,e)$ or $(e,d)$, where $(d,e)=\udim P_{m+1}-r\udim P_m$ for $m\geq 1$ and $1\leq r\leq n-1$.
      Then every quiver Grassmannian $\Gr_\bfe(X)$ has a cell decomposition.
  \end{enumerate}
\end{theorem}
\begin{proof}
  We consider a representation $X$ of $K(n)$ which is either an indecomposable preprojective or truncated preprojective (c.f. Proposition~\ref{pro:indecomposables}.2), the results for preinjectives or the duals of truncated preprojectives follow via the isomorphisms 
  \[\Gr_{\bfe}(P_m)\cong \Gr_{\udim P_m-\bfe}(Q_m) \qquad \text{and} \qquad \Gr_{\bfe}(P_{m+1}^V)\cong \Gr_{\udim P_{m+1}^V-\bfe}\big((P_{m+1}^V)^*\big),\]
  where $(-)^*=\Hom_\CC(-,\CC)$ is the $\CC$-linear duality.

  By Theorem~\ref{thm:smooth rigid}, each quiver Grassmannian $\Gr_\bfe(P_m)$ is smooth.
  Theorem~\ref{thm:truncpp} shows each $\Gr_{\bfe}(P_{m+1}^V)$ is smooth and that, after an isomorphism, we may assume $V$ is a coordinate subspace under the decomposition in Lemma~\ref{le:basic homological properties}.
  In particular, we can assume $P_{m+1}^V$ admits a lift to the universal covering quiver $\widetilde{K(n)}$.
  This is true in the preprojective case by Lemma~\ref{le:lifts of transjectives}.

  It follows that $X$ lifts to each iterated abelian cover $\widehat{K(n)}^{(k)}$.
  As in Corollary~\ref{cor:truncated lift}, we see that each such lift is preprojective and thus the lifted representations all have smooth quiver Grassmannians.

  Using Corollary~\ref{cor:iterated fixed points} and the associated iteration of Theorem~\ref{thm:bb}, we reduce the problem of constructing cell decompositions for the quiver Grassmannians of $X$ to constructing cells decompositions for the quiver Grassmannians of the lift to $\widetilde{K(n)}$.
  The claim then follows from Theorem~\ref{cellscover}.
\qed\end{proof}
...
\begin{proof}
  ...
  \[X=\bigoplus_{i=1}^rP_{j_i}\oplus\bigoplus_{i=1}^sQ_{k_i},\]
  where we assume that $j_i\leq j_{i+1}$ and write
  \[P(r'):=\bigoplus_{i=1}^{r'} P_{j_i},\quad I(s'):=\bigoplus_{i=1}^{s'}Q_{k_i}\]
  for $r'\leq r$ and $s'\leq s$. 

  By Theorem~\ref{celldec}, the claim is true for all quiver Grassmannians attached to $P_{j_i}$ or $Q_{k_i}$.
  ...
  \[\ses{I(s)}{X}{P(r)}.\]
  As every quotient of $I(s)$ is preinjective and as every subrepresentation of $P(r')$ is preprojective, the same argument as above shows that every quiver Grassmannian attached to $X$ has a cell decomposition.
\qed\end{proof}
\begin{remark}
  The interested reader may utilize the iterative construction of the cell decomposition above to describe the cells using strong successor closed subsets of an appropriate 2-quiver as we do for the truncated preprojectives in the next section.
\end{remark}

As the $\cF$-polynomials of truncated preprojective representations only depend on the dimension vector, we may denote them by $\cF_{d(m,r)}$.
...
\begin{corollary}
  ...
  \[\cF_{d(m,r)}=\cF_{d(m,r+1)}\cF_{\udim P_m}-x^{d(m,r)}\cF_{d(m-2,r)}.\]
\end{corollary}
...
\begin{remark} 
  ...
  \end{align}
  The image of a nonzero map $\tilde P_{m,i}\to\tau\tilde P_{m+1}^{[r]}$ is the representation $\tilde C_m$ from the appropriate sequence \eqref{eq:tau sequence}.
  ...
  \[\Ext\big(\tilde P_{m+1}^{[r]},\tilde P_{m,i}\big)\cong\Ext\big(\tilde P_{m+1}^{[r]},\tilde C_m\big).\]
...
Indeed, the compatible pairs are shown to compute the counting polynomials of the quiver Grassmannians $\Gr_\bfe(P_{m+1})$ over a finite field (these coincide with their Poincar\'e polynomials in this case).  
...
\bibitem{haupt} N.~Haupt: Euler Characteristics of Quiver Grassmannians and Ringel-Hall Algebras of String Algebras. Algebras and Representation Theory \text{15}, 755-793 (2012). 
