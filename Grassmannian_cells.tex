...
If each component $C_i$ admits a cell decomposition, Theorem~\ref{thm:bb} implies the same is true of $X$. Indeed, we can trivialize each affine bundle $\Att(C_i)\to C_i$ over each affine piece of $C_i$ and then taking the natural ordering of the resulting affine spaces gives a cell decomposition of $X$.
...
For $\cG_{\tbff,\tbfg}:=\Psi^{-1}_\tbfe\big(\Gr_\tbff(M)\times \Gr_\tbfg(N)\big)$, we have $$\Gr_\tbfe(B)=\bigsqcup_{\tbff+\tbfg=\tbfe} \cG_{\tbff,\tbfg}.$$
Then $\Psi_\tbfe$ restricts to a map
\[\Psi_\tbfe^{\tbff,\tbfg}:\cG_{\tbff,\tbfg}\to \Gr_\tbff(M)\times \Gr_\tbfg(N).\]
The following is proved in \cite[Section 3]{cefr}.
\begin{theorem}
  \label{vb}
  If $\mathrm{Im}\big(\Psi_\tbfe^{\tbff,\tbfg}\big)$ is locally closed and the fiber dimension of $\Psi_\tbfe^{\tbff,\tbfg}$ is constant over $\mathrm{Im}\big(\Psi_\tbfe^{\tbff,\tbfg}\big)$, then $\Psi_\tbfe^{\tbff,\tbfg}:\cG_{\tbff,\tbfg}\to \mathrm{Im}\big(\Psi_\tbfe^{\tbff,\tbfg}\big)$ is an affine bundle.
...
\begin{corollary}
  The image of $\Psi_{m+1,I,j}$ is open for all $\tbfe$, $\tbff$, and $\tbfg$.
\end{corollary}
...
\begin{proof}
  ...
  \sayD{When $\Psi_{m+1,I,j}$ is surjective, $\cG_{\tbff,\tbfg}$ is a closed subset of the Grassmannian.
  The map is not surjective exactly when $\tbfg=\udim \tilde P_{m+1}^I$ and so we get a cell decomposition by taking the affine cells from this $\cG_{\tbff,\tbfg}$ last.}
