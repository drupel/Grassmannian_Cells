...
\begin{example}
  ...
  Now $\tilde P_{3,i}$ can be found as the representation corresponding to the subquiver
  \[\xymatrix@R40pt@C10pt{
    &&&1\ar_{\alpha_1}[llld]\ar_(.65){\alpha_{i-1}}[ld]\ar^{\alpha_i}[d]\ar^(.65){\alpha_{i+1}}[rd]\ar^{\alpha_n}[rrrd]&&&\\
    1&\hdots&1&n-1&1&\hdots&1\\
  ...
  where the top vertex is $(2,e)$ and the central vertex is $(1,\alpha_i)$.
\end{example}
...
For $l\ge1$, set
\[A_1^{(l)}:=\{(i_1,\ldots,i_l)\mid i_j\in\{1,\ldots,n\}\text{ for $1\le j\le l$}\}.\]%, i_j\ne i_{j+1}\text{ for $1\le j<l$}\}.\]
