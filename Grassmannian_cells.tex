...
It turns out that all $2$-quivers can be built from the $2$-quivers of the lifts $\tilde P_1$, $\tilde P_2$ and $\tilde P_2^{[n-1]}:=\tilde P_2^{(1,\ldots,n-1)}$. Using the conventions of Section \ref{Lifting} the representation $\tilde P_2^{[n-1]}$ is defined by
\[
  \tilde P_2^{[n-1]}=(1,\alpha_1)\xlongleftarrow{\alpha_1}(2,1).%=P_1^{(1)}=j^{(1)}_1\xlongleftarrow{v_1}i^{(1)}_1
\]
We now recursively consider the lifted truncated preprojectives $\tilde P_m^{[k]}:=\tilde P_m^{(1,\ldots,k)}$ for $m\geq 2$ and $0\leq k\leq n-1$. We define $P_m^{[k]}:=F_{K(n)}\tilde P_m^{[k]}$.
...
\draw (0,0) node (A) {$(1,\alpha_{n-k})$} +(3,0) node (B) {$(1,\alpha_{n-k+1})$}+(3,-.8) node (P1) {\footnotesize$P_2^{[k+1]}$} +(6,0) node (B1) {$\dots$} +(9,0) node (C) {$(1,\alpha_{2})$} +(9,-.7) node (P3) {\footnotesize$P_2^{[n-2]}$}+(11.5,0) node (D) {$(1,\alpha_1)$}  +(14,0) node (E) {$(2,1)$} +(11.5,-.6) node (P4) {\footnotesize$P_2^{[n-1]}$}; 
...
The next step is to define the $2$-quivers of certain truncated preprojective lifts $\tilde P^{[k]}_3$ (resp. of $P^{[k]}_3$) whence the remaining $2$-quivers attached to the truncated preprojectives $P_m^{[k]}$ can be obtained recursively. 
...
for $i=1,\ldots,n-1$. 
 ...
 This basis recursively gives rise to the following $2$-quiver of $\tilde P_3^{[k]}=\tilde P_3^{(1,\ldots,k)}$ (resp. for $P_3^{[k]}$) for $k=n-1,\ldots,0$ where we drop the indices of the vertices in abuse of notation. Note that the middle vertex of $\tilde P_3^{[n-1]}$ is $(2,\alpha_n^{-1})$ and that all $2$-arrows end in the vertex $(1,1)$ of $\widetilde{K(n)}$.\\
