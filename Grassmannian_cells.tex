  ...
  \begin{enumerate}
    ...
    \item For any admissible word $\ui\in A_1^{(k)}$ with $0\le k< m$ and any proper subsets $I,J\subsetneq\{1,\ldots,n\}$ with~$i_k\in I$ and $i_1\notin J$ if $k\ge1$ or with $J\subsetneq I$ if $k=0$, there exist (unique up to scale) surjective maps~$\tilde\pi_{m+1}^{\ui,I,J}:\tilde P_{m+1}^J\to \tilde P_{m+1-k,\ui}^I$.
      ...
      \begin{itemize}
        ...
        \item $\tilde\pi_{m+1}^{\ui,I,J}\Big|_{\tilde P_{m+1-\ell,\ui_\ell}}=\tilde\pi_{m+1-\ell,\ui_\ell}^{\ui'_\ell,I,\varnothing}$ for $0\le \ell\le k$, where $\ui_\ell:=(i_1,\ldots,i_\ell)$ and $\ui'_\ell:=(i_{\ell+1},\ldots,i_k)$.
      \end{itemize}
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  \[\tilde\pi_{m+1}^{\ui,I,J}:=\tilde\pi_{m+1-k,\ui}^{I,\{i_k\}}\circ\tilde\pi_{m+2-k,\ui_{k-1}}^{\{i_k\}^c,\{i_{k-1}\}}\circ\cdots\circ\tilde\pi_{m,i_0}^{\{i_2\}^c,\{i_1\}}\circ\tilde\pi_{m+1}^{\{i_1\}^c,J}.\]
...
\begin{proof}
  ...
  From part (1), we see that the fiber $\Psi^{-1}(F,\tilde P_{m+1}^I)$ is non-empty in this case.
\qed\end{proof}
...
\begin{theorem}
  ...
  \begin{itemize}
    \item[($\dagger$)] for any $E\in\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$ such that $\tilde\pi_{m+1}^{\ui,I,J}(E)\neq 0$ for some admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, with $i_1\notin J$ and subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$, we have $\tilde\pi_{m+1}^{\ui,I,J}(E')\neq 0$ for all $E'\in C_E$, where $C_E$ is the affine cell which contains $E$.
  \end{itemize}
\end{theorem}
\begin{corollary}
...
\subsubsection*{Proof of Theorem~\ref{cellscover}}
  ...
  This gives $\tilde P_{m+1}^J\cong\tilde P_{m,j}^{\{j\}}$ by Lemma~\ref{le:truncated quotients}.1 and thus the projection maps $\tilde\pi_{m+1}^{\ui,I,J}$ and $\tilde\pi_{m,j}^{\ui,I,\{j\}}$ coincide for any admissible sequence $\ui\in A_1^{(k)}$, $k<m$, with $i_1=j$ and any subset $I\subsetneq\{1,\ldots,n\}$ with $i_k\in I$.
  By the induction on $m$, the quiver Grassmannian $\Gr^{\tilde Q}_\tbfe(\tilde P_m^{\{j\}})$ admits a cell decomposition so that the compatibility condition~($\dagger$) holds for each of the maps $\tilde\pi_m^{\ui'_1,I,\{j\}}:\tilde P_m^{\{j\}}\to\tilde P_{m+1-k,\ui'_1}^I$, where $\ui'_1=(i_2,\ldots,i_k)$ with $i_2\ne j$.
  But~$\Gr^{\tilde Q}_\tbfe(\tilde P_{m,j}^{\{j\}})$ is naturally isomorphic to $\Gr^{\tilde Q}_\tbfe(\tilde P_m^{\{j\}})$ using the action of $W_n$, under this correspondence the cell decomposition becomes compatible with the projection maps $\tilde\pi_{m,j}^{\ui,I,\{j\}}:\tilde P_{m,j}^{\{j\}}\to\tilde P_{m+1-k,\ui}^I$.
  Then using the equivalence $\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)\cong\Gr^{\tilde Q}_\tbfe(\tilde P_{m,j}^{\{j\}})$ coming from Lemma~\ref{le:truncated quotients}.1, we obtain the desired cell decomposition of $\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$.

  Consider $J\subsetneq I\subsetneq \{1,\ldots,n\}$ with $I\setminus J=\{j\}$.
  ...
  \begin{equation}
    \label{eq:cellB}
    \Gr^{\tilde Q}_\tbff(\tilde P_{m,j})=\coprod_{k=1}^r B_{\tbff,k}
  \end{equation}
  and
  \[\Gr^{\tilde Q}_\tbfg(\tilde P_{m+1}^I)=\coprod_{\ell=1}^s C_{\tbfg,\ell}.\]
  Proposition~\ref{fibers} shows that the fiber of $\Psi_{\tbff,\tbfg}$ over $(F,G)\in B_{\tbff,k}\times C_{\tbfg,\ell}$ is empty exactly when $\tbfg=\udim\tilde P_{m+1}^I$ and one of the following conditions is satisfied
  \begin{itemize}
    \item $m=2$ with $\tbff_{(2,\alpha_j^{-1})}\ne0$ or $\tbff_{(1,e)}\ne0$;
    \item $m\ge3$ with $\tilde\pi_{m,j}^{j,I,\varnothing}(F)=0$.
  \end{itemize}
  ...
  Theorem \ref{vb} then provides a cell decomposition for $\cG_{\tbff,\tbfg}$, where cells take the form $B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$ with~$d=\langle\tbfg,\udim\tilde P_{m,j}-\tbff\rangle$.
  By Lemma~\ref{le:good partition}, we may order the affine cells in the $\cG_{\tbff,\tbfg}$ appropriately to obtain a cell decomposition of $\Gr_{\tbfe}^{\tilde Q}(\tilde P_{m+1}^J)$ (c.f. Remark~\ref{rem:cell decompositions}).

  Now consider $E\in\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$ which is contained in such a cell, say $E\in B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$, and assume $\tilde\pi_{m+1}^{\ui,I',J}(E)\neq 0$ for some admissible sequence $\ui=(i_1,\ldots,i_k)$, $k<m$, with $i_1\notin J$ and some subset $I'\subsetneq\{1,\ldots,n\}$ with $i_k\in I'$.
  ...
  \begin{itemize}
    \item If $i_1\ne j$, the map $\tilde\pi_{m+1}^{\ui,I',J}$ factors as $\tilde\pi_{m+1}^{\ui,I',I}\circ\tilde\pi_{m+1}^{I,J}$ by Lemma~\ref{le:truncated quotients}.4.
      By construction, we have $\tilde\pi_{m+1}^{I,J}(E')\in C_{\tbfg,\ell}$ for all~$E'\in B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$.
      But $\tilde\pi_{m+1}^{\ui,I',I}\big(\tilde\pi_{m+1}^{I,J}(E)\big)=\tilde\pi_{m+1}^{\ui,I',J}(E)\ne0$ implies $\tilde\pi_{m+1}^{\ui,I',I}(G)\ne0$ for all~$G\in C_{\tbfg,\ell}$, in particular $\tilde\pi_{m+1}^{\ui,I',J}(E')=\tilde\pi_{m+1}^{\ui,I',I}\big(\tilde\pi_{m+1}^{I,J}(E')\big)\neq 0$ for all $E'\in B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$.
    \item If $i_1=j$, the restricted map $\tilde\pi_{m+1}^{\ui,I',J}\Big|_{\tilde P_{m,j}}$ coincides with $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}$ by Lemma~\ref{le:truncated quotients}.4.
      By construction, we have $E'\cap P_{m,j}\in B_{\tbff,k}$ for all ~$E'\in B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$.
      But $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(E\cap\tilde P_{m,j})=\tilde\pi_{m+1}^{\ui,I',J}(E)\ne0$ implies $\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(F)\ne0$ for all~$F\in B_{\tbff,k}$, in particular $\tilde\pi_{m+1}^{\ui,I',J}(E')=\tilde\pi_{m,j}^{\ui'_1,I',\varnothing}(E'\cap \tilde P_{m,j})\neq 0$ for all $E'\in B_{\tbff,k}\times C_{\tbfg,\ell}\times\AA^d$.
  \end{itemize}
  Thus the compatibility condition ($\dagger$) holds for the cell decomposition of $\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^J)$.
\qed
