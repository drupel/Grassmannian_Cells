...
\begin{lemma}
  \label{le:subrep}
  For $m\ge1$ and $I\subsetneq\{1,\ldots,n\}$, all non-trivial proper subrepresentations of $\tilde P_{m+1}^I$ are direct sums of shifted preprojective representations $\tilde P_{m+1-k,\ui}$ for $1\le k\le m$ and $\ui\in A_1^{(k)}$.	
\end{lemma}
\begin{proof}
  \sayD{The proof needs to be improved for the new stronger statement.}
...
\begin{definition}
  ...
  \end{enumerate}
  Here we take $I_{k+1}=\varnothing$.
...
For part (1), we consider the following commutative diagram, where the square on the left hand side is the corresponding Auslander-Reiten sequence:
\[\xymatrix{0\ar[r]&\tau\tilde P_{m+1}=\tilde P_{m-1} \ar[r]\ar[d] & \tilde P_{m,j}\ar[d]\ar[r]& \tilde P_{m+1}^I\ar[r]\ar@{=}[d]& 0\\
  0\ar[r] &\bigoplus_{i\in I}\tilde P_{m,i}\ar[r] &  \tilde P_{m+1}\ar[r] & \tilde P_{m+1}^I\ar[r] & 0}\]
...
The next step is to associate a $2$-quiver $\mathcal Q_m^{[k]}$ with the lifted truncated preprojective representations $\tilde P_m^{[k]}:=\tilde P_m^{\{1,\ldots,k\}}$ for $m\geq 2$ and $0\leq k\leq n-1$  whose strong successor closed subsets are in one-to-one correspondence with cells of quiver Grassmannians of $\tilde P_m^{[k]}$. Recall that the cells of the quiver Grassmannians attached to $P_m^{[k]}:= F_{K(n)}\tilde P_m^{[k]}$ are in one-to-one correspondence with those attached to the lift $\tilde P_m^{[k]}$. Moreover, the quiver Grassmannians $\Gr_{\tbfe}(P_m^V)$ of arbitrary truncated preprojective representations $P_m^V$ only depend on $\tbfe$ and $\udim P_m^V$. In short, in view of Theorem \ref{thm:truncpp}, the $2$-quivers $\mathcal Q_m^{[k]}$ can be seen as being attached to any (truncated) preprojective representation of the respective dimension.	
...
\begin{remark}It will turn out that every vertex of the quiver $\mathcal Q_m^{[k]}$ uniquely corresponds to a vertex of the universal covering quiver $\widetilde{K(n)}$. Also every usual arrow of $\mathcal Q_m^{[k]}$ does. In particular, the quiver obtained from $\mathcal Q_m^{[k]}$ when forgetting the $2$-arrows gives rise to a (non-disjoint) union of subquivers of $\widetilde{K(n)}$.

For general $m$, we will see that every $2$-arrow $V:\Gamma(1)\to\Gamma(2)$ corresponds to a basis element of $\Ext(\tilde P_{m+1}^{[n-1]},\tilde P_m)$. If the subquivers $\Gamma(1)$ and $\Gamma(2)$ have precisely one source and precisely one sink respectively, the $2$-arrow corresponds to a unique arrow of the original quiver $K(n)$ as every arrow of the universal covering does. In these cases, we label the $2$-arrow by the corresponding arrow.

Recall that, for $m\geq 3$, we have
...
These isomorphisms are also present in the structure of the $2$-quivers we define.
\end{remark}


%It turns out that all $2$-quivers can be built from the $2$-quivers of $\tilde P_1$, $\tilde P_2$ and $\tilde P_2^{[n-1]}$. 
The $2$-quiver $\mathcal Q_1=\cQ_1^{[0]}$ of $\tilde P_1$ is given by the vertex $(1,e)$. Moreover, we define the translated $2$-quivers $\cQ_{1,i}$ by the vertices $(1,\alpha_i)$. Recall that the representation $\tilde P_2^{[n-1]}$ is defined by the support quiver
\[
...
We can recursively build the (truncated) preprojective representation $\tilde P_{2}^{[k]}$ for $k=n-1,\ldots,0$ defined by the support quivers
\begin{align}
  \label{coeff}\xymatrix@R10pt@C20pt{&&(1,\alpha_1)\\ (2,e)\ar^{\alpha_1}[rru]\ar^{\alpha_{n-k}}[rrd] &&\vdots\\&&(1,\alpha_{n-k})}.
\end{align}
...
i.e. we translate all vertices by $\alpha_n^{-1}$. Now $\cQ_3^{[k]}$ can be found as a subquiver of $\cQ_{3}$ which is given by connecting $\cQ_3^{[n-1]}$ to $\cQ_{2,i}$ for $i=n-1,\ldots, 1$ in the following way: \\
...
\noindent In abuse of notation, we dropped the labels of the vertices in the illustration. Note that the middle vertex of $\mathcal Q_3^{[n-1]}$ corresponds to $(2,\alpha_n^{-1})$ and that all $2$-arrows, which are again coloured red, end in the vertex corresponding to $(1,1)$ of $\widetilde{K(n)}$. We again define translated versions $\cQ_{3,i}:=\alpha_i.\cQ_3$ of $\cQ_3$.
...
Note that the image of the homomorphism corresponding to the basis element spanning $\Ext(\tilde P_3^{[n-1]},\alpha_i^{-1}\tilde P_2)$ corresponds to the subquiver $(2,\alpha_i^{-1})\xrightarrow{\alpha_i} (1,1)$ for $i=k+1,\ldots, n$. Thus in this case the $2$-arrow connects the $2$-quiver of $\tilde P_3^{[k]}$ to the image of the corresponding homomorphism $\tilde P_{2,k}\to \tau \tilde P_3^{[k]}$ which is of dimension $(1,1)$. In view of Lemma \ref{lem:2equivalence}, we forgo to insert the corresponding rectangles around the subquiver and just connected the $2$-arrows to the sinks of the corresponding subquivers of dimension $(1,1)$. In summary, the chosen basis of $\Ext(P_3^{[n-1]},P_2)$ motivated the definition of the $2$-quiver $\mathcal Q_3=\mathcal Q_3^{[0]}$ of $\tilde P_3$ and the $2$-quivers $\mathcal Q_3^{[k]}$ for $k=n-1,\ldots,1$.
\end{remark}
...
%\end{align}
Now we can recursively build the $2$-quivers $\cQ_{m+1}^{[k]}$ of $ \tilde P_{m+1}^{[k]}$ for $m\geq 3$ and $k=n-1,\ldots,0$ when repeating the procedure. 
...
Let us assume that we already constructed the $2$-quivers $\cQ_{l}^{[j]}$ of $ P_{l}^{[j]}$ for $l\leq m$ and $j=0,\ldots, n-1$ and $l=m+1$ and $j=n-1,\ldots,k+1$. As before, we define the translated $2$-quivers $\cQ_{m,i}=\alpha_i^{(-1)^{m+1}}.\cQ_m$.

For $m\geq 4$, the truncated preprojective $\tau\tilde P_{m+1}^{[k]}\cong\tilde P_{m-1}^{[k]}$ can uniquely be found as a quotient of $\tilde P_{m,k}$. This can also be seen on the level of $2$-quivers, i.e. we find $\cQ_{m-1}^{[k]}$ as a subquiver of $\cQ_{m,k}$.  More precisely, this subquiver can be found at the very right of the $2$-quiver $\mathcal Q_{m,k}$.

Now the $2$-quiver $\mathcal Q_{m+1}^{[k-1]}$ of $\tilde P^{[k-1]}_{m+1}$ is given by glueing the $2$-quiver $\mathcal Q_{m+1}^{[k]}$ to $\mathcal Q_{m,k}$ by a $2$-arrow. Thereby, the $2$-arrow connects $\mathcal Q_{m+1}^{[k]}$ to the subquiver $\cQ_{m-1}^{[k]}$ in $\cQ_{m,k}$. This gives the following $2$-quiver $\mathcal Q_{m+1}^{[k-1]}$:
\\
...
\draw (0,0) node (B) {$\mathcal Q_{m,k+1}$}+(0,-.8) node (P1) {\footnotesize$\mathcal Q_{m+1}^{[k]}$} +(2.5,0) node (B1) {$\dots$} +(5,0) node (C) {$\mathcal Q_{m,n-1}$} +(5,-.7) node (P3) {\footnotesize$\mathcal Q_{m+1}^{[n-2]}$}+(7.5,0) node (D) {$\mathcal Q_{m,n}$}  +(10,0) node (E) {$\mathcal Q_{m+1}^{[n-1]}=\mathcal Q_{m}^{[1]}$} +(7.5,-.6) node (P4) {\footnotesize$\mathcal Q_{m+1}^{[n-1]}$}; 
\node (Q1) [draw=black,fit={(D) (E) (P4)}]{};
\node (Q2) [draw=black,fit={(Q1) (C) (P3)}]{};
\node (Q2b)[draw=black,fit={(Q2) (B1) }]{};
...
%In order to keep the diagrams as simple as possible we did not insert the labels of the vertices. As every vertex  $q\in(\mathcal Q_m^{[k]})_0$ corresponds a vertex $\tilde q\in \widetilde{K(n)_0}$, in any case it is straightforward to associate an arrow $\alpha_i\in K(n)_1$ with every ($2$-)arrow of $\mathcal Q_m^{[k]}$. 
 Note that two different vertices of $\mathcal Q_m^{[k]}$ can correspond to the same vertex of $\widetilde{K(n)}$. Writing dimension vectors $\tbfe\in\NN^{\widetilde{K(n)_0}}$ as $\tbfe=\sum_{q\in \widetilde{K(n)_0}}\tbfe_q\cdot q$, the dimension types $\tbfe (\beta)$ and  $\bfe (\beta)$ of a subset $\beta\subset (\mathcal Q_m^{[k]})_0$ are defined by
