...
\begin{definition}
  ...
  A sequence of subsets $\bfI=(I_0,I_1,\ldots,I_k)$, $k\ge1$, in $\{1,\ldots,n\}$ is \emph{admissible} if the following hold:
  \begin{enumerate}
    \item $|I_0|=n-1$ and $|I_l|=n-2$ for $1\le l\le k-1$;
    \item the sets $I'_l$ defined recursively by $I'_0=I_0$ and $I'_{l+1}:=(I'_l)^c\cup I_{l+1}$ for $0\le l\le k$ satisfy $(I'_l)^c\cap I_{l+1}=\varnothing$ for $0\le l\le k$.
  \end{enumerate}
  Here we take $I_{k+1}=\varnothing$ so that there is no condition imposed on the set $I_k$.

  Given an admissible sequence $\bfI$, define a new admissible sequence $\delta\bfI=(I'_1,I_2,\ldots,I_k)$, where we set $\delta(I_0)=(I'_1)$.
\end{definition}
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item Consider an admissible sequence $\bfI=(I_0,\ldots,I_k)$ with $k<m$ and write $(I'_l)^c=\{i_l\}=:J_l$ for $0\le l\le k-1$.
      For $\ui=(i_0,\ldots,i_{k-1})$, there exists a commutative diagram:
      \begin{equation}
        \label{dia:truncated coherence}
        \xymatrix@R40pt@C70pt{\tilde P_{m+1} \ar^(.4){\pi_{m+1}^{I_0}}[d] & \ar[l] \tilde P_{m,i_0} \ar_(.4){\pi_{m,i_0}^{J_0}}[dl]\ar^(.4){\pi_{m,i_0}^{I'_1}}[d] & \ar[l] \tilde P_{m-1,(i_0,i_1)} \ar_(.4){\pi_{m-1,(i_0,i_1)}^{J_1}\;}[dl]\ar^(.4){\pi_{m-1,(i_0,i_1)}^{I'_2}}[d] & \ar[l] \ar[dl] \cdots & \ar[l] \tilde P_{m+1-k,\ui} \ar_(.4){\pi_{m+1-k,\ui}^{J_{k-1}}\;}[dl]\ar^(.4){\pi_{m+1-k}^{I'_k}}[d]\\
...
Indeed, for this such an action must satisfy $X_\alpha(t.U_i)\subset t.U_j$ for every arrow $\alpha:i\to j$ of $Q$. 
\begin{lemma}
...
\end{lemma}
\begin{proof} 
  Since $X$ can be lifted, for any arrow $\alpha\in Q_1$ we can write $X_\alpha:X_i\to X_j$ as a block matrix consisting of linear maps $X_{(\alpha,\chi)}:X_{(i,\chi)}\to X_{(j,\chi+e_\alpha)}$ for $\chi\in A_Q$.
  Fix $U\in\Gr_\bfe(X)$ and consider $u_i\in U_i$.
  Then we can write $u_i=\sum_{\chi\in A_Q} u_{(i,\chi)}$ for some vectors $u_{(i,\chi)}\in U_i\cap X_{(i,\chi)}$.
  But observe that for an arrow $\alpha:i\to j$ of $Q$, we have $X_\alpha(u_{(i,\chi)})=X_{(\alpha,\chi)}(u_{(i,\chi)})\in U_j\cap X_{(j,\chi+e_\alpha)}$, say $X_{(\alpha,\chi)}(u_{(i,\chi)})=u_{(j,\chi+e_\alpha)}$.
  It follows that
  \begin{align*}
    X_\alpha(t.u_i)=\sum\limits_{\chi\in A_Q}X_{(\alpha,\chi)}(t.u_{(i,\chi)})=\sum\limits_{\chi\in A_Q} t^{d(i,\chi)}X_{(\alpha,\chi)}(u_{(i,\chi)})=\sum\limits_{\chi\in A_Q} t^{d(i,\chi)}u_{(j,\chi+e_\alpha)}=t^{-c_\alpha}\sum\limits_{\chi\in A_Q} t.u_{(j,\chi+e_\alpha)}
  \end{align*}
  and thus $X_\alpha(t.U_i)\subset t.U_j$ for every arrow $\alpha:i\to j$ of $Q$.
\end{proof}


\begin{lemma}
  \label{degreecondition} 
  Let $X$ be an indecomposable representation of $\hat Q$.
  There exists $d:\mathrm{supp}(X)\to\ZZ$ and $c_\alpha\in\mathbb N_+$ for each $\alpha\in Q_1$ such that
  \begin{enumerate}
    \item for $(i,\chi),\,(i,\chi')\in\mathrm{supp}(X)$ with $\chi\neq\chi'$, we have $d(i,\chi)\ne d(i,\chi')$;
    \item for $(i,\chi),\,(j,\chi')\in\mathrm{supp}(X)$, we have $d(j,\chi')-d(i,\chi)=c_\alpha$ if and only if $\chi'=\chi+e_\alpha$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  For convenience we introduce the notation $Q_1=\{\alpha_1,\ldots,\alpha_\}$.
  ...
  This implies that, for $(i',\chi)\in\mathrm{supp}(X)$ with $\chi=\sum_{l=1}^n \kappa_l e_{\alpha_l}$, we have $|\kappa_l|\leq K$.

  Set $c_{\alpha_1}=1$ and choose $c_{\alpha_l}$ recursively in such way that 
  \[c_{\alpha_l}\geq 2(K+1)\sum_{k=1}^{l-1} c_{\alpha_k}\]
  for $l=2,\ldots,n$.  

  Let $f:A_Q\to\ZZ$ be the group homomorphism defined by $f(e_\alpha)=c_\alpha$ for all $\alpha\in Q_1$ and define $d(i',\chi):=f(\chi)$ for $(i',\chi)\in\mathrm{supp}(X)$.
  In particular, for $(i',\chi)$ with $\chi=\sum_{l=1}^n \kappa_l e_{\alpha_l}$, we have
  \[d(i',\chi)=\sum_{l=1}^n \kappa_l c_{\alpha_l}.\]

  Now assume that $d(i',\chi)=d(i',\chi')$ for $\chi=\sum_{l=1}^n \kappa_l e_{\alpha_l}$ and $\chi'=\sum_{l=1}^n \kappa'_l e_{\alpha_l}$.
  This implies
  ...
  \[|\sum_{l=1}^{n-1}(\kappa_l-\kappa'_l)c_{\alpha_l}|\le 2K\sum_{l=1}^{n-1}c_{\alpha_l} < c_{\alpha_n}.\]
  This inductively yields $\kappa_l=\kappa'_l$ for $l=n,\ldots,1$ by the choice of the $c_{\alpha_l}$. 

  By definition, we have $d(j,\chi+e_\alpha)-d(i,\chi)=c_\alpha$.
  Assume that $d(j,\chi')-d(i,\chi)=c_\alpha$.
...
\begin{proof}
  ...
  Thus, apart from being a subrepresentation of $X$, each component $U_i$ is a fixed point of the torus actions on the usual Grassmannian $\Gr_{\bfe_i}(X_i)$ induced by the decompositions $X_i=\bigoplus_{\chi\in A_Q} X_{(i,\chi)}$.
  By Lemma~\ref{usualGrass}, this holds precisely when we have a decomposition 
...
Following \cite[Section 3]{cc}, any nonempty fiber of $\Psi_\tbfe$ satisfies $\Psi^{-1}_\tbfe(V,W)=\mathbb{A}^{\dim\Hom(W,M/V)}$.
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item The fiber $\Psi_{m+1,I,j}^{-1}(U,\tilde P_{m+1}^I)$ is empty if and only if $U\in\Phi_{m,I,j}^{-1}(V,0)$ for some $V\subset\tilde P_m(I,j)$, i.e.\ $U$ is already a subrepresentation of $\tilde P_m(I,j)$.
  \end{enumerate} 
\end{proposition}
\begin{proof}
  ...
  Thus the injective map $\CC=\Hom(\tilde K_m,\tilde K_m)\to\Ext(\tilde P_{m+1}^I,\tilde K_m)$ induced by the Auslander-Reiten sequence \eqref{eq:ar2} is actually bijective and so applying $\Hom(-,\tilde K_m)$ to this sequence yeilds $\Ext(Z,\tilde K_m)=0$.
...
\begin{proposition}
  ...
  The following hold:
  \begin{enumerate}
    \item For $V\subsetneq\tilde P_{m+1}^I$ and $U\subseteq\tilde P_{m,j}$, we have $\Psi_{m+1,I,j}^{-1}(U,V)=\AA^{\langle V,\tilde P_{m,j}/U\rangle}$.
    \item If $V=\tilde P_{m+1}^I$, the fiber $\Psi_{m+1,I,j}^{-1}(U,V)$ is not empty if and only if $\Phi_{m,I,j}(U)\neq(U,0)$.
      In this case, we have $\Psi_{m+1,I,j}^{-1}(U,V)=\AA^{\langle V,\tilde P_{m,j}/U\rangle}$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  ...
  Thus we have $\Ext(V,\tilde P_{m,j}/U)=0$. 
  If $U=0$, we have $\Ext(V,\tilde P_{m,j})=0$ because for dimension reasons every indecomposable direct summand of $V$ is isomorphic to a lift of some $P_l$ with $l\leq m$ and, moreover, $\Ext(P_l,P_k)=0$ for $l\leq k+1$.
...
\begin{theorem}
  \label{cellscover}
  For $m\geq 1$ and $I\subset\{1,\ldots,n\}$, every quiver Grassmannian $\Gr^{\tilde Q}_\tbfe(\tilde P_{m+1}^I)$ admits a cell decomposition into affine spaces.
\end{theorem}
\begin{proof}
...
The next step is to associate a $2$-quiver $\mathcal Q_m^{[k]}$ with the lifted truncated preprojective representations $\tilde P_m^{[k]}:=\tilde P_m^{\{1,\ldots,k\}}$ for $m\geq 2$ and $0\leq k\leq n-1$  whose strong successor closed subsets are in one-to-one correspondence with cells of quiver Grassmannians of $\tilde P_m^{[k]}$. Recall that, by Theorem \ref{celldec}, the cells of the quiver Grassmannians attached to $P_m^{[k]}:= G_{K(n)}\tilde P_m^{[k]}$ are in one-to-one correspondence with those attached to the lift $\tilde P_m^{[k]}$. Moreover, by Theorem \ref{thm:truncpp}, the quiver Grassmannians $\Gr_{\tbfe}(P_m^V)$ of arbitrary truncated preprojective representations $P_m^V$ only depend on $\tbfe$ and $\udim P_m^V$. In short, the $2$-quivers $\mathcal Q_m^{[k]}$ can be seen as being attached to any (truncated) preprojective representation of the respective dimension.	

For building the mentioned $2$-quivers, we use the recursive construction described in Section \ref{Lifting}. 
%In particular, we choose bases of the $\Ext$-groups which are compatible with the lifts to the universal covering.
...
\begin{remark}\begin{enumerate}\item It will turn out that every vertex of the quiver $\mathcal Q_m^{[k]}$ uniquely corresponds to a vertex of the universal covering quiver $\widetilde{K(n)}$, but that different vertices of $\cQ_m^{[k]}$ can correspond to the same vertex of $\widetilde{K(n)}$. Also every usual arrow of $\mathcal Q_m^{[k]}$ does. In particular, the quiver obtained from $\mathcal Q_m^{[k]}$ when forgetting the $2$-arrows gives rise to a (non-disjoint) union of subquivers of $\widetilde{K(n)}$.

\item The following two remarks are meant to understand the structure of the $2$-quivers. Recall that the representation $\tilde P_2^{[n-1]}$ is defined by the support quiver
\[
...
i.e. we translate all vertices - understood as vertices of $\widetilde{K(n)}$ - by $\alpha_n^{-1}$. Now $\cQ_3^{[k]}$ can be found as a subquiver of $\cQ_{3}$ which is given by connecting $\cQ_3^{[n-1]}$ to $\cQ_{2,i}$ for $i=n-1,\ldots, 1$ in the following way: \\
...
\noindent In abuse of notation, we did not label the vertices in the illustration. Note that the middle vertex of $\mathcal Q_3^{[n-1]}$ corresponds to $(2,\alpha_n^{-1})$ and that all $2$-arrows, which are again coloured red, end in a vertex corresponding to $(1,1)$ of $\widetilde{K(n)}$. We again define translated versions $\cQ_{3,i}:=\alpha_i.\cQ_3$ of $\cQ_3$ which again means that we translate $\cQ_{3}$ as a union subquiver contained in $\widetilde{K(n)}$.
...
Let us assume that we already constructed the $2$-quivers $\cQ_{l}^{[j]}$ of $ P_{l}^{[j]}$ for $l\leq m$ and $j=0,\ldots, n-1$ and $l=m+1$ and $j=n-1,\ldots,k$. As before, we define the translated $2$-quivers $\cQ_{m,i}=\alpha_i^{(-1)^{m+1}}.\cQ_m$. Moreover, we define $$\cQ_{m+1}^{[n-1]}:=\alpha_{n}^{(-1)^{m+1}}.\cQ_{m}^{[1]}.$$\sayT{check: is it always $\alpha_n$?}
...
\draw (0,0) node (B) {$\mathcal Q_{m,k+1}$}+(0,-.8) node (P1) {\footnotesize$\mathcal Q_{m+1}^{[k]}$} +(2.5,0) node (B1) {$\dots$} +(5,0) node (C) {$\mathcal Q_{m,n-2}$} +(5,-.7) node (P3) {\footnotesize$\mathcal Q_{m+1}^{[n-3]}$}+(7.5,0) node (D) {$\mathcal Q_{m,n-1}$}  +(11,0) node (E) {$\mathcal Q_{m+1}^{[n-1]}=\alpha_{n}^{(-1)^{m+1}}.\cQ_{m}^{[1]}$} +(7.5,-.6) node (P4) {\footnotesize$\mathcal Q_{m+1}^{[n-2]}$}; 
\node (Q1) [draw=black,fit={(D) (E) (P4)}]{};
\node (Q2) [draw=black,fit={(Q1) (C) (P3)}]{};
\node (Q2b)[draw=black,fit={(Q2) (B1) }]{};
...
where $\tilde q\in\widetilde{K(n)}_0$ is the vertex which corresponds to $q\in \beta$.
...
We proceed by induction on $m$ and $k$. The case of the representation $\tilde P_1$ is trivial. We have $(\tilde P_2^{[k]})_q\in\{0,1\}$ for all $q\in\widetilde{K(n)}$, whence the  subrepresentations are in one-to-one correspondence with the successor closed subsets the quiver (\ref{coeff}) which is equivalent to  $\mathcal Q_2^{[k]}$ . Equivalently, we have $\Gr_{\tbfe}(\tilde P_2^{[k]})\in\{\varnothing,\{\pt\}\}$ such that $\Gr_{\tbfe}(\tilde P_2^{[k]})=\{\pt\}$ if and only if $\tbfe\subset \mathcal Q_2^{[k]}$ is strong successor closed.
 ...
 % Denote by $d(\beta_1+\beta_2)$ the dimension vector corresponding to this subset.
  By Proposition \ref{quotient}, the fibre over the pair of cells corresponding to $\beta_1\coprod\beta_2$ is empty if and only if $\beta_2=(\mathcal Q_{m+1}^{[k-1]})_0$ and $\beta_1\cap(\mathcal Q_{m-1}^{[k]})_0=\varnothing$ where $\mathcal Q_{m-1}^{[k]}$ is considered as a $2$-quiver of $\mathcal Q_{m,k}$.
