...
\begin{lemma}
  For any $m\ge1$, we have $\dim\Hom(P_m,P_m)=1$, $\dim\Hom(P_m,P_{m+1})=n$, and $\dim\Hom(P_{m+1},P_m)=0$.  
...
\begin{proof}
  ...
  To get the isomorphism with $\kk f_1\oplus\cdots\oplus\kk f_k$, we work by induction on $k$.  
  Indeed, $\Hom(P_m^k,P_m)$ is spanned by the coordinate projections $\pi_i:P_m^k\to P_m$ for $1\le i\le k$ and each of these gives rise to a commutative diagram
  ...
  Part (3) follows immediately from the observation that $\langle P_{m+1}^{\{f_1,\ldots,f_k\}},P_m\rangle=-k$.
\end{proof}
