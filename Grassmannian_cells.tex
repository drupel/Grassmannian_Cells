...
\begin{proof}\sayT{Maybe we can shorten the proof. I don't see a  proof which does not use reflection functors at the moment.}
  ...
  \[\xymatrix{0 \ar[r] & P_{m-1}\ar[r]^{\iota_{m-1}} & P_m\otimes H_m \ar[r]^{ev} &P_{m+1} \ar[r] & I_{2-m}\ar[r]&0\\0 \ar[r] & P_{m-1}\ar@{=}[u]\ar[r] &P_{m-1}\oplus P_m\otimes V \ar@{}[ur]|(.7){\urcorner} \ar[r]\ar[u]^{(\iota_{m-1},\mathrm{id}_{P_m}\otimes \iota_V)} &P_{m}\otimes V \ar[r]\ar_{ev_V}[u] & 0 \ar[u]}\]
...
\end{lemma}
\begin{proof}
  We proceed by simultaneous induction on $m$ and $\codim_{H_m} V$.

  When $m=1$, the dimension vector of $P_{m+1}^V$ is $(\codim_{H_m} V,1)$.
  In particular, it is immediate that each proper subrepresentation of $P_{m+1}^V$ is isomorphic to $P_1^k$ for some $0\le k\le l$ which gives both claims in this case.
...
It will be rather important that the lifts $\tilde P_{2l}$ and $\tilde P_{2l-1}$ live on two different components of the universal covering quiver.
...
To simplify the notation, we abbreviate $P_{2l-1,i}:=P_{2l-1,\alpha_i}$ and $P_{2l,i}:=P_{2l,\alpha_i^{-1}}$.
\begin{lemma}
  For each $m\ge2$, the representation $\tilde P_m$ has precisely $n$ subrepresentations covering $P_{m-1}$.
  These are the representations $P_{m-1,i}$ corresponding the $n$ different arrows of $K(n)$. 
\end{lemma}
\begin{proof}
...
\[
  \Ext(P_2^{(n-1)},P_1)\cong\langle (2,1)\xrightarrow{\alpha_l}(1,\alpha_l)\mid l=2,\ldots,n\rangle.
\]

This defines recursively the truncated preprojective representation $P_{2}^{(k)}$ for $k=n-1,\ldots,0$ with $P_2^{(0)}=P_2$ by the lifts 
\begin{align}
...
The next step is to define the $2$-quivers of certain truncated preprojectives $P^{(k)}_3$ whence the remaining $2$-quivers attached to truncated preprojectives $P_m^{(k)}$ can be obtained recursively. In view of Theorem \ref{thm:truncpp}, these $2$-quivers can be seen as being attached to any truncated preprojective of this dimension.

As in Section \ref{Lifting}, let $\tilde P_2$ be supported at $(2,1)$. Then we have 
$$\Ext(P_2^{(1)},P_2)\cong\bigoplus_{i=1}^{n-1}\Ext(\tilde P_2^{(1)},\alpha_i^{-1}\alpha_n\tilde P_2).$$
This decomposition induces a basis of $\Ext(P_2^{(1)},P_2)$, each basis element given by the linear map $$(2,1)\xlongrightarrow{\alpha_n}(1,\alpha_n)\in\Hom_{\CC}((\tilde P_2^{(1)})_{(2,1)}),(\alpha_i^{-1}\alpha_n\tilde P_2)_{(1,\alpha_n)})$$
for $i=1,\ldots,n-1$.

Now define $\tilde P_3^{(n-1)}:=\tilde P_2^{(1)}$

This basis gives rise to the following $2$-quiver of $P_3^{(k)}$ for $k=n-1,\ldots,0$ where we drop the indices of the vertices in abuse of notation. Note that the middle vertex of $P_3^{(n-1)}$ is $(2,1)$ and that all $2$-arrows end in a vertex corresponding to the vertex $(1,\alpha_n)$ of $\widetilde{K(n)}$.\\
...
\draw[->,red] (Q2)--(J4N1) node[pos=.5,above,sloped]{$\alpha_{n}$};
\node (Q3) [draw=black,fit={(P4) (Q2) (I4) (J41) (J42) (J4N1) (J4N)}]{};
\draw[->,red] (Q3)--(J3N2)  node[pos=.5,above,sloped] {$\alpha_{n}$};
\node (Q4) [draw=black,fit={(P3) (Q3) (I3) (J31) (J3N1) (J3N2) (J3N)}]{};
\draw[->,red] (Q4)--(DOTS)  node[pos=.5,above] {$\alpha_{n}$};
\node (Q4b) [draw=black,fit={(P2b) (Q4) (DOTS)}]{};

\node (Q5) [draw=black,fit={(P1) (Q4b) (I2) (J21) (J22) (J2N1) (J2N)}]{};
\draw[->,red] (Q5)--(J11)  node[pos=.5,above,sloped,sloped] {$\alpha_{n}$};
\draw[->,red] (Q4b)--(J22)  node[pos=.5,above,sloped] {$\alpha_{n}$};
\draw[->,red] (Q5)--(J11)  node[pos=.5,above,sloped] {$\alpha_{n}$};

\end{tikzpicture}
\\


We have $\dim \Hom(P_{m-1},\tau P_m^{(k)})=\dim\Ext(P_m^{(k)},P_{m-1})=n-k$. If a basis of $\Ext(P_m^{(k)},P_{m-1})$ is compatible with a lift to the universal covering,  there is a one-to-one correspondence between basis elements of $\Ext(P_m^{(k)},P_{m-1})$ and $\Hom(P_{m-1},\tau P_m^{(k)})$. For $m=3$, the lift of $\tau P_3^{(k)}$ is given by 
\[\xymatrix@R10pt@C20pt{&&(2,\alpha_1^{-1})\ar_{\alpha_1}[lld]\\ (1,1) &&\vdots\\&&(2,\alpha_k^{-1})\ar_{\alpha_{k}}[llu]}.\]
The image of the homomorphism of the corresponding exact sequence induced by the basis element $i_2^{(n-1)}\xrightarrow{\alpha_n} j_{2,l}$ is the quotient quiver $j\xlongleftarrow{\alpha_l} i_l$. Thus in this case the $2$-arrow connects the $2$-quiver of $P_3^{(k)}$ to the image of the corresponding homomorphism $P_2\to P_1^{(k)}$ which is of dimension $(1,1)$. In the above illustration, we forgo to insert the corresponding rectangles and just connected the $2$-arrows to the sinks of the corresponding subquivers. It can be seen that this is no restriction from the combinatorial point of view. 

We have $\dim \Hom(P_m,\tau P_m^{(k)})=\dim\Ext(P_m^{(k)},P_m)=n-k$. If a basis of $\Ext(P_m^{(k)},P_m)$ is compatible with a lift to the universal covering, it is straightforward to get a one-to-one correspondence between basis elements of $\Ext(P_m^{(k)},P_m)$ and $\Hom(P_{m-1},\tau P_m^{(k)})$.
