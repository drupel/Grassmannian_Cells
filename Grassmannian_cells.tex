...
\begin{lemma}\label{AR}\footnote{special case $m=3$}
 Let $m\geq 4$ and $n-1\geq l\geq 1$ and let $(j_1,\ldots,j_l)$ be an admissible sequence. Consider the short exact sequence $$\sesm{\tilde P_{m-1,j_l}}{\tilde P_{m}^{(j_1,\ldots,j_{l-1})}}{\tilde P_{m}^{(j_1,\ldots,j_l)}}{\pi^{(j_1,\ldots,j_l)}_m}$$. The following holds:
\begin{enumerate}
...
\[\sesm{\bigoplus_{\substack{1\leq i\leq n\\i\neq j_l}}\tilde P_{m-2,(j_l,i)}\oplus \bigoplus_{i=1}^{l-1}\tilde P_{m-3,(j_l,j_l,i)}}{\tilde P_{m-1,j_l}}{\tau\tilde P_{m}^{(l)}}{\pi^{\bf j_l}_{m-1,j_l}}\]
where ${\bf j}_l=((1,\ldots,j_l-1,\widehat{j_l},j_{l}+1,\ldots,n),(j_1,\ldots,j_{l-1})).$
\item We have $\Hom(\tilde P_{m}^{\uj},\tau \tilde P_{m}^{\uj})=0$, $\Ext(\tilde P_{m}^{\uj},\tau \tilde P_{m}^{\uj})\cong\Ext(\tilde P_{m}^{\uj},\tilde P_{m-1,j_l})$ and $\Hom(\tilde P_{m}^{\uj},\tilde P_{m,j_l})=0$ and thus $P(m,\uj)\in (\tilde P_{m}^{\uj})^\perp$. 
\item We have $\Ext(\tilde P_{m}^{(j_1,\ldots,j_l)},\tilde P_{m-1,j_l})\cong \Ext(\tilde P_{m}^{(j_1,\ldots,j_l)},\tau\tilde P_{m}^{(j_1,\ldots,j_{l})})=\kk$, $\Hom(\tilde P_{m}^{((j_1,\ldots,j_l)},\tau\tilde P_{m}^{(j_1,\ldots,j_l)})=0$. 
...
\end{lemma}
\begin{proof}
Using the identities (\ref{recursive}), it suffices to prove the all statements for $m=4$ and arbitrary $1\leq l\leq n-1$. Without loss of generality, we choose $(j_1,\ldots,j_l)=(1,\ldots,l)$ and define $\tilde P_m^{(0)}=\tilde P_m$ respectively. We should point out that it is very important for this prove that we consider fixed lifts of all representations in the universal covering as defined in the beginning of this section. Note that, starting with these cases, we only reflect at sinks and no representation appearing is isomorphic to the simple injective representation $S_2$.  

The representation $\tilde P_2$ is given determined by the following quiver and dimension vector:

\begin{align*}\xymatrix@R10pt@C20pt{&&\bullet\\ \bullet\ar^{a_1}[rru]\ar^{a_{n}}[rrd] &&\vdots\\&&\bullet}\end{align*}
The quiver of $\tilde P_2^{(i)}$, which we denote by $\mathcal A_i$ during this proof, can be obtained from this quiver by erasing the arrow $a_i$ and the corresponding sink. In turn, the quiver of $\tilde P_2^{(1,\ldots, l)}$ is obtained by erasing the arrows $a_{a_1},\ldots, a_l$. 

 Using this notation the support of $\tilde P_4$ is given by
...
%\end{tikzpicture}
Now it is straightforward that the representation $\tau \tilde P_4^{(1,\ldots,l)}=\tilde P_2^{(1,\ldots,l)}$, which is given by reflecting at all the sinks twice, is supported at the source lying in the upper row and the $n-l$ sinks connected to it by the arrows $a_{l+1},\ldots,a_n$. \footnote{say something concerning $W_Q$-translation?} This shows the first statement.

Considering the quiver $\tilde P_{3,l}$ from above, it is clear that $\tilde P_2^{(1,\ldots,l)}$ is a quotient. Moreover, the representation
$$\bigoplus_{\substack{1\leq j\leq n\\j\neq l}}\tilde P_{2,(l,j)}\oplus \bigoplus_{j=1}^{l-1}\tilde P_{1,(l,l,j)}$$
is the kernel of the corresponding map. Actually, the first summand corresponds to the quiver $\tilde A_i$ at the bottom and the second summand to the $l-1$ sources in the second row which lying on the left. This shows the second/third statement.
...
If $m=3$, we consider the sequence
 \[\xymatrix{0\ar[r] &P(3,\uj)\ar[r] &  \tilde P_{2,j_l}\ar[r] & \bullet\xrightarrow{a_{j_l}}\bullet\ar[r] & 0}\]	
	where the kernel is clearly just a direct sum of simple representations. In this case, the Auslander-Reiten translate of $\tau\tilde P_{3}^{(j_1,\ldots,j_l)}$ is given by
...
Let $\uj=(i_1,\ldots,i_l)$ be an admissible sequence. Every quiver Grassmannian $\Gr^{\tilde Q}_\tbfe(\tilde P_{m}^{\uj})$ admits a cell decomposition into affine spaces.
\end{theorem}
\begin{proof}
...
\noindent {\bf Claim.} Every quiver Grassmannian $\Gr^{\tilde Q}_\tbfe(\tilde P_{m}^{\uj})$ has a cell decomposition which is compatible with a fixed quotient map $\pi_m^{\bf j}$ with ${\bf j}_1=\uj$ in the following way: 
...
We proceed by induction on $m$ and reverse induction on $l$. If $m=1$ or $m=2$, the statement is straightforward as all quiver Grassmannians are points.
The short exact sequence $$\sesm{\tilde P_{m-1,j_l}}{\tilde P_{m}^{(j_1,\ldots,j_{l-1})}}{\tilde P_{m}^{\uj}}{\pi_m^{\uj}}$$ induces a map between quiver Grassmannians
$$\Psi_{m,\uj}:\Gr^{\tilde Q}_\tbfe(\tilde P_{m}^{(j_1,\ldots,j_{l-1})})\to\bigsqcup_{\tbff+\tbfg=\tbfe} \Gr^{\tilde Q}_\tbff( \tilde P_{m-1,j_l})\times \Gr^{\tilde Q}_\tbfg(\tilde P_{m}^{\uj}).$$
We consider the short exact sequence
$$\sesm{P(m,\uj)}{\tilde P_{m-1,j_l}}{\tau\tilde P_{m}^{\uj}}{\pi^{\bf j}_m}$$
with
$${\bf j}=((1,\ldots,j_l-1,\widehat{j_l},j_l+1,\ldots,n),(j_1,\ldots,j_{l-1}))$$
and $\tau\tilde P_{m}^{\uj} =\tilde P^{\uj}_{m-2}=\tilde P_{m-2}^{\bf j}$.
...
Thus it remains to show that there exists a cell decomposition of $\Gr^{\tilde Q}_\tbfe(\tilde P_{m}^{(j_1,\ldots,j_{l-1})})$ which is compatible with a fixed $\pi_m^{\bf j'}$,
